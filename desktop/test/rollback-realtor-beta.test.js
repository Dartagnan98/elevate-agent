...
function recoveryCandidateFixture(t) {
  ...
  const candidateFeed = feedFixture("1.2.98", "Elevate-Beta");
  const recoveryFeed = feedFixture("1.2.99", "Elevate-Beta-Recovery");
  const rollbackFeed = feedFixture("1.2.65", "Elevate");
  const stableFeed = feedFixture("1.2.63", "Elevate");
  const rollbackSnapshot = snapshot("beta", "1.2.65", rollbackFeed);
  const stableSnapshot = snapshot("latest", "1.2.63", stableFeed);
...
function recoveryRemoteFixture(t) {
  ...
  fs.mkdirSync(root);
  const stableBytes = Buffer.from("stable-feed-exact\n");
  const candidateFeedBytes = Buffer.from("candidate-beta-feed-1.2.98-exact\n");
  const recoveryFeedBytes = Buffer.from("retained-recovery-feed-1.2.99-exact\n");
  ...
  const candidateArtifacts = ["x64", "arm64"].flatMap((arch) => [
    { name: `Elevate-Beta-1.2.98-mac-${arch}.zip`, ...record(candidateZipBytes[arch]) },
    { name: `Elevate-Beta-1.2.98-mac-${arch}.dmg`, ...record(candidateDmgBytes[arch]) },
  ]);
  const recoveryArtifacts = ["x64", "arm64"].flatMap((arch) => [
    { name: `Elevate-Beta-Recovery-1.2.99-mac-${arch}.zip`, ...record(recoveryZipBytes[arch]) },
    { name: `Elevate-Beta-Recovery-1.2.99-mac-${arch}.dmg`, ...record(recoveryDmgBytesByArch[arch]) },
  ]);
...
test("recovery activation contract binds the exact receipt, recovery package, Stable, DMGs, and aliases", (t) => {
  ...
  assert.equal(contract.candidateId, fixture.receipt.candidate_id);
  assert.equal(contract.candidateReceiptSha256, fixture.options.expectedCandidateReceiptSha256);
  assert.equal(contract.candidateVersion, "1.2.98");
...
test("recovery activation contract fails closed for every receipt or package drift", (t) => {
  ...
  const mutations = [
    ...
    ["recovery not the pinned target", (receipt) => {
      receipt.recovery.version = "1.2.100";
      receipt.recovery.reserved_version = "1.2.100";
    }, /exact Beta 1\.2\.99/],
    ["recovery not a roll-forward", (receipt) => {
      receipt.release.version = "1.2.99";
      receipt.recovery.candidate_version = "1.2.99";
      receipt.release.artifact_names = receipt.release.artifact_names.map((name) => name.replace("1.2.98", "1.2.99"));
      receipt.artifacts = Object.fromEntries(Object.entries(receipt.artifacts).map(([name, value]) => [
        name.replace("1.2.98", "1.2.99"), value,
      ]));
    ...
    ["recovery artifact set drift", (receipt) => { receipt.recovery.artifact_names[0] = "Elevate-Beta-Recovery-1.2.99-mac-x64.pkg"; }, /exact retained recovery set/],
...
test("real recovery execute activates 1.2.99 from retained bytes only and commits the feed last", (t) => {
  const fixture = recoveryRemoteFixture(t);
  const script = buildRemoteRecoveryInnerScript({
    contract: fixture.contract,
    mode: "execute",
