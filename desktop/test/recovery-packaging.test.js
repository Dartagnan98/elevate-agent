...
test("recovery builder is an exact 1.2.99 Beta roll-forward package", () => {
  const build = config();
  assert.equal(RECOVERY_VERSION, "1.2.99");
  assert.equal(CANDIDATE_VERSION, "1.2.98");
  ...
  assert.equal(build.extraMetadata.version, "1.2.99");
  ...
  assert.equal(build.extraMetadata.elevateRecoveryCandidateVersion, "1.2.98");
...
test("recovery config fails closed without exact provenance or candidate version", () => {
  ...
  assert.throws(
    () => config({ manifest: { version: "1.2.99" } }),
    /bound to candidate 1\.2\.98/,
  );
});
...
test("recovery afterPack verifies exact bundle identity, updater lane, and runtime absence", async (t) => {
  ...
  assert.equal(evidence.recovery_version, "1.2.99");
  assert.equal(evidence.candidate_version, "1.2.98");
...
test("recovery provenance must match the exact Beta 1.2.98 source receipt", () => {
  let inputs = null;
  ...
  assert.equal(inputs.version, "1.2.98");
