...
test("ship requires static dual-arch and host live-AI evidence bound to the exact candidate", (t) => {
  ...
  fs.writeFileSync(candidateFeedPath, "version: 1.2.75\n");
  ...
  const recoveryArtifactNames = ["x64", "arm64"].flatMap((arch) => [
    `Elevate-Beta-Recovery-1.2.76-mac-${arch}.zip`,
    `Elevate-Beta-Recovery-1.2.76-mac-${arch}.dmg`,
  ]);
  const recoveryApps = {};
  const recoveryDmgs = {};
  for (const arch of ["x64", "arm64"]) {
    ...
    recoveryApps[arch] = {
      ...
      info_plist: {
        ...
        CFBundleShortVersionString: "1.2.76",
      },
      packaged_metadata: {
    ...
    recoveryDmgs[arch] = {
      artifact: `Elevate-Beta-Recovery-1.2.76-mac-${arch}.dmg`,
      evidence: dmgSigning,
  ...
  const primaryRecoveryZip = "Elevate-Beta-Recovery-1.2.76-mac-x64.zip";
  ...
  const recovery = {
    ...
    candidate_version: "1.2.75",
    version: "1.2.76",
    reserved_version: "1.2.76",
    next_full_beta_minimum_exclusive: "1.2.76",
    source_receipt_id: sourceReceiptId,
    channel: "beta",
...
test("recoveryStaticProvenance emits a pinned static-provenance key and shape contract", () => {
  ...
  assert.equal(provenance.candidate_version, "1.2.75");
  assert.equal(provenance.recovery_version, "1.2.76");
