...
test("ship requires static dual-arch and host live-AI evidence bound to the exact candidate", (t) => {
  ...
  fs.writeFileSync(candidateFeedPath, "version: 1.2.102\n");
  ...
  const recoveryArtifactNames = ["x64", "arm64"].flatMap((arch) => [
    `Elevate-Beta-Recovery-1.2.103-mac-${arch}.zip`,
    `Elevate-Beta-Recovery-1.2.103-mac-${arch}.dmg`,
  ]);
  const recoveryApps = {};
  const recoveryDmgs = {};
  for (const arch of ["x64", "arm64"]) {
    ...
    recoveryApps[arch] = {
      ...
      info_plist: {
        ...
        CFBundleShortVersionString: "1.2.103",
      },
      packaged_metadata: {
    ...
    recoveryDmgs[arch] = {
      artifact: `Elevate-Beta-Recovery-1.2.103-mac-${arch}.dmg`,
      evidence: dmgSigning,
  ...
  const primaryRecoveryZip = "Elevate-Beta-Recovery-1.2.103-mac-x64.zip";
  ...
  const recovery = {
    ...
    candidate_version: "1.2.102",
    version: "1.2.103",
    reserved_version: "1.2.103",
    next_full_beta_minimum_exclusive: "1.2.103",
    source_receipt_id: sourceReceiptId,
    channel: "beta",
...
test("later Betas require explicit evidence that the pinned recovery contract is not applicable", (t) => {
  ...
  const receiptPath = path.join(root, "candidate-receipt.json");
  const receipt = {
    candidate_id: "candidate-1.2.102",
    source_receipt_id: "source-1.2.102",
    production_feed_untouched: true,
    release: {
      version: "1.2.102",
      channel: "beta",
...
test("recoveryStaticProvenance emits a pinned static-provenance key and shape contract", () => {
  ...
  assert.equal(provenance.candidate_version, "1.2.102");
  assert.equal(provenance.recovery_version, "1.2.103");
