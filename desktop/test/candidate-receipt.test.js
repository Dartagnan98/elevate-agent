...
test("ship requires static dual-arch and host live-AI evidence bound to the exact candidate", (t) => {
  ...
  fs.writeFileSync(candidateFeedPath, "version: 1.2.93\n");
  ...
  const recoveryArtifactNames = ["x64", "arm64"].flatMap((arch) => [
    `Elevate-Beta-Recovery-1.2.94-mac-${arch}.zip`,
    `Elevate-Beta-Recovery-1.2.94-mac-${arch}.dmg`,
  ]);
  const recoveryApps = {};
  const recoveryDmgs = {};
  for (const arch of ["x64", "arm64"]) {
    ...
    recoveryApps[arch] = {
      ...
      info_plist: {
        ...
        CFBundleShortVersionString: "1.2.94",
      },
      packaged_metadata: {
    ...
    recoveryDmgs[arch] = {
      artifact: `Elevate-Beta-Recovery-1.2.94-mac-${arch}.dmg`,
      evidence: dmgSigning,
  ...
  const primaryRecoveryZip = "Elevate-Beta-Recovery-1.2.94-mac-x64.zip";
  ...
  const recovery = {
    ...
    candidate_version: "1.2.93",
    version: "1.2.94",
    reserved_version: "1.2.94",
    next_full_beta_minimum_exclusive: "1.2.94",
    source_receipt_id: sourceReceiptId,
    channel: "beta",
...
test("later Betas require explicit evidence that the pinned recovery contract is not applicable", (t) => {
  ...
  const receiptPath = path.join(root, "candidate-receipt.json");
  const receipt = {
    candidate_id: "candidate-1.2.95",
    source_receipt_id: "source-1.2.95",
    production_feed_untouched: true,
    release: {
      version: "1.2.95",
      channel: "beta",
...
test("recoveryStaticProvenance emits a pinned static-provenance key and shape contract", () => {
  ...
  assert.equal(provenance.candidate_version, "1.2.93");
  assert.equal(provenance.recovery_version, "1.2.94");
