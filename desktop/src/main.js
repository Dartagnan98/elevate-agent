...
function startSmsOutboxWatcher() {
  ...
  const drain = () => {
    ...
    for (const f of files) {
      ...
      child.on("close", (code) => {
        clearTimeout(killer);
        writeRes({ ok: code === 0, code, stdout: out.slice(-500), error: code === 0 ? null : (err || out).slice(-300) });
      });
      child.on("error", (e) => { clearTimeout(killer); writeRes({ ok: false, error: String(e).slice(-300) }); });
    }
  };
