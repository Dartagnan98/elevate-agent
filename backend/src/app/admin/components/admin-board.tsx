...
function AdminBoard() {
  ...
  const activeDeals    = isBuyer ? ADMIN_BUYER_DEALS    : ADMIN_DEALS;
  ...
  const totals = useMemo(() => {
    const deals = ADMIN_DEALS;
    const blocked = deals.filter(d => d.blocked).length;
    const review  = ADMIN_ACTIONS.filter(a => a.kind === "review").length;
    const live    = ADMIN_AUTOMATIONS.filter(a => a.status === "scheduled").length;
    const broken  = ADMIN_AUTOMATIONS.filter(a => a.status === "error" || a.status === "blocked").length;
    const health  = Math.round(((ADMIN_AUTOMATIONS.length - broken) / ADMIN_AUTOMATIONS.length) * 100);
    return { active: deals.length, blocked, review, health };
  }, []);
    ...
    <main className="admin-board">
      <header className="ab-top">
        ...
        <div className="ab-top-actions">
          <button className="ab-btn ghost" type="button"><Refresh /><span>Refresh</span></button>
          <button className="ab-btn ghost" type="button"><Sparkles /><span>Re-run onboarding</span></button>
      ...
      <div className="ab-scroll">
        {/* Top 25 deals strip */}
        <Top25Deals deals={activeDeals} mode={tab} onOpenDeal={setActiveDeal} />

        {/* KPI tiles */}
        <section className="ab-kpis">
          <KpiTile label="Pipeline value"        value="$4.2M" breakdown="15 listing · 3 buyer"     delta="+$320K this month"  deltaTone="up" />
          <KpiTile label="Closing this month"    value="5"     breakdown="$84K projected"          delta="2 firm · 3 conditional" />
          <KpiTile label="Avg time to close"     value="38d"   breakdown="intake → closed"         delta="−4d vs last quarter" deltaTone="up" />
          <KpiTile label="Drafts awaiting approval" value="7"  breakdown="3 SMS · 4 email"         delta="2 urgent"            deltaTone="warn" />
          <KpiTile label="Inbox triage"          value="12"    breakdown="Lofty 5 · iMessage 4 · Gmail 3" delta="3 hot leads"  deltaTone="warn" />
          <KpiTile label="Documents missing"     value="14"    breakdown="across 6 deals"          delta="2 overdue"           deltaTone="warn" />
          <KpiTile label="Compliance status"     value="87%"   breakdown="FINTRAC + signing"       delta="2 to verify"         deltaTone="warn" />
          <KpiTile label="Upcoming showings"     value="5"     breakdown="this week"               delta="next · Today 4:30 PM" />
        </section>
          ...
          <header className="ab-card-head">
            <div className="ab-tabs">
              <button className={"ab-tab" + (tab === "listing" ? " active" : "")} onClick={() => setTab("listing")}>
                <span>Listing admin</span><span className="count mono">{ADMIN_DEALS.length}</span>
              </button>
              <button className={"ab-tab" + (tab === "buyer" ? " active" : "")} onClick={() => setTab("buyer")}>
                <span>Buyer admin</span><span className="count mono">{ADMIN_BUYER_DEALS.length}</span>
              </button>
              <CyclingShowings items={ADMIN_SHOWINGS || []} />
            </div>
