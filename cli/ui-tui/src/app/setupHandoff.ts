import type { RunExternalProcess } from '@elevate/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`elevate ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching elevate: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`elevate ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
