...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', () => {
    expect(DEFAULT_THEME.brand.name).toBe('Elevate')
...
describe('detectLightMode', () => {
  ...
  it('honors ELEVATE_TUI_LIGHT on/off', () => {
    expect(detectLightMode({ ELEVATE_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ ELEVATE_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ ELEVATE_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ ELEVATE_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ ELEVATE_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets ELEVATE_TUI_LIGHT=0 override a light COLORFGBG', () => {
    expect(detectLightMode({ COLORFGBG: '0;15', ELEVATE_TUI_LIGHT: '0' })).toBe(false)
  })
})
