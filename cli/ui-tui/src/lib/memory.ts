...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.ELEVATE_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.elevate', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `elevate-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
