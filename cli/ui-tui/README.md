...
React + Ink terminal UI for Elevate. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ELEVATE_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.elevate/.elevate_history` or under `ELEVATE_HOME`.
...
```text
ui-tui/
  packages/elevate-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      elevate-ink.d.ts    type declarations for @elevate/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ElevateCLI subprocess for slash commands
```
