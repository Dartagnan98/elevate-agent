...
_DEFAULT_CONTAINER_TAG = "elevate"
...
def _load_supermemory_config(elevate_home: str) -> dict:
    config = _default_config()
    config_path = Path(elevate_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, elevate_home: str) -> None:
    config_path = Path(elevate_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `elevate memory setup`.
        # All other options are documented for $ELEVATE_HOME/supermemory.json
    ...
    def save_config(self, values, elevate_home):
        ...
        _save_supermemory_config(sanitized, elevate_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from elevate_constants import get_elevate_home
        self._elevate_home = kwargs.get("elevate_home") or str(get_elevate_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._elevate_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "elevate", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "elevate_memory", "target": target, "type": "explicit_memory"},
