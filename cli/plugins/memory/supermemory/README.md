...
Config file: `$ELEVATE_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `elevate` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `elevate-{identity}` → `elevate-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Elevate profile:

```json
{
  "container_tag": "elevate-{identity}"
}
```

For a profile named `coder`, this resolves to `elevate-coder`. The default profile resolves to `elevate-default`. Without `{identity}`, all profiles share the same container.
