...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with elevate)
2. User-installed providers: ``$ELEVATE_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ELEVATE_HOME/plugins/`` or None if unavailable."""
    try:
        from elevate_constants import get_elevate_home
        d = get_elevate_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ELEVATE_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_elevate_user_memory.{active_provider}.cli"
    try:
