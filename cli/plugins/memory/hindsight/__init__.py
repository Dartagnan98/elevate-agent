...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: elevate)
...
Or via $ELEVATE_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Elevate can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ELEVATE_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_elevate_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "elevate": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "elevate"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Elevate config."""
    profile = config.get("profile", "elevate")
    return str(profile or "elevate")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Elevate profile name (from agent_identity)
      {workspace} — Elevate workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``elevate-{user}`` with no user becomes ``elevate``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Elevate and the User"
    ...
    def save_config(self, values, elevate_home):
        """Write config to $ELEVATE_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(elevate_home) / "hindsight"
    ...
    def post_setup(self, elevate_home: str, config: dict) -> None:
        ...
        from elevate_cli.memory_setup import _curses_select
        ...
        self.save_config(provider_config, elevate_home)

        if env_writes:
            env_path = Path(elevate_home) / ".env"
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(elevate_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(elevate_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "elevate"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: elevate-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Elevate and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "elevate"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "elevate"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("elevate", {})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "elevate")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Elevate and the User")
