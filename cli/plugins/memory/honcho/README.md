...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/elevate>
...
```bash
elevate honcho setup    # full interactive wizard (cloud or local)
elevate memory setup    # generic picker, also works
```

Or manually:
```bash
elevate config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.elevate/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ELEVATE_HOME/honcho.json` | Profile-local (isolated Elevate instances) |
| 2 | `~/.elevate/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Elevate profile: `elevate` (default) or `elevate.<profile>`.
...
| 4 | `per-session` strategy | Elevate session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`elevate`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`elevate`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-elevate`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening elevate in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Elevate session ID (timestamp + hex). Every `elevate` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Elevate profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`elevate`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Elevate profile: `elevate` (default) or `elevate.<profile>` (e.g. `elevate -p coder` → host key `elevate.coder`).
...
| `ELEVATE_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `elevate honcho setup` | Full interactive setup wizard |
| `elevate honcho status` | Show resolved config for active profile |
| `elevate honcho enable` / `disable` | Toggle Honcho for active profile |
| `elevate honcho mode <mode>` | Change recall or observation mode |
| `elevate honcho peer --user <name>` | Update user peer name |
| `elevate honcho peer --ai <name>` | Update AI peer name |
| `elevate honcho tokens --context <N>` | Set context token budget |
| `elevate honcho tokens --dialectic <N>` | Set dialectic max chars |
| `elevate honcho map <name>` | Map current directory to a session name |
| `elevate honcho sync` | Create host blocks for all Elevate profiles |
