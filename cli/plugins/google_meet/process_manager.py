...
session-scoped state file under ``$ELEVATE_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ELEVATE_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_elevate_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    env = os.environ.copy()
    env["ELEVATE_MEET_URL"] = url
    env["ELEVATE_MEET_OUT_DIR"] = str(out)
    env["ELEVATE_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ELEVATE_MEET_HEADED"] = "1"
    if auth_state:
        env["ELEVATE_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ELEVATE_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ELEVATE_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ELEVATE_MEET_MODE"] = mode
    if realtime_model:
        env["ELEVATE_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ELEVATE_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ELEVATE_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ELEVATE_MEET_REALTIME_KEY"] = realtime_api_key
