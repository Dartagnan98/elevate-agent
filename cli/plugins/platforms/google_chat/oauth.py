...
- Per-user tokens (keyed by sender email):
    ``${ELEVATE_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ELEVATE_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ELEVATE_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ELEVATE_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${ELEVATE_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's ELEVATE_HOME helper so the token follows the user's
# profile (e.g. tests can override via ELEVATE_HOME=/tmp/...).
try:
    from elevate_constants import display_elevate_home, get_elevate_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where elevate_constants isn't importable
...
def _hermes_home() -> Path:
    """Resolve ELEVATE_HOME at call time (NOT module import).

    Tests and ``ELEVATE_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.elevate/google_chat_user_tokens/`` trivial.
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ELEVATE_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_elevate_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_elevate_home()}/google_chat_user_token.json"
    )
