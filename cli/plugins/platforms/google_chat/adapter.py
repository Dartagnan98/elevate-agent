...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ELEVATE_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from elevate_constants import get_elevate_home as _get_elevate_home
            _hermes_home = _get_elevate_home()
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ELEVATE_HOME", str(_Path.home() / ".hermes"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ELEVATE_HOME}/google_chat_user_tokens/<sanitized>.json
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``elevate_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.elevate/.env`` so the next
    gateway restart picks them up.
    """
    from elevate_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print()
    print_success("Google Chat configuration saved to ~/.elevate/.env")
