...
Auto-tracks and cleans up ephemeral files created during Elevate
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ELEVATE_HOME` and `/tmp/elevate-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ELEVATE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ELEVATE_HOME` or `/tmp/elevate-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ELEVATE_HOME/disk-cleanup/` is itself excluded
- `$ELEVATE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
