"""disk_cleanup — ephemeral file cleanup for Elevate.
...
Rules:
  ...
  - empty dirs    → always delete (under ELEVATE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ELEVATE_HOME and /tmp/elevate-*
Never touches: ~/.elevate/logs/ or any system directory.
"""
...
try:
    ...
    def get_elevate_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ELEVATE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".elevate").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ELEVATE_HOME/logs/``."""
    return get_elevate_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ELEVATE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ELEVATE_HOME or ``/tmp/elevate-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    elevate_home = get_elevate_home()
    try:
        path.resolve().relative_to(elevate_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/elevate-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("elevate-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ELEVATE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ELEVATE_HOME (but leave ELEVATE_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "elevate", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(elevate_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == elevate_home:
                continue
            try:
                rel_parts = dirpath.relative_to(elevate_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    elevate_home = get_elevate_home()
    try:
        rel = path.resolve().relative_to(elevate_home)
        ...
        # Path isn't under ELEVATE_HOME (e.g. /tmp/elevate-*) — fall through.
        pass
