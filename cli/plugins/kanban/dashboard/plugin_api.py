...
``elevate_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__ELEVATE_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``elevate_cli/web_server.py``.
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from elevate_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``elevate_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from elevate_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from elevate_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``ELEVATE_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from elevate_cli.kanban import _check_dispatcher_presence
...
# spawn failures, stuck-blocked). See elevate_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    prev_env = os.environ.get("ELEVATE_KANBAN_BOARD")
    try:
        os.environ["ELEVATE_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from elevate_cli import kanban_specify  # noqa: WPS433 (intentional)
        ...
        if prev_env is None:
            ...
            os.environ["ELEVATE_KANBAN_BOARD"] = prev_env
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.elevate/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.elevate/profiles/*`` on disk and every
...
def list_profile_roster():
    ...
    """
    try:
        from elevate_cli import profiles as profiles_mod
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from elevate_constants import get_elevate_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_elevate_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from elevate_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    try:
        os.environ["ELEVATE_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from elevate_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.elevate/config.yaml.
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in elevate_cli/web_server.py.
