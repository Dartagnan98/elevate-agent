...
1. **Theme** — copy the theme YAML into your Elevate home:

   ```
   cp theme/strike-freedom.yaml ~/.elevate/dashboard-themes/
   ```
...
2. **Plugin** — the `dashboard/` directory gets auto-discovered because
   ...
   whole plugin directory into `~/.elevate/plugins/`:

   ```
   cp -r . ~/.elevate/plugins/strike-freedom-cockpit
   ```
