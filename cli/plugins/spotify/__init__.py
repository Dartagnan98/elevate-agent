...
``_check_spotify_available()`` — when the user has not run ``elevate auth
spotify``, the tools remain registered (so they appear in ``elevate
...
The Spotify auth flow (``elevate auth spotify``), CLI plumbing, and docs
