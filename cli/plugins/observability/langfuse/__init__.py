...
Required env vars (set in ~/.elevate/.env):
  ELEVATE_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  ELEVATE_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  ELEVATE_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  ELEVATE_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  ELEVATE_LANGFUSE_RELEASE     - release/version tag
  ELEVATE_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  ELEVATE_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  ELEVATE_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("ELEVATE_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("ELEVATE_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("ELEVATE_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("ELEVATE_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("ELEVATE_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset ELEVATE_LANGFUSE_PUBLIC_KEY / ELEVATE_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("ELEVATE_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("ELEVATE_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("ELEVATE_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("ELEVATE_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid ELEVATE_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("ELEVATE_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
