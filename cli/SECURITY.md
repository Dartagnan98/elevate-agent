# Elevate Security Policy

This document outlines the security protocols, trust model, and deployment hardening guidelines for **Elevate**.

## 1. Vulnerability Reporting

Elevate does **not** operate a bug bounty program. Security issues should be reported by emailing **security@elevationrealestatehq.com**. Do not open public issues for security vulnerabilities.
...
- **Environment:** Output of `elevate version`, commit SHA, OS, and Python version.
...
The core assumption is that Elevate is a **personal agent** with one trusted operator.
...
- **Trusted State Access:** Reports that require pre-existing write access to `~/.elevate/`, `.env`, or `config.yaml` (these are operator-owned files).
...
- **File permissions:** Run as non-root (the Docker image uses UID 10000); protect credentials with `chmod 600 ~/.elevate/.env` on local installs.
...
### Skills & Supply Chain
- **Skill installation:** Review Skills Guard reports (`tools/skills_guard.py`) before installing third-party skills. The audit log at `~/.elevate/skills/.hub/audit.log` tracks every install and removal.
...
### Credential Storage
- API keys and tokens belong exclusively in `~/.elevate/.env` — never in `config.yaml` or checked into version control.
...
- **Communication:** All updates occur via email correspondence with security@elevationrealestatehq.com.
