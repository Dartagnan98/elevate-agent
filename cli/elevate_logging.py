"""Centralized logging setup for Elevate.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.elevate/logs/`` (profile-aware via ``get_elevate_home()``).
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_elevate_session_injector", False):
    ...
    _session_record_factory._elevate_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``elevate logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the Elevate logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    elevate_home
        Override for the Elevate home directory.  Falls back to
        ``get_elevate_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_elevate_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._elevate_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the elevate group. However, both _open() (initial creation) and
