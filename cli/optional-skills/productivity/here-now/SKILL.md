...
- Skill helper paths:
  - `${ELEVATE_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ELEVATE_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ELEVATE_SKILL_DIR}/scripts/publish.sh"
...
```bash
DRIVE="${ELEVATE_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
