...
## 5. Elevate Configuration

### Config location

`$ELEVATE_HOME/config.yaml` (defaults to `~/.elevate/config.yaml` when `ELEVATE_HOME` is unset)
...
Restart the Elevate session for changes to take effect. The MCP connection is
