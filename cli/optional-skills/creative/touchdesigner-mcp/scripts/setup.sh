...
ELEVATE_HOME_DIR="${ELEVATE_HOME:-$HOME/.elevate}"
ELEVATE_CFG="${ELEVATE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Elevate config has twozero_td MCP entry ──
if [[ ! -f "$ELEVATE_CFG" ]]; then
    echo -e " ${FAIL} Elevate config not found at ${ELEVATE_CFG}"
    manual_steps+=("Create ${ELEVATE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ELEVATE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Elevate config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Elevate config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$ELEVATE_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ELEVATE_CFG} manually"); }
    manual_steps+=("Restart Elevate session to pick up config change")
fi
