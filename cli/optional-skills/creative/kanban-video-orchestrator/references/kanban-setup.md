...
3. **Configure profiles** — patch each profile's
   `~/.elevate/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
...
Each profile has a YAML config at `~/.elevate/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.elevate/profiles/{profile}/config.yaml")
with open(p) as f:
...
Each profile gets a `SOUL.md` at `~/.elevate/profiles/<name>/SOUL.md` that
...
`~/.elevate/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.elevate/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.elevate/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.elevate/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}
