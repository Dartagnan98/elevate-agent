...
```bash
mkdir -p ~/.elevate/cache/darwinian-evolver && cd ~/.elevate/cache/darwinian-evolver
...
```bash
cd ~/.elevate/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.elevate/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.elevate/skills/research/darwinian-evolver
DE_DIR=~/.elevate/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hermes core.
   Custom driver scripts under `~/.elevate/skills/...` are user-side and fine.
