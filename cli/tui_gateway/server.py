...
load_elevate_dotenv(
    elevate_home=_elevate_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.elevate/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_elevate_home, "logs", "tui_gateway_crash.log")
...
_SLASH_WORKER_TIMEOUT_S = max(
    5.0, float(os.environ.get("ELEVATE_TUI_SLASH_TIMEOUT_S", "45") or 45)
)
...
_pool = concurrent.futures.ThreadPoolExecutor(
    max_workers=max(2, int(os.environ.get("ELEVATE_TUI_RPC_POOL_WORKERS", "4") or 4)),
...
def _save_cfg(cfg: dict):
    ...
    path = _elevate_home / "config.yaml"
...
def resolve_skin() -> dict:
    try:
        from elevate_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from elevate_cli.banner import get_available_skills
    ...
    try:
        ...
        info["mcp_servers"] = []
    try:
        from elevate_cli.banner import get_update_result
    ...
    **_kwargs,
    ...
    if event_type.startswith("subagent."):
        ...
        if _kwargs.get("child_session_id"):
            # The subagent's own session id — lets the UI open + message it.
...
def _resolve_personality_prompt(cfg: dict) -> str:
    ...
    try:
        ...
        try:
            from elevate_cli.config import load_config as _load_full_cfg
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"elevate_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
#
# Layout:  $ELEVATE_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_elevate_home() / "spawn-trees"
    ...
    def run():
        ...
        try:
            ...
            if (
                ...
                try:
                    from elevate_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: elevate_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from elevate_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _elevate_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from elevate_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_elevate_home), "display": display_elevate_home()})
    if key == "full":
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `elevate` is interactive — use `/elevate chat -q …` or run `elevate` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`elevate setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`elevate gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`elevate sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`elevate config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m elevate_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "elevate_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from elevate_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        ...
        # "models" with the curated list (same source as `elevate model` and
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ELEVATE_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ELEVATE_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in ("on", "off"):
        ...
        os.environ["ELEVATE_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from elevate_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["ELEVATE_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from elevate_cli.voice import start_continuous
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("ELEVATE_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ELEVATE_BASE_URL", "") or cfg.get("base_url", "")
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "search":
            from elevate_cli.skills_hub import (
                unified_search,
                GitHubAuth,
        ...
        if action == "install":
            from elevate_cli.skills_hub import do_install
        ...
        if action == "browse":
            from elevate_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from elevate_cli.skills_hub import inspect_skill
