...
description: "Run Elevate directly on an Android phone with Termux"
---

# Elevate on Android with Termux

This is the tested path for running Elevate directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Elevate from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `elevate` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `elevate` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/elevate" "$PREFIX/bin/elevate"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `elevate` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.elevate/.env`.
...
### `elevate doctor` says ripgrep or Node is missing
