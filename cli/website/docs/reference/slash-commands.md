...
Elevate has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `elevate_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.elevate/plans/` relative to the active workspace/backend working directory.
...
User-defined quick commands map a short alias to a longer prompt. Configure them in `~/.elevate/config.yaml`:
...
| `/compact [focus topic]` | Manually compact conversation context. Optional focus topic narrows what the summary preserves. |
