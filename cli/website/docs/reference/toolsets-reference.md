...
description: "Reference for Elevate core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `elevate-cli` is the default for interactive CLI sessions)
...
```bash
elevate chat --toolsets web,file,terminal
elevate chat --toolsets debugging        # composite — expands to file + terminal + web
elevate chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - elevate-cli          # default for CLI
  # - elevate-telegram   # override for Telegram gateway
```

### Interactive management

```bash
elevate tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Elevate tools programmatically. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `elevate-cli` or other messaging toolsets. |
...
Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `elevate-cli`:

| Toolset | Differences from `elevate-cli` |
|---------|-------------------------------|
| `elevate-cli` | Full toolset — all 36 core tools including `clarify`. The default for interactive CLI sessions. |
| `elevate-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, homeassistant tools. Focused on coding tasks in IDE context. |
| `elevate-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Adds everything else — suitable for programmatic access where user interaction isn't possible. |
| `elevate-telegram` | Same as `elevate-cli`. |
| `elevate-discord` | Same as `elevate-cli`. |
| `elevate-slack` | Same as `elevate-cli`. |
| `elevate-whatsapp` | Same as `elevate-cli`. |
| `elevate-signal` | Same as `elevate-cli`. |
| `elevate-matrix` | Same as `elevate-cli`. |
| `elevate-mattermost` | Same as `elevate-cli`. |
| `elevate-email` | Same as `elevate-cli`. |
| `elevate-sms` | Same as `elevate-cli`. |
| `elevate-bluebubbles` | Same as `elevate-cli`. |
| `elevate-dingtalk` | Same as `elevate-cli`. |
| `elevate-feishu` | Same as `elevate-cli`. Note: the `feishu_doc` / `feishu_drive` toolsets are used only by the document-comment handler, not by the regular Feishu chat adapter. |
| `elevate-qqbot` | Same as `elevate-cli`. |
| `elevate-wecom` | Same as `elevate-cli`. |
| `elevate-wecom-callback` | Same as `elevate-cli`. |
| `elevate-weixin` | Same as `elevate-cli`. |
| `elevate-homeassistant` | Same as `elevate-cli` plus the `homeassistant` toolset always on. |
| `elevate-webhook` | Same as `elevate-cli`. |
| `elevate-gateway` | Internal gateway orchestrator toolset — union of the broadest possible tool set when the gateway needs to accept any message source. |
...
## Relationship to `elevate tools`

The `elevate tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
