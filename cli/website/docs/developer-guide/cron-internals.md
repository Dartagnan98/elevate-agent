...
description: "How Elevate stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `elevate_cli/cron.py` | CLI `elevate cron` subcommands |
...
Jobs are stored in `~/.elevate/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `elevate cron` commands are run or during active CLI sessions.
...
```python
# ~/.elevate/scripts/check_competitors.py
...
2. **Environment variable** — `ELEVATE_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.elevate/cron/output/` |
...
The `elevate cron` CLI provides direct job management:

```bash
elevate cron list                    # Show all jobs
elevate cron create                  # Interactive job creation (alias: add)
elevate cron edit <job_id>           # Edit job configuration
elevate cron pause <job_id>          # Pause a running job
elevate cron resume <job_id>         # Resume a paused job
elevate cron run <job_id>            # Trigger immediate execution
elevate cron remove <job_id>         # Delete a job
```
