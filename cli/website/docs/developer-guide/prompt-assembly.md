...
description: "How Elevate builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Elevate deliberately separates:
...
1. agent identity — `SOUL.md` from `ELEVATE_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.elevate/SOUL.md)
You are Elevate, an AI assistant created by Elevation Real Estate HQ.
...
`SOUL.md` lives at `~/.elevate/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are Elevate, an intelligent AI assistant created by Elevation Real Estate HQ.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_elevate_md(cwd_path)       # 1. .elevate.md / ELEVATE.md (walks to git root)
    ...
    # SOUL.md from ELEVATE_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.elevate.md`, `ELEVATE.md` | CWD up to git root | Elevate-native project config |
...
- **YAML frontmatter stripped** — `.elevate.md` frontmatter is removed (reserved for future config overrides)
...
1. `.elevate.md` / `ELEVATE.md` (walks to git root)
