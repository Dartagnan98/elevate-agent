...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Elevate Atropos integration"
---

# Environments, Benchmarks & Data Generation

Elevate includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Elevate/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- ElevateAgentBaseEnv
    ElevateAgentBaseEnv <|-- TerminalTestEnv
    ElevateAgentBaseEnv <|-- ElevateSweEnv
    ElevateAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The elevate layer (`environments/elevate_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls elevate's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `ElevateAgentLoop` and scores the result
...
Your environment inherits from `ElevateAgentBaseEnv` and implements five methods:
...
`ElevateAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as elevate's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any elevate tool |
...
parser = get_parser("elevate")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `elevate`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"elevate"`, `"qwen"`, `"mistral"`)
...
```python
from environments.elevate_base_env import ElevateAgentBaseEnv, ElevateAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(ElevateAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(ElevateAgentBaseEnv):
    name = "my-env"
...
### ElevateAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which elevate toolsets to enable |
...
| `tool_call_parser` | `str` | `"elevate"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "elevate[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "elevate[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── elevate_base_env.py          # Abstract base class (ElevateAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (ElevateAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── elevate_parser.py        # Elevate/ChatML <tool_call> format
...
├── elevate_swe_env/             # SWE-bench training environment
│
