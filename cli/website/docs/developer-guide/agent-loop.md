...
Elevate supports three API execution modes, resolved from provider selection, explicit args, and base URL heuristics:
...
- Default: 150 iterations (configurable via `agent.max_turns`)
...
After each turn:
- Messages are saved to the session store (SQLite via `elevate_state.py`)
- Memory changes are flushed to `MEMORY.md` / `USER.md`
- The session can be resumed later via `/resume` or `elevate chat --resume`
