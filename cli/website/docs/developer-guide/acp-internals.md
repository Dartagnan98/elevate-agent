...
The ACP adapter wraps Elevate' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
elevate acp / elevate-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.elevate/.env
  -> configure stderr logging
  -> construct ElevateACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Elevate `once`
- `allow_always` -> Elevate `always`
- reject options -> Elevate `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Elevate tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["elevate-acp"])
...
Instead it reuses Elevate' runtime resolver:

- `acp_adapter/auth.py`
- `elevate_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Elevate provider/credentials.
...
- `toolsets.py` — `elevate-acp` toolset definition
- `elevate_cli/main.py` — `elevate acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `elevate-acp` script
