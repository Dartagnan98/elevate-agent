...
sidebar_position: 1
title: "Architecture"
description: "Elevate internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Elevate internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ elevate_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ElevateCLI — interactive terminal UI (~10,000 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── elevate_state.py           # SQLite session/state database with FTS5
├── elevate_constants.py       # ELEVATE_HOME, profile-aware paths
...
│
├── elevate_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `elevate` subcommands (~6,000 lines)
...
│   ├── skills_config.py      # elevate skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # elevate tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # elevate gateway start/stop
│
...
```text
User input → ElevateCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .elevate.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.elevate/plugins/` (user), `.elevate/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `elevate plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-elevate-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Elevate as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`elevate -p <name>`) gets its own ELEVATE_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
