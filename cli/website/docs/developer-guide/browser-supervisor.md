...
[PR #12550](https://github.com/Dartagnan98/elevate-agent/pull/12550) proposed a
...
(`elevate-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Elevate `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `elevate-acp`, `elevate-api-server`, core toolsets (gated on CDP reachability)
...
- `elevate_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
