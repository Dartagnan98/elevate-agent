...
The messaging gateway is the long-running process that connects Elevate to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `elevate_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.elevate/.env` | API keys, bot tokens, platform credentials |
| `~/.elevate/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.elevate/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `elevate gateway start` / `elevate gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.elevate/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `elevate gateway stop` stops only the current profile's gateway. `elevate gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
