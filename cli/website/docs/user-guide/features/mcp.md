...
description: "Connect Elevate to external tool servers via MCP — and control exactly which MCP tools Elevate loads"
---

# MCP (Model Context Protocol)

MCP lets Elevate connect to external tool servers so the agent can use tools that live outside Elevate itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Elevate to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Elevate tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Elevate to see
...
2. Add an MCP server to `~/.elevate/config.yaml`:
...
4. Ask Elevate to use the MCP-backed capability.
...
Elevate will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Elevate connects to directly.
...
- you do not want Elevate spawning a local subprocess for that integration

## Basic configuration reference

Elevate reads MCP config from `~/.elevate/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Elevate skips the server entirely |
...
## How Elevate registers MCP tools

Elevate prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Elevate sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Elevate also registers utility tools around MCP resources and prompts:
...
- Elevate only registers resource utilities if the MCP session actually supports resource operations
- Elevate only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Elevate, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Elevate skips the server completely and does not even attempt a connection.
...
You can also separately disable Elevate-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Elevate does not create an empty runtime MCP toolset for that server.
...
Elevate discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Elevate when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Elevate receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Elevate does not blindly pass your full shell environment.
...
Then verify your config and restart Elevate.
...
Because Elevate now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from Elevate via the `sampling/createMessage` protocol. This allows an MCP server to ask Elevate to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Elevate as an MCP server

In addition to connecting **to** MCP servers, Elevate can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Elevate's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Elevate
- You want a single MCP server that bridges to all of Elevate's connected messaging platforms at once
- You already have a running Elevate gateway with connected platforms
...
Add Elevate to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Elevate in a specific location:

```json
{
  "mcpServers": {
    "elevate": {
      "command": "/home/user/.elevate/elevate/venv/bin/elevate",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Elevate-specific channel browser:
...
The MCP server includes a live event bridge that polls Elevate's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
elevate mcp serve              # Normal mode
elevate mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Elevate's session store (`~/.elevate/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Elevate agent itself.
