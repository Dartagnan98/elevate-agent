...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Elevate — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Elevate supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Elevate](/docs/guides/use-voice-mode-with-elevate).

## Prerequisites

Before using voice features, make sure you have:

1. **Elevate installed** — `pip install elevate` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `elevate model` or set your preferred provider credentials in `~/.elevate/.env`
3. **A working base setup** — run `elevate` to verify the agent responds to text before enabling voice

:::tip
The `~/.elevate/` directory and default `config.yaml` are created automatically the first time you run `elevate`. You only need to create `~/.elevate/.env` manually for API keys.
:::
...
pip install "elevate[messaging]"

# Premium TTS (ElevenLabs)
pip install "elevate[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "elevate[all]"
```
...
```bash
elevate                # Start the interactive CLI
```
...
1. Start the CLI with `elevate` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.elevate/config.yaml` (default: `ctrl+b`).
:::
...
```bash
elevate gateway        # Start the gateway (connects to configured platforms)
elevate gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@elevatebyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.elevate/.env`:
```bash
...
```bash
elevate gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.elevate/.env`
...
- Review gateway logs: `tail -f ~/.elevate/logs/gateway.log`
