...
Schedule tasks to run automatically with natural language or cron expressions. Elevate exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Elevate disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
elevate cron create "every 2h" "Check server status"
elevate cron create "every 1h" "Summarize new feed items" --skill blogwatcher
elevate cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Elevate normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Elevate will use the unified `cronjob` tool internally.
...
```bash
# Standalone CLI
elevate cron create --schedule "every 1d at 09:00" \
...
```bash
elevate cron edit <job_id> --schedule "every 4h"
elevate cron edit <job_id> --prompt "Use the revised task"
elevate cron edit <job_id> --skill blogwatcher --skill maps
elevate cron edit <job_id> --add-skill maps
elevate cron edit <job_id> --remove-skill blogwatcher
elevate cron edit <job_id> --clear-skills
```
...
```bash
elevate cron list
elevate cron pause <job_id>
elevate cron resume <job_id>
elevate cron run <job_id>
elevate cron remove <job_id>
elevate cron status
elevate cron tick
```
...
sudo elevate gateway install --system   # Linux: boot-time system service for servers
elevate gateway             # Or run in foreground
...
On each tick Elevate:

1. loads jobs from `~/.elevate/cron/jobs.json`
...
A file lock at `~/.elevate/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.elevate/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.elevate/cron/output/`), but no message is sent to the delivery target.
...
Or set the `ELEVATE_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Elevate skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.elevate/cron/jobs.json`. Output from job runs is saved to `~/.elevate/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Elevate resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
