...
description: "Overview of Elevate's tools — what's available, how toolsets work, and terminal backends"
---
...
Elevate ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `elevate model` to enable it, or configure individual tools with `elevate tools`.
:::
...
elevate chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `elevate-cli`, `elevate-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
```
```bash
# Set credentials in ~/.elevate/.env
...
# Configure
elevate config set terminal.backend singularity
elevate config set terminal.singularity_image ~/python.sif
```
...
modal setup
elevate config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.elevate/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.elevate/.env`.
:::
