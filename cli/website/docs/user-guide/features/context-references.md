...
Type `@` followed by a reference to inject content directly into your message. Elevate expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Elevate env: `$ELEVATE_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ELEVATE_HOME/skills/.hub/`
