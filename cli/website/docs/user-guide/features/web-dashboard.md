...
The web dashboard is a browser-based UI for managing your Elevate installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
```bash
# Custom port
elevate dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
elevate dashboard --host 0.0.0.0

# Start without opening browser
elevate dashboard --no-open
```

## Prerequisites

The default `elevate` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'elevate[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install elevate[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `elevate dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.
...
- Node.js (same requirement as `elevate --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'elevate[web,pty]'`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `elevate config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.elevate/skills/` and grouped by category.
...
This re-reads `~/.elevate/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `elevate_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `elevate update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `elevate dashboard` will build it on first launch.
...
|-------|---------|------------|--------|
| **Elevate Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
...
Themes that reference Google Fonts (everything except Elevate Teal) load the stylesheet on demand — the first time you switch to them, a `<link>` tag is injected into `<head>`.

### Custom themes

Drop a YAML file in `~/.elevate/dashboard-themes/` and it appears in the picker automatically. The file can be as minimal as a name plus the fields you want to override — every missing field inherits a sane default.

Minimal example (colors only, bare hex shorthand):

```yaml
# ~/.elevate/dashboard-themes/neon.yaml
name: neon
label: Neon
...
```yaml
# ~/.elevate/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
The CSS is injected as a single scoped `<style data-elevate-theme-css>` tag on theme apply and cleaned up on theme switch. Capped at 32 KiB per theme.

## Dashboard plugins

Plugins live in `~/.elevate/plugins/<name>/dashboard/` (user) or repo `plugins/<name>/dashboard/` (bundled). Each ships a `manifest.json` plus a plain JS bundle that uses the plugin SDK exposed on `window.__ELEVATE_PLUGIN_SDK__`.
...
Plugins inject components into named shell locations by calling `window.__ELEVATE_PLUGINS__.registerSlot(pluginName, slotName, Component)`. Multiple plugins can populate the same slot — they render stacked in registration order.

| Slot | Location |
|------|----------|
| `backdrop` | Inside the backdrop layer stack |
| `header-left` | Before the Elevate brand in the top bar |
...
Exposed on `window.__ELEVATE_PLUGIN_SDK__`:
