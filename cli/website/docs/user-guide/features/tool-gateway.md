...
When you run `elevate model` and select Nous Portal as your provider, Elevate automatically offers to enable the Tool Gateway:
...
Set the `use_gateway` flag directly in `~/.elevate/config.yaml`:
...
The gateway authenticates using your Nous Portal credentials (stored in `~/.elevate/auth.json` after `elevate model`).
...
```bash
elevate tools    # Select the tool → choose a direct provider
```
...
When you select a non-gateway provider in `elevate tools`, the `use_gateway` flag is automatically set to `false` to prevent contradictory config.
...
```bash
TOOL_GATEWAY_DOMAIN=elevationrealestatehq.com     # Base domain for gateway routing
...
Tools that were routed through the gateway will stop working until you [renew your subscription](https://portal.nousresearch.com/manage-subscription) or switch to direct API keys via `elevate tools`.
...
Modal (serverless terminal backend) is available as an optional add-on through the Nous subscription. It's not enabled by the Tool Gateway prompt — configure it separately via `elevate setup terminal` or in `config.yaml`.
