...
- Give it a title like "elevate-&lt;machine-name>"
...
elif [ -f ~/.elevate/.env ] && grep -q "^GITHUB_TOKEN=" ~/.elevate/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.elevate/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
