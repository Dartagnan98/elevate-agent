...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.elevate/.env ] && grep -q "^GITHUB_TOKEN=" ~/.elevate/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.elevate/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${ELEVATE_HOME:-$HOME/.elevate}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Elevate. Code looks clean — good test coverage, no security concerns."
