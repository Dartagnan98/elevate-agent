...
| | |
|---|---|
| Source | Optional — install with `elevate skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Elevate or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Elevate. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.elevate/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.elevate/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Elevate integration, either:

- configure the server in `~/.elevate/config.yaml` using the `native-mcp` skill, or
...
### Elevate cannot see the deployed server

The server-building part may be correct while the Elevate config is not. Load the `native-mcp` skill and configure the server in `~/.elevate/config.yaml`, then restart Elevate.
