...
| | |
|---|---|
| Source | Optional — install with `elevate skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Elevate |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Elevate terminal calls (desktop app flow only)
...
### Service Account (recommended for Elevate)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.elevate/.env` (the skill will prompt for this on first load).
...
## Elevate Execution Pattern (desktop app flow)

Elevate terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/elevate-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/elevate-op.sock"
