...
| | |
|---|---|
| Source | Optional — install with `elevate skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Elevate"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.elevate/.env`:
...
```bash
CANVAS="python $ELEVATE_HOME/skills/productivity/canvas/scripts/canvas_api.py"
