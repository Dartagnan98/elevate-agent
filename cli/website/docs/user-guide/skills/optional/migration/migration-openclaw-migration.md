---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Elevate"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Elevate"
---
...
Migrate a user's OpenClaw customization footprint into Elevate. Imports Elevate-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
# OpenClaw -> Elevate Migration

Use this skill when a user wants to move their OpenClaw setup into Elevate with minimal manual cleanup.
...
```bash
elevate claw migrate              # Full interactive migration
elevate claw migrate --dry-run    # Preview what would be migrated
elevate claw migrate --preset user-data   # Migrate without secrets
elevate claw migrate --overwrite  # Overwrite existing conflicts
elevate claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `elevate setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_elevate.py` to:

- import `SOUL.md` into the Elevate home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Elevate memory entries
- merge OpenClaw command approval patterns into Elevate `command_allowlist`
- migrate Elevate-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.elevate/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Elevate workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.elevate/tts/`
- archive non-secret docs that do not have a direct Elevate destination
...
- `scripts/openclaw_to_elevate.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.elevate/skills/migration/openclaw-migration/scripts/openclaw_to_elevate.py`

Do not guess a shorter path like `~/.elevate/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.elevate/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Elevate-compatible secrets, currently:
...
Elevate CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Elevate. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Elevate workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Elevate skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.elevate/skills/migration/openclaw-migration/scripts/openclaw_to_elevate.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.elevate/skills/migration/openclaw-migration/scripts/openclaw_to_elevate.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.elevate/skills/migration/openclaw-migration/scripts/openclaw_to_elevate.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.elevate/skills/migration/openclaw-migration/scripts/openclaw_to_elevate.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.elevate/skills/migration/openclaw-migration/scripts/openclaw_to_elevate.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.elevate/skills/migration/openclaw-migration/scripts/openclaw_to_elevate.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Elevate unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Elevate targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Elevate destination. Unsupported auth blobs must still be reported as skipped.
...
- Elevate persona state imported
- Elevate memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.elevate/skills/openclaw-imports/`
