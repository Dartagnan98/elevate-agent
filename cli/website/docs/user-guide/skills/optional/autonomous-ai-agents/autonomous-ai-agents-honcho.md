...
description: "Configure and use Honcho memory with Elevate -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Elevate -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `elevate skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Elevate

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Elevate profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/elevate#running-honcho-locally-with-elevate

### Verify

```bash
elevate honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Elevate profile's AI peer
...
Honcho models conversations as interactions between **peers**. Elevate creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Elevate instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Elevate run |
| `global` | Single session across all directories |

Manual override: `elevate honcho map my-project-name`
...
Each Elevate profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block elevate.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `elevate.coder` host block in `honcho.json`
...
```bash
elevate honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Elevate when Honcho memory is active.
...
Config file: `$ELEVATE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `elevate honcho setup`. Ensure `memory.provider: honcho` is in `~/.elevate/config.yaml`.

### Memory not persisting across sessions
Check `elevate honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `elevate profile create <name> --clone`. For existing profiles: `elevate honcho sync`.
...
| Command | Description |
|---------|-------------|
| `elevate honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `elevate honcho status` | Show resolved config, connection test, peer info for active profile |
| `elevate honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `elevate honcho disable` | Disable Honcho for the active profile |
| `elevate honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `elevate honcho peers` | Show peer identities across all profiles |
| `elevate honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `elevate honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `elevate honcho sessions` | List known directory-to-session-name mappings |
| `elevate honcho map <name>` | Map current working directory to a Honcho session name |
| `elevate honcho identity` | Seed AI peer identity or show both peer representations |
| `elevate honcho sync` | Create host blocks for all Elevate profiles that don't have one yet |
| `elevate honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Elevate + Honcho |
| `elevate memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `elevate memory status` | Show active memory provider and config |
| `elevate memory off` | Disable external memory provider |
