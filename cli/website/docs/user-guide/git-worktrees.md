...
title: "Git Worktrees"
description: "Run multiple Elevate agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Elevate is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Elevate so each session has a clean, isolated working directory.

## Why Use Worktrees with Elevate?

Elevate treats the **current working directory** as the project root:

- CLI: the directory where you run `elevate` or `elevate chat`
...
git worktree add ../repo-feature feature/elevate-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/elevate-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Elevate there:

```bash
cd ../repo-feature

# Start Elevate in the worktree
elevate
```
...
git worktree add ../repo-experiment-a feature/elevate-a
git worktree add ../repo-experiment-b feature/elevate-b
```
...
Each Elevate process:

- Works on its own branch (`feature/elevate-a` vs `feature/elevate-b`).
...
- Elevate checkpoint data under `~/.elevate/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Elevate experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/elevate-checkpoints-docs`, `feature/elevate-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Elevate from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `elevate -w` (Automatic Worktree Mode)

Elevate has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `elevate/elevate-<hash>`).
...
```bash
elevate -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `elevate -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Elevate session its own clean checkout.
