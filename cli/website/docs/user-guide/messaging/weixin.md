...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect Elevate to personal WeChat accounts via the iLink Bot API"
---

# Weixin (WeChat)

Connect Elevate to [WeChat](https://weixin.qq.com/) (微信), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts — this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.
...
- Terminal QR rendering is included when Elevate is installed with the `messaging` extra
...
5. Save the account credentials automatically to `~/.elevate/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.elevate/.env`:
...
- **QR code login** — scan-to-connect setup via `elevate gateway setup`
...
- Tokens are saved per account+peer to `~/.elevate/weixin/accounts/<account_id>.context-tokens.json`
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `elevate gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `elevate gateway setup` |
| `Another local Elevate gateway is already using this Weixin token` | Stop the other gateway instance first — only one poller per token is allowed |
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `elevate gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `pip install elevate[messaging]`. Alternatively, open the URL printed above the QR |
