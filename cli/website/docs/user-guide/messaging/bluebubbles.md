...
Connect Elevate to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Elevate and the BlueBubbles server
...
Or set environment variables directly in `~/.elevate/.env`:
...
When someone messages your iMessage, Elevate automatically sends them a pairing code. Approve it with:
```bash
elevate pairing approve bluebubbles <CODE>
```
Use `elevate pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.elevate/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.elevate/.env`):
```bash
...
Elevate will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Elevate
Elevate → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Elevate sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Elevate resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `elevate logs gateway` for webhook errors (or `elevate logs -f` to follow in real-time)
