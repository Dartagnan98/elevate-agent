...
sidebar_position: 4
title: "Slack"
description: "Set up Elevate as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Elevate to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Elevate instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Elevate uses the modern
...
4. Enter an app name (e.g., "Elevate") and select your workspace
5. Click **Create App**
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `elevate-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Elevate configuration issue.
:::
...
Elevate uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
elevate gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
elevate gateway              # Foreground
elevate gateway install      # Install as a user service
sudo elevate gateway install --system   # Linux only: boot-time system service
```
...
Understanding how Elevate behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Elevate what time is it?`). In channels, Elevate replies in a thread attached to that message. |
| **Threads** | If you @mention Elevate inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.elevate/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Elevate in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Elevate will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Elevate`).

---

## Multi-Workspace Support

Elevate can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Elevate also loads tokens from an **OAuth token file** at:

```
~/.elevate/slack_tokens.json
```
...
- When a message arrives, Elevate uses the correct workspace-specific client to respond.
...
Elevate supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Elevate` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Elevate` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Elevate`)
...
- Tokens should be stored in `~/.elevate/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Elevate config directory
