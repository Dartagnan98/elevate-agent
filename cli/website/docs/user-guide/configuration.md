...
sidebar_position: 2
title: "Configuration"
description: "Configure Elevate — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.elevate/` directory for easy access.
...
```bash
elevate config              # View current configuration
elevate config edit         # Open config.yaml in your editor
elevate config set KEY VAL  # Set a specific value
elevate config check        # Check for missing options (after updates)
elevate config migrate      # Interactively add missing options

# Examples:
elevate config set model anthropic/claude-opus-4
elevate config set terminal.backend docker
elevate config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `elevate config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `elevate chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.elevate/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.elevate/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `ELEVATE_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `ELEVATE_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the legacy defaults (`ELEVATE_API_TIMEOUT=1800`s, `ELEVATE_API_CALL_STALE_TIMEOUT=300`s, native Anthropic 900s). Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/Dartagnan98/elevate-agent/blob/main/cli-config.yaml.example).

## Terminal Backend Configuration

Elevate supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Elevate will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
:::warning
The agent has the same filesystem access as your user account. Use `elevate tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Elevate probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.elevate/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.elevate/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.elevate/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `elevate-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/elevate` (HPC convention) → `~/.elevate/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `elevate config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Elevate logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `elevate doctor` to check.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.elevate/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.elevate/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.elevate/cache/documents/report.txt`
...
Elevate resolves each listed variable from your current shell first, then falls back to `~/.elevate/.env` if it was saved with `elevate config set`.
...
Docker sandboxes stay isolated by default. Elevate does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Elevate from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Elevate from
...
```bash
elevate config set terminal.persistent_shell false
```
...
- `elevate config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `elevate config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
elevate config set skills.config.myplugin.path ~/myplugin-data
```
...
Three related caps control how much raw output a tool can return before Elevate truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, Elevate keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
```yaml
worktree: true    # Always create a worktree (same as elevate -w)
...
Elevate automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `elevate plugins` → Provider Plugins → Context Engine.
...
When the agent is working on a complex task with many tool calls, it can burn through its iteration budget (default: 150 turns) without realizing it's running low. Budget pressure automatically warns the model as it approaches the limit:
...
```yaml
agent:
  max_turns: 150               # Max iterations per conversation turn (default: 150)
```
...
Elevate has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `ELEVATE_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `ELEVATE_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `ELEVATE_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `ELEVATE_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Elevate raises this to 30 minutes when it detects a local endpoint. If you explicitly set `ELEVATE_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Elevate disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `ELEVATE_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
Elevate uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.

### The universal config pattern

Every model slot in Elevate — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Elevate ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Elevate uses that provider's built-in auth and base URL.
...
If you use a reasoning-heavy model for `auxiliary.session_search`, Elevate now gives you two built-in controls:

- `auxiliary.session_search.max_concurrency`: limits how many matched sessions Elevate summarizes at once
...
Use `extra_body` only when your provider documents OpenAI-compatible request-body fields you want Elevate to pass through for that task. Elevate forwards the object as-is.

:::warning
`extra_body` is only effective when your provider actually supports the field you send. If the provider does not expose a native OpenAI-compatible reasoning-off flag, Elevate cannot synthesize one on its behalf.
:::
...
Or via environment variable (in `~/.elevate/.env`):
...
| `"nous"` | Force Nous Portal | `elevate auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `elevate model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `elevate model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Elevate uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider Elevate uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `elevate config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
`interim_assistant_messages` is gateway-only. When enabled, Elevate sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Elevate falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
Streaming is disabled by default. Enable it in `~/.elevate/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Elevate to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Elevate still keys DMs by chat/DM ID as usual.
...
Control what Elevate does when an unknown user sends a direct message:
...
- `pair` is the default. Elevate denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.elevate/elevate && git pull && pip install -e .
  gpu:
    type: exec
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Elevate's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search`, `web_extract`, and `web_crawl` tools support four backend providers. Configure the backend in `config.yaml` or via `elevate tools`:
...
**Exa:** Set `EXA_API_KEY` in `~/.elevate/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.elevate/browser_recordings/
  max_actions_per_session: 0     # Unlimited; set a positive per-task command cap to constrain it
  stuck_threshold: 0             # No stuck/blocker stop notices; set a positive threshold to enable
  # Optional CDP override — when set, Elevate attaches directly to your own
...
- `must_respond` — capture the dialog, surface it in `browser_snapshot.pending_dialogs`, and wait for the agent to call `browser_dialog(action=...)`. After `dialog_timeout_s` seconds with no response, the dialog is auto-dismissed to prevent the page's JS thread from stalling forever.
...
- `auto_accept` (default) — capture, accept immediately. Useful for pages with aggressive `beforeunload` prompts.
...
Control how Elevate handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `ELEVATE_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: true                  # Enable automatic checkpoints (also: elevate --checkpoints)
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Elevate falls back to `OPENAI_API_KEY` only.
...
Elevate uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.elevate/SOUL.md` or `$ELEVATE_HOME/SOUL.md` |
| `.elevate.md` / `ELEVATE.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Elevate falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.elevate.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Elevate automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`elevate`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.elevate/.env or ~/.elevate/config.yaml:
