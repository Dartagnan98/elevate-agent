...
This page covers setting up inference providers for Elevate — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Elevate.

## Inference Providers

You need at least one way to connect to an LLM. Use `elevate model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `elevate model` (OAuth, subscription-based) |
| **OpenAI Codex** | `elevate model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `elevate model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `elevate model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `elevate model` (Claude Pro/Max via Claude Code auth, Anthropic API key, or manual setup-token) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.elevate/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.elevate/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.elevate/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.elevate/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.elevate/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.elevate/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.elevate/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.elevate/.env` (provider: `minimax-cn`) |
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `~/.elevate/.env` (provider: `alibaba`, aliases: `dashscope`, `qwen`) |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.elevate/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.elevate/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.elevate/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.elevate/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.elevate/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.elevate/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.elevate/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `elevate model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **Custom Endpoint** | `elevate model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
# → done — Elevate auto-provisions your free tier on first request
```

Elevate ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.elevate/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `ELEVATE_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Elevate works unchanged
...
| Workspace / Standard / Enterprise account | Set `ELEVATE_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Elevate detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Elevate shows
...
```bash
ELEVATE_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
ELEVATE_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Elevate stores the resulting credentials in its own auth store under `~/.elevate/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::
...
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/docs/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. It's offered automatically during `elevate model` setup, or enable it later with `elevate tools`.
:::

### Two Commands for Model Management

Elevate has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`elevate model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Elevate chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `elevate model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `elevate model`, complete the provider setup, then start a new session.
...
elevate chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `elevate model`
# Elevate will use Claude Code's credential store directly when available
elevate model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
elevate chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
elevate chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `elevate model`, Elevate prefers Claude Code's own credential store over copying the token into `~/.elevate/.env`. That keeps refreshable Claude credentials refreshable.
...
Elevate supports GitHub Copilot as a first-class provider with two modes:
...
```bash
elevate chat --provider copilot --model gpt-5.4
```
...
If no token is found, `elevate model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `elevate model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `elevate model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Elevate
Elevate sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Elevate now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Elevate therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
elevate chat --provider copilot-acp --model copilot-acp
...
| `ELEVATE_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `ELEVATE_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# z.ai / ZhipuAI GLM
elevate chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.elevate/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
elevate chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.elevate/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
elevate chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.elevate/.env

# MiniMax (global endpoint)
elevate chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.elevate/.env

# MiniMax (China endpoint)
elevate chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.elevate/.env

# Alibaba Cloud / DashScope (Qwen models)
elevate chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.elevate/.env

# Xiaomi MiMo
elevate chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.elevate/.env

# Arcee AI (Trinity models)
elevate chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.elevate/.env
```
...
When using the Z.AI / GLM provider, Elevate automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.elevate/.env` and pick xAI in `elevate model`, or drop `grok` as a shortcut into `/model grok-4-1-fast-reasoning`.

When using xAI as a provider (any base URL containing `x.ai`), Elevate automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `elevate tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

### Ollama Cloud — Managed Ollama Models, OAuth + API Key

[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `elevate model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Elevate auto-discovers the available models.
...
elevate chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 elevate chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `elevate model`, sign in through the browser, and Elevate persists the refresh token.

```bash
elevate model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.elevate/auth.json

elevate chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `ELEVATE_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
elevate chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.elevate/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 elevate chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
elevate chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.elevate/.env

# Short alias
elevate chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Elevate works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Elevate at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Elevate — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `elevate setup` or config migration. Use `elevate model` or edit `config.yaml` directly.
:::
...
:::warning elevate model vs /model
**`elevate model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Elevate chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `elevate model`, set up the new provider, then start a new session.
:::
...
When switching providers, Elevate persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Elevate.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Elevate) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `elevate` (Qwen 2.5, Elevate 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Elevate to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Elevate won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Elevate 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Elevate) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.
...
Use that IP in your Elevate config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Elevate config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Elevate.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Elevate inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser elevate` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Elevate's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Elevate thinks the context is
...
Elevate auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Elevate uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Elevate with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in Elevate.
...
Then configure Elevate with `elevate model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Elevate with `elevate model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `elevate model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Elevate uses this to decide when to compress history and to validate API requests.
...
Elevate uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`elevate model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Elevate uses "no-key-required" for keyless local servers
  - name: work
...
You can also select named custom providers from the interactive `elevate model` menu.
...
:::tip
You can switch between providers at any time with `elevate model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Elevate at your instance (no API key needed):
   ```bash
   elevate config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.elevate/config.yaml`:
...
Configure a backup provider:model that Elevate switches to automatically when your primary model fails (rate limits, server errors, auth failures):
