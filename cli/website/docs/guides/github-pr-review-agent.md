...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Elevate  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Elevate instantly when PRs are opened or updated.
:::
...
- **Elevate installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  elevate gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.elevate/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Elevate can access GitHub. Start a chat:
...
```
Run: gh pr list --repo Dartagnan98/elevate-agent --state open --limit 3
```
...
Still in the chat, ask Elevate to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo Dartagnan98/elevate-agent
```
...
A skill gives Elevate consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.elevate/skills/code-review
```

Create `~/.elevate/skills/code-review/SKILL.md`:
...
Verify it loaded — start `elevate` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Elevate your team's standards:
...
2. Teach Elevate your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
elevate gateway status    # Is the gateway running?
elevate cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-elevate-plugin)** — wrap the review logic into a shareable plugin
