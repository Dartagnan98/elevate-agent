...
elevate cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/elevate GitHub repo.

1. Run: gh issue list --repo NousResearch/elevate --state open --json number,title,labels,author,createdAt --limit 30
...
elevate cron create "0 9 * * 1" \
  "Scan the NousResearch/elevate repo for documentation drift.

1. Run: gh pr list --repo NousResearch/elevate --state merged --json number,title,files,mergedAt --limit 30
