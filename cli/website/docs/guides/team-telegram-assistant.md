...
This tutorial walks you through setting up a Telegram bot powered by Elevate that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Elevate installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.elevate/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Elevate itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Elevate Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_elevate_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Elevate. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.elevate/.env`:
...
```
[Gateway] Starting Elevate Gateway...
...
elevate gateway stop
elevate gateway status

# View live logs
journalctl --user -u elevate-gateway -f
...
elevate gateway start
elevate gateway stop
tail -f ~/.elevate/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `elevate gateway install` to update the plist.
:::
...
```bash
elevate gateway stop && elevate gateway start
```
...
elevate pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.elevate/.env`:
...
Control how much detail the bot shows when using tools. In `~/.elevate/config.yaml`:
...
Customize how the bot communicates by editing `~/.elevate/SOUL.md`:

For a full guide, see [Use SOUL.md with Elevate](/docs/guides/use-soul-with-elevate).
...
```bash
# From the CLI
elevate cron list          # View all scheduled jobs
elevate cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u elevate-gateway` (Linux) or `~/.elevate/logs/gateway.log` (macOS) |
| Cron job output | `~/.elevate/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.elevate/cron/jobs.json` |
| Pairing data | `~/.elevate/pairing/` |
| Session history | `~/.elevate/sessions/` |
