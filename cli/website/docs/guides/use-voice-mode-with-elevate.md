---
sidebar_position: 8
title: "Use Voice Mode with Elevate"
description: "A practical guide to setting up and using Elevate voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Elevate
...
- you want Elevate sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Elevate.
...
## Step 1: make sure normal Elevate works first
...
Elevate supports both local and cloud speech stacks.
...
### If you use `elevate setup`

If you choose NeuTTS in the setup wizard, Elevate checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Elevate transcribes and responds
...
- asking Elevate to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Elevate loop.

## Tuning CLI behavior

### Silence threshold

If Elevate starts/stops too aggressively, tune:
...
Elevate stays a normal chat bot, but can speak replies.
...
- you want Elevate to function like a portable research or ops assistant
...
Elevate joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Elevate detects speech boundaries
- transcripts are posted in the associated text channel
- Elevate responds in text and audio
...
1. get text Elevate working
2. install `elevate[voice]`
