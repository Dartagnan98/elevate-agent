...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Dartagnan98/elevate-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    elevate_meta = (fm.get("metadata") or {}).get("elevate") or {}
    tags = elevate_meta.get("tags") or []
    related = elevate_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `elevate skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Elevate"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Elevate ships with a large built-in skill library copied into `~/.elevate/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
