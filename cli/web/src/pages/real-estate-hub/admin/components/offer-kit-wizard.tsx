  ...
  const buildKit = useCallback(async () => {
    ...
    try {
      ...
      if (!buildRes || !buildRes.ok) {
        ...
        setBuiltMsg(`✗ Build failed — ${String(detail).slice(0, 160) || "could not seed the kit"}`);
        return;
      }
      ...
      if (failed.length) {
        setBuiltMsg(`⚠ Built ${ok} of ${enabled.length} — ${failed.length} failed to generate (${failed.join(", ")}). Check the fields and rebuild.`);
      } else if (warns.length) {
        setBuiltMsg(`✓ Built ${ok} documents — ${warns.length} blank to complete: ${warns.join("; ")}`);
      } else {
        setBuiltMsg(`✓ Built ${ok} documents into the kit`);
      }
      onUpdate?.();
