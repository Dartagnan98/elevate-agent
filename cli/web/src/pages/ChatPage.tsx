...
export default function ChatPage() {
  ...
  const backgroundTasks = useMemo<BackgroundTaskItem[]>(() => {
    ...
    // Real per-session output totals from the durable child rows — used to
    // upgrade a finished live entry's thinking-token estimate to the truth.
...
function SubagentSummaryRow({
  ...
  return (
    <div className="flex items-center gap-2 rounded-[6px] px-1 py-0.5 text-[12.5px] leading-5 text-[var(--chat-muted-strong)]">
      ...
      {subagent.thinkingTokens ? (
        <span
          className="shrink-0 text-[10.5px] tabular-nums text-[var(--fg-faint)]"
          title="Output tokens generated by this subagent."
        >
          {running
            ? `↓ ${subagent.thinkingTokens.toLocaleString()} out`
            : `${subagent.thinkingTokens.toLocaleString()} out`}
        </span>
      ) : null}
      {canOpen ? (
