...
/**
 * Rising sweep — used while onboarding is loading / seeding.
 *
 * Pitch sweeps from ~80 Hz up to ~1600 Hz over `durationSeconds`, with a
 * filtered noise layer riding underneath for build-up texture. Returns a
 * `stop()` handle so the caller can cut the riser short (fading it out)
 * when loading actually completes — avoids the riser overshooting into
 * silence.
 */
