  ...
  return (
    <>
      <button
        ...
        // Moved off the top-right rail (2026-07-27). It was fixed at
        // `right-3 top-2`, which is exactly where every page puts its own
        // toolbar — on /leads it rendered on top of "+ New lead" and on
        // /reporting it covered the range picker. Bottom-right is unoccupied on
        // every page, so it no longer collides with anything.
        className={cn(
          "fixed bottom-3 right-3 z-[45] hidden items-center gap-2 rounded-full sm:flex",
      ...
      <button
        ...
        title="Report a bug"
        aria-label="Report a bug"
        className="fixed bottom-3 right-3 z-[45] flex h-9 w-9 items-center justify-center rounded-full border border-[var(--ds-border)] bg-[var(--ds-card)] text-[var(--ds-muted)] backdrop-blur-sm shadow-sm sm:hidden"
      >
        {capturing ? (
