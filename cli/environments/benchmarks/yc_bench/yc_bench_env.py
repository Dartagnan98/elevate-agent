...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs ElevateAgentLoop with terminal tool only
...
Requires: pip install elevate[yc-bench]
"""
...
class YCBenchEvalConfig(ElevateAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends ElevateAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(ElevateAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The ElevateAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs ElevateAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our ElevateAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the ElevateAgentLoop
            # ----------------------------------------------------------
