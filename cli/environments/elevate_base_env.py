"""
ElevateAgentBaseEnv -- Abstract Base Environment for Elevate + Atropos

Provides the Atropos integration plumbing that all elevate environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via ElevateAgentLoop
...
# Ensure the elevate repo root is on sys.path so that imports like
...
from environments.agent_loop import AgentResult, ElevateAgentLoop
...
# Import elevate toolset infrastructure
...
class ElevateAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for elevate Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of elevate toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: elevate, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/elevate-results/ "
...
class ElevateAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for elevate Atropos integration.
    ...
    name: Optional[str] = "elevate"
    env_config_cls = ElevateAgentEnvConfig

    def __init__(
        self,
        config: ElevateAgentEnvConfig,
        ...
        # Set terminal environment variables so elevate tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY elevate tool (terminal, file, web,
