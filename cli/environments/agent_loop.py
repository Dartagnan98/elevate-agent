"""
ElevateAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the elevate tool-calling loop using standard OpenAI-spec tool calling.
...
identical to elevate's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by ElevateAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by ElevateAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class ElevateAgentLoop:
    """
    Runs elevate's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # elevate's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("elevate")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via elevate's dispatch
