# nix/nixosModules.nix — NixOS module for elevate
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: elevate runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ELEVATE_HOME/.env
# and read by elevate at startup — no container recreation needed for env changes.
#
# Tool resolution: the elevate wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    generatedConfigFile = pkgs.writeText "elevate-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "elevate-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ELEVATE_DOC_EOF'\n${value}\nELEVATE_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "elevate";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/elevate";
    ...
    # elevate user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "elevate-container-entrypoint" ''
      set -eu

      ELEVATE_UID="''${ELEVATE_UID:?ELEVATE_UID must be set}"
      ELEVATE_GID="''${ELEVATE_GID:?ELEVATE_GID must be set}"

      # ── Group: ensure a group with GID=$ELEVATE_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ELEVATE_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="elevate"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ELEVATE_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ELEVATE_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ELEVATE_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ELEVATE_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="elevate"
        TARGET_HOME="/home/elevate"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ELEVATE_UID" -g "$ELEVATE_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ELEVATE_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ELEVATE_UID:$ELEVATE_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ELEVATE_HOME is owned by the target user
      if [ -n "''${ELEVATE_HOME:-}" ] && [ -d "$ELEVATE_HOME" ]; then
        chown -R "$ELEVATE_UID:$ELEVATE_GID" "$ELEVATE_HOME"
      fi
      ...
      if [ ! -f /var/lib/elevate-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/elevate-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/elevate ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/elevate
        chmod 0440 /etc/sudoers.d/elevate
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ELEVATE_UID" --regid="$ELEVATE_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ELEVATE_HOME/.env.
    ...
    # Default: /var/lib/elevate/workspace → /data/workspace.
  ...
  in {
    options.services.elevate = with lib; {
      enable = mkEnableOption "Elevate gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = elevate;
        description = "The elevate package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/elevate";
        description = "State directory. Contains .elevate/ subdir (ELEVATE_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Elevate config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ELEVATE_HOME/.env at activation time.
          Elevate reads this file on every startup via load_elevate_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ELEVATE_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ELEVATE_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `elevate gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the elevate CLI to environment.systemPackages and export
          ELEVATE_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.elevate symlink to the service
            stateDir. These users are automatically added to the elevate group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.elevate.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the elevate CLI to system PATH and export ELEVATE_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.elevate/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.ELEVATE_HOME = "${cfg.stateDir}/.elevate";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.elevate: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed elevate
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure elevate is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.elevate        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elevate/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elevate/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elevate/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.elevate/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."elevate-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.elevate
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.elevate ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.elevate ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.elevate -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.elevate/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.elevate/$_subdir"
            chmod 2770 "${cfg.stateDir}/.elevate/$_subdir"
            find "${cfg.stateDir}/.elevate/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.elevate/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.elevate/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.elevate/config.yaml
            chmod 0640 ${cfg.stateDir}/.elevate/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.elevate/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.elevate/.managed
          chmod 0644 ${cfg.stateDir}/.elevate/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.elevate/.container-mode <<'ELEVATE_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
elevate_bin=${containerDataDir}/current-package/bin/elevate
ELEVATE_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.elevate/.container-mode
            chmod 0644 ${cfg.stateDir}/.elevate/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.elevate/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.elevate";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.elevate" ]; then
                  rm -f "${symlinkPath}"
                  echo "elevate: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.elevate -> stateDir/.elevate for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.elevate";
                target = "${cfg.stateDir}/.elevate";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "elevate: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.elevate/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.elevate/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.elevate/auth.json
              fi
            ''}
          ''}
          ...
          # Elevate reads $ELEVATE_HOME/.env at startup via load_elevate_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.elevate/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ELEVATE_NIX_ENV_EOF'
${envFileContent}
ELEVATE_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.elevate = {
          ...
          environment = {
            HOME = cfg.stateDir;
            ELEVATE_HOME = "${cfg.stateDir}/.elevate";
            ELEVATE_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $ELEVATE_HOME/.env by the activation script. load_elevate_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/elevate"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the elevate group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.elevate = {
          description = "Elevate Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ELEVATE_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ELEVATE_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ELEVATE_UID="$ELEVATE_UID" \
                --env ELEVATE_GID="$ELEVATE_GID" \
                --env ELEVATE_HOME=${containerDataDir}/.elevate \
                --env ELEVATE_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/elevate gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
