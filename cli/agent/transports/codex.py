...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # Elevate-only: also clamp ``xhigh`` -> ``high``. The Codex Responses
        # route accepts low / medium / high; Elevate's reasoning-effort knob
        # exposes a wider range (``minimal`` and ``xhigh``) so a user setting
        # ``xhigh`` doesn't 400 the request. Hermes upstream only maps minimal.
