...
from elevate_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.elevate/SOUL.md as the primary
    ...
    if agent.verbose_logging:
        ...
        # on the fact that elevate_logging.setup_logging() does not
        ...
        # handler level inside elevate_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from elevate_cli.auth import build_minimax_oauth_token_provider
    ...
    # worker (kanban_show tool is present iff ELEVATE_KANBAN_TASK is set).
    ...
    try:
        from elevate_cli.config import load_config as _load_sess_cfg
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from elevate_cli.config import get_custom_provider_context_length
