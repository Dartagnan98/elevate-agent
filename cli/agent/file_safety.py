...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Elevate path.

    Two categories are blocked:

      * Internal Elevate cache files under ``ELEVATE_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ELEVATE_HOME and the global Elevate
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.elevate/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ELEVATE_HOME (profile-aware) AND the global
    # Elevate root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ELEVATE_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    elevate_dirs: list[Path] = []
    for base in (_elevate_home_path(), _elevate_root_path()):
        try:
            real = base.resolve()
            if real not in elevate_dirs:
    ...
    for hd in elevate_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Elevate credential store "
    ...
    for hd in elevate_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Elevate MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Elevate MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
