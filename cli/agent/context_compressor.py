...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the elevate compressor emits).
    """
    if not messages:
        return messages
...
class ContextCompressor(ContextEngine):
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates schema-heavy requests so we compress before a provider
        rejects the payload. After a successful compressed API call, though,
        ...
        fit under the threshold. This is the fix for the post-compaction
        re-compaction loop (issue: compaction fires but the inflated rough
        estimate stays over threshold, so it re-fires forever).
        """
