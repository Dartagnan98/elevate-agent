...
as ``kind: backend``) or ``~/.elevate/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$ELEVATE_HOME/cache/images/``, creating parents as needed."""
    from elevate_constants import get_elevate_home

    path = get_elevate_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ELEVATE_HOME/cache/images/``.
