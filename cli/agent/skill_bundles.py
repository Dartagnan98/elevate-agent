...
Storage
-------
Bundles live in ``~/.elevate/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ELEVATE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``elevate bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ELEVATE_HOME.

    Honors ``ELEVATE_BUNDLES_DIR`` for tests; falls back to
    ``<ELEVATE_HOME>/skill-bundles``.
    """
    override = os.environ.get("ELEVATE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_elevate_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `elevate bundles` CLI subcommand.
# ---------------------------------------------------------------------------
