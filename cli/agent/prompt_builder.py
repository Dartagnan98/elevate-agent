...
_ELEVATE_MD_NAMES = (".elevate.md", "ELEVATE.md")


def _find_elevate_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.elevate.md`` or ``ELEVATE.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Elevate, an intelligent AI assistant created by Nous Research. "
...
ELEVATE_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Elevate Agent "
    "itself, load the `elevate-agent` skill with skill_view(name='elevate-agent') "
    "before answering. Docs: https://elevate-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.elevate/kanban.db`. Your task id is in "
    "`$ELEVATE_KANBAN_TASK`; your workspace is `$ELEVATE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ELEVATE_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `elevate kanban <verb>` for board operations. Use "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_elevate_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.elevate/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (selective)\n"
            "Before doing specialized work, scan the skills below and match the user's "
            "request against each skill's description/triggers. When a request matches a "
            "skill — ESPECIALLY any multi-step ACTION that drives an external system "
            "(MLS/Matrix, SkySlope, Lofty, a CRM, a browser, documents, listings) or a "
            "named workflow — load that skill with skill_view(name) BEFORE you act. Do not "
            "attempt a skilled workflow from general reasoning: the skill carries the exact "
            "steps, commands, click-paths, and pitfalls, and improvising without it stalls "
            "or fails. Also load whenever the user explicitly names a skill or asks you to "
            "run a skill/workflow.\n"
            "Do NOT load a skill for status checks, direct questions, simple follow-ups, "
            "meta/debug messages, or small tasks fully answerable from the currently loaded "
            f"context — answer those directly with {basic_tool_phrase} and skip the "
            "skill_view round-trip. That round-trip is only wasteful before a plain answer "
            "or verification; it is never wasteful before a real skilled action.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: elevate setup, elevate setup tools, elevate setup terminal, elevate status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ELEVATE_HOME and return its content, or None.
...
def _load_elevate_md(cwd_path: Path) -> str:
    """.elevate.md / ELEVATE.md — walk to git root."""
    elevate_md_path = _find_elevate_md(cwd_path)
    if not elevate_md_path:
        return ""
    try:
        content = elevate_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = elevate_md_path.name
        try:
            rel = str(elevate_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".elevate.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", elevate_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .elevate.md / ELEVATE.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ELEVATE_HOME is independent and always included when present.
    ...
    # SOUL.md from ELEVATE_HOME only — skip when already loaded as identity
    if not skip_soul:
