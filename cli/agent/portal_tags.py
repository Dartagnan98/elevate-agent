...
The version is sourced live from ``elevate_cli.__version__`` so it auto-aligns
...
``elevate_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``elevate_cli`` cannot be imported (should
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=hermes-agent", elevate_client_tag()]
