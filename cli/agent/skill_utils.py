...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ELEVATE_PLATFORM`` or
            ``ELEVATE_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.elevate/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(elevate, dict):
        elevate = {}
    return {
        "fallback_for_toolsets": elevate.get("fallback_for_toolsets", []),
        "requires_toolsets": elevate.get("requires_toolsets", []),
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    elevate = metadata.get("elevate")
    if not isinstance(elevate, dict):
        return []
    raw = elevate.get("config")
    if not raw:
        return []
