...
as ``kind: backend``) or ``~/.elevate/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``elevate tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``elevate tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``elevate tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``elevate tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ELEVATE_HOME/cache/videos/``, creating parents as needed."""
    from elevate_constants import get_elevate_home

    path = get_elevate_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ELEVATE_HOME/cache/videos/``.
