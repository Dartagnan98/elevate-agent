...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Elevate starts and finds ``~/.openclaw/``.

    Points users at ``elevate claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``elevate claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Elevate, run "
        "`elevate claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `elevate claw cleanup` (renames it to ~/.openclaw.pre-migration — "
