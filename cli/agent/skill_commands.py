...
# Tokens that don't resolve (e.g. ${ELEVATE_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(ELEVATE_SKILL_DIR|ELEVATE_SESSION_ID)\}")
...
def _substitute_template_vars(
    ...
    """Replace ${ELEVATE_SKILL_DIR} / ${ELEVATE_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "ELEVATE_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "ELEVATE_SESSION_ID" and session_id:
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.elevate.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_elevate_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.elevate/skills/ and return a mapping of /command -> skill info.
