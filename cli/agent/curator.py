...
def _state_file() -> Path:
    return get_elevate_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.elevate/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``elevate update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``elevate curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `elevate curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Elevate' background skill CURATOR. This is an "
    ...
    "into ~/.elevate/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: elevate-config-*, elevate-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.elevate/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.elevate/logs/curator/``)
    ...
    authored skill data in ``~/.elevate/skills/``.

    ``ensure_elevate_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_elevate_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every elevate install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: elevate curator status
        keep an umbrella stable: elevate curator pin document-tools
    ...
    lines.append("full report: elevate curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: elevate curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.elevate/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.elevate/skills/.archive/` for "
            "safety and can be restored via `elevate curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.elevate/skills/.archive/`. "
            "Restore any via `elevate curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `elevate curator restore <name>`")
    lines.append("- All archives live under `~/.elevate/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `elevate curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `elevate curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``elevate model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `elevate model` → auxiliary
