...
Plugins built on Elevate Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Elevate
...
:class:`~elevate_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Elevate' main config
...
handles every provider, fallback chain, and per-task override Elevate
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`elevate_cli.plugins.PluginContext`
