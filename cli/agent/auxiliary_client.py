...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.elevate/auth.json active provider)
...
NOUS_EXTRA_BODY = {"tags": ["product=elevate"]}
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Elevate)",
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=elevate"])
    if merged_extra:
