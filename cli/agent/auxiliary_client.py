...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.elevate/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Elevate triggers summarization.  Higher values delay
...
# Backwards-compat alias for the pre-rename name. ``run_agent.py`` and
# tests still import the bare ``_OR_HEADERS`` symbol when wiring static
# default-header dicts onto OpenAI clients; keep the alias so the import
# chain doesn't break while the dynamic ``build_or_headers()`` helper
# gets adopted call site by call site.
_OR_HEADERS = _OR_HEADERS_BASE
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ELEVATE_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ELEVATE_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ELEVATE_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ELEVATE_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "ElevateAgent",
}
...
# in lockstep with elevate_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``elevate_cli.__version__`` is
...
_AUTH_JSON_PATH = get_elevate_home() / "auth.json"
...
# Elevate-only auto-fallback model for the ChatGPT-account Codex endpoint.
# Hermes upstream made codex require an explicit model (drifting allow-list),
# but Elevate keeps a hardcoded default so codex can sit in the auto provider
# chain as a fallback when OR / Nous / custom all return (None, None).
# Bump when the allow-list rotates. See _try_codex() and _get_provider_chain().
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Elevate Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``elevate_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.elevate/auth.json for an active Nous provider.
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        creds = resolve_nous_runtime_credentials(
            min_key_ttl_seconds=max(60, int(os.getenv("ELEVATE_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
            timeout_seconds=float(os.getenv("ELEVATE_NOUS_TIMEOUT_SECONDS", "15")),
            inference_auth_mode=(
                NOUS_INFERENCE_AUTH_MODE_LEGACY
                if force_refresh
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``elevate auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``elevate_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from elevate_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Elevate auth store.
    ...
    try:
        from elevate_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from elevate_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from elevate_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from elevate_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from elevate_cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `elevate setup` or `elevate model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_codex() -> Tuple[Optional[Any], Optional[str]]:
    """Elevate-only auto-fallback try-helper for the Codex chain rung.

    Hermes upstream removed this in favor of forcing callers to pass a model
    to ``_build_codex_client``, but Elevate keeps it so ``_get_provider_chain``
    can include ``("openai-codex", _try_codex)`` as a no-args fallback.
    Returns (None, None) when no Codex OAuth token is available.
    """
    pool_present, entry = _select_pool_entry("openai-codex")
    if pool_present:
...
def _try_azure_foundry(
    *,
    ...
    :func:`elevate_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from elevate_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _get_provider_chain() -> List[tuple]:
    ...
    Elevate-specific: ``openai-codex`` IS in this chain (uses the hardcoded
    ``_CODEX_AUX_MODEL`` fallback). Hermes upstream removed codex from
    the auto chain because the ChatGPT-account Codex endpoint accepts a
    drifting, undocumented model allow-list. We keep the rung because
    Elevate users routinely rely on Codex OAuth as their backstop when
    OR / Nous / custom all 402.
    """
    return [
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second elevate
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``elevate config aux reset``)."""
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `elevate model` but the
    #    old OPENAI_BASE_URL lingers in ~/.elevate/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: elevate model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.elevate/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from elevate_cli.copilot_auth import copilot_request_headers
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: elevate auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
    ...
    if provider == "openai-codex":
        if not model:
            # Elevate-only: fall back to the hardcoded ``_CODEX_AUX_MODEL``
            # so the auto provider chain and raw-codex callers can route
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: elevate model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: elevate model -> xAI Grok OAuth — SuperGrok Subscription)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: elevate doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("elevate_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from elevate_cli.models import _should_use_copilot_responses_api
