...
lane (restore 1.2.65). For the pinned 1.2.73 candidate there is also a
roll-forward lane that advances Beta to the minimal 1.2.74 recovery package;
read "Roll-forward recovery (1.2.74)" below and choose the lane first.
...
## Roll-forward recovery (1.2.74)

The second recovery lane for the same isolated `beta` channel advances the
Beta feed and all four Beta download aliases forward to 1.2.74, a minimal
...
exact 1.2.73 Beta candidate, its own version is exactly 1.2.74, and the next
full Beta release must be strictly newer than 1.2.74
...
- Roll-forward when installed 1.2.73 Beta machines must be contained through
...
## What the 1.2.74 recovery package is
...
`desktop/dist/recovery/`: `Elevate-Beta-Recovery-1.2.74-mac-x64.zip/.dmg`,
`Elevate-Beta-Recovery-1.2.74-mac-arm64.zip/.dmg`, the recovery
...
The 1.2.73 `candidate-receipt.json` carries a `recovery` block
(`kind: elevate-beta-recovery-package`) that pins the SHA-256 and SHA-512 of
the recovery `beta-mac.yml` and all four 1.2.74 artifacts, both recovery app
...
All four versioned 1.2.74 recovery artifacts must already exist on the
update host, matching the receipt's recovery hashes, before the 1.2.73
...
recovery feed (`.realtor-beta-recovery-1.2.74-beta-mac.yml`, a dot-name in
...
version other than exact 1.2.74, a recovery package bound to a different
...
(`.realtor-beta-recovery-1.2.74-beta-mac.yml`) and all four retained public
...
`desktop/dist/rollback-receipts/beta/<version>-<candidate-id>/recovery-1.2.74/`
...
Beta readback is exactly 1.2.74 with the receipt-bound recovery feed hash,
...
- the Beta feed advanced to exactly 1.2.74 with the receipt-bound recovery
