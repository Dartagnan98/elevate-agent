# Skyleigh Admin Hub — BC Guide × Elevate Skills Gap Analysis

Cross-references BC's `transaction-guide-bc/` content + Skyleigh's PCS scripts
...
**Settings model:** Province is a backend setting. Skyleigh = BC. When other
...
same layout — see `~/skyleigh-tools/scripts/normalize-province-layout.cjs`.)
...
| PCS leads scraper | `~/skyleigh-tools/scripts/pcs-leads-scraper.cjs` | B1 entry (Xposure → Lofty hot-buyer pipeline) |
| PCS hot-leads analyzer | `~/skyleigh-tools/scripts/pcs-hot-leads-analyzer.cjs` | B1 (auto-score, set Lofty stage) |
| PCS hot-leads PDF | `~/skyleigh-tools/scripts/pcs-hot-leads-pdf.cjs` | B1 (weekly report) |
...
## Top 5 Build Priority (by Skyleigh-impact)
