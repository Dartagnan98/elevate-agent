# Skyleigh Admin Hub — Kanban Model

Reconciles `skyleigh-admin-hub-seller-buyer-phases.md` (formal 8+8 phases)
with Skyleigh's whiteboard sketch (Buyer Admin / Seller Admin / TOP 25 / Other items).
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ TOP 25  (pinned active clients across both lanes — Skyleigh's focus list)│
├──────────────────────────────────────────────────────────────────────────┤
│ SELLER ADMIN                                                              │
...
### TOP 25 (pinned)
Skyleigh's focus list. Manually pinned cards across both lanes — the 25 (or N)
...
Movement is **manual** by Skyleigh (drag-and-drop) but the system **suggests
...
Skyleigh always confirms the move. The system never auto-advances a card.
...
| Deadlines | Card-level state, edited inline by Skyleigh |
| Drafts | `tasks.jsonl` (lead_follow_up + admin task types) |
| Transaction guide | `~/skyleigh-tools/knowledge/exp-agent-centre/transaction-guide-bc/` (referenced inline by checklist) |
