# Admin Hub — Agentic Workflow Plan

> Companion to `admin-hub-bc-gap-analysis.md`. The gap doc lists what
...
chain and DigiSign integration ship in **client-tools**; the kanban + dispatcher
work ships in **elevate-agent**. Elevate loads client-tools at runtime via
`skills_hub.py`.
...
- 9 provinces normalized to identical layout under
  `~/client-tools/knowledge/exp-agent-centre/transaction-guide-{prov}/`
...
- **Target shape:** 2 lanes (Listing / Buyer) × 10 stages per side (Stage 0
  CMA/Lead → Stage 9 Closed), matching the pilot realtor's master sheet. The current
...
- **client-specific skills already pipelined** at `~/.elevate/skills/real-estate-admin/`,
  ...
  - **`marketing` router** → `marketing-inputs` → `marketing-render` (Playwright)
    ...
    Working dir: `~/.elevate/tmp/client-tools/data/marketing/runs/<run>/`.
...
- PCS hot-leads: `~/client-tools/scripts/`
...
| B | **Deal store** — SQLite + JSONL event log | `~/.elevate/data/operational.db` + `~/.elevate/data/deal-events.jsonl` |
...
| E | **Province content router** | new `cli/elevate_cli/province_guide.py` — `get_lifecycle_steps`, `get_form`, `get_conditional_docs(toggles)` reads from SQLite reference tables seeded from `~/client-tools/knowledge/exp-agent-centre/transaction-guide-{prov}/` and the master sheet |
...
4. Workspace settings UI: province dropdown (defaults to BC for pilot realtor).
5. `province_guide.py` — `get_checklist(province, slug) → markdown`,
   ...
   via existing `sources.tools_root` config (not a hardcoded `~/client-tools`).
...
**Exit criteria:** pilot realtor can open the kanban in two browsers, move a card,
...
> **Email tooling note:** the master sheet says "Flodesk" on Coming Soon, Just Listed, Moving Checklist, and Sold blasts. the pilot realtor's Flodesk subscription was canceled (see `subscription-audit-2026-04-27.md`). Email is now **Mailjet via the Elevate `marketing` skill** — the seed script normalizes lifecycle_steps rows where `tool_or_form = 'Flodesk'` to `marketing skill (Mailjet)`. Templates (Coming Soon, Just Listed, Moving Checklist, Sold) live inside the marketing skill, not in a separate ESP.
...
9. **Three triggers live first** (highest impact, all skills already exist):
    ...
    - **Stage 0 entry (CMA requested) → `cma` router.** Full chain produces
      cma-pdf and audits it. Existing `cma-audit` already enforces the pilot realtor's
...
|---|---|---|
| 1 | **Action registry format** — YAML vs Python decorators | YAML is editable by non-devs (pilot realtor could tune triggers later); decorators give type-safety and refactor support. Default: YAML. |
| 2 | **Card store backend** — JSONL append log vs hosted cloud DB | JSONL matches existing `lead-events.jsonl` pattern, zero new infra. A hosted cloud DB is multi-device but adds auth complexity. Default: JSONL for single-operator; revisit if RAgent multi-tenant lands. |
| 3 | ~~E-sign vendor~~ | **RESOLVED**: `webforms` skill handles BCREA form fill, `digisign` skill (DigiSign via SkySlope API) handles signed delivery. Already implemented at `~/.elevate/skills/real-estate-admin/{webforms,digisign}/`. No vendor decision needed. |
...
- **eXp e-sign vendor confirmation** from pilot realtor (Webforms vs DigiSign vs Authentisign).
