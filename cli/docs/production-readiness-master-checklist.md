...
| API-06 | Debug routes | PASS iff debug endpoints redact secrets and are not production footguns | UNKNOWN | Tests/source review |
...
| REL-04 | Bundled runtime parity | PASS iff installed bundle `cli` and `web_dist` match expected source/build | UNKNOWN | Fresh arm64 candidate at `desktop/dist/smoke-arm64/mac-arm64/Elevate.app` passed non-`--skip-sidecar` runtime smoke with `--skip-seal`: bundled `web_dist` matched repo, served assets matched bundled assets, and `/api/ws` streamed/resumed; `/Users/dartagnanpatricio/Applications/Elevate.app` still needs replacement from finalized/notarized artifacts plus installed-app smoke |
