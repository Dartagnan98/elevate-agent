...
| CRM-04 | Source inbox/leads | PASS iff source inbox/leads routes have caller, test, runtime probe | UNKNOWN | Backend source-inbox debug contracts plus frontend empty/fallback debug note pass; `/leads` profile status caller now maps menu labels to source-inbox status enums; route contracts prove `POST /api/source-inbox/profile`, `POST /api/source-inbox/draft`, and `POST /api/source-inbox/apple-messages/directions`; live source filters now key on `sourceId`; dynamic Composio source actions now accept real `composio-*` sources; DB-only skipped drafts now surface and restore back to `pending_approval`; backend lead-section lanes now feed Hot, Follow-ups, and buyer-search counts |
