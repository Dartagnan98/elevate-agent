...
| API-06 | Debug routes | PASS iff debug endpoints redact secrets and are not production footguns | UNKNOWN | `/api/source-inbox?debug=1` fallback errors now return exception type plus stable code only, with a regression asserting DSNs/tokens/emails/local paths are absent; `debug share --no-redact`, public plugin rescan, and blank-trace redaction still need closure |
