...
| API-05 | Feature route contracts | PASS iff each dashboard route family has a caller and a contract test or explicit gap | UNKNOWN | `/api/activity` now has a local route contract proving DB-backed surface activity projection, agent filter, limit, and response shape; remaining explicit local gaps: composio, ayrshare, social, integrations |
