...
| HOST-05 | Admin/account | PASS iff account/admin APIs enforce guards and expose visible errors | UNKNOWN | Account/org read contracts, admin missing-record 404s, org seat-limit enforcement for direct member add plus stale invite accept, inactive invite accept no longer consumes the invite or adds membership before returning `402`, admin license revoke tenant-safety, org owner/seat/audit invariants, and account email/password audit-outage contracts pass; broader admin permission matrix still needs coverage |
