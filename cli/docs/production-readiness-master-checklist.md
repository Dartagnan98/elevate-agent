...
| API-05 | Feature route contracts | PASS iff each dashboard route family has a caller and a contract test or explicit gap | UNKNOWN | `/api/activity` now has a local route contract proving DB-backed surface activity projection, agent filter, limit, and response shape; `/api/integrations` now has a local GET/PUT/POST contract; `/api/ayrshare/*` now has local status/key/profiles/scheduled/history coverage; remaining explicit local gaps: composio, social |
