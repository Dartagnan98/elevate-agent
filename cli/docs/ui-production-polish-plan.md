...
| todo | P2 | `/memory` | `real-estate-hub/memory/index.tsx` right Pipeline card | 4 HubMetric (Pending/Processed/Failed/Active sessions) duplicate the SummaryTile pipeline state; first 3 are the same data shown twice | Drop Pending/Processed/Failed from the right card; keep only Active sessions, or remove the card entirely | tsc + vite |
| todo | P2 | `/social` | `real-estate-hub/social/index.tsx` "No snapshot yet" Card | `py-6 text-center` heavy empty state, matches the pattern already cleaned across leads/skills/oauth | Replace with single-line muted text in `text-xs text-muted-foreground/80 px-1 py-1` style | tsc + vite |
| todo | P2 | `/social` | `real-estate-hub/social/index.tsx` idea queue loading | `flex items-center justify-center py-12` loading block jumps layout | Switch to a muted single-line loading row inside the existing container | tsc + vite |
| todo | P2 | `/social` | `real-estate-hub/social/index.tsx` "Your posts" + "All posts" headers | Subtitles use loud `font-mono-ui uppercase tracking-wider` even after S5 cleanup | Drop uppercase mono on subtitles + "{N} of {M}" counter; use normal sentence case in muted text | tsc + vite |
| todo | P2 | `/cron` | `CronPage.tsx` empty state (line ~735) | `py-8 text-center` heavy empty state pattern already cleaned elsewhere | Switch to muted single-line text matching the leads/skills convention | tsc + vite |
| todo | P2 | `/cron` | `CronPage.tsx` loading state (line ~588) | `py-24` causes large layout jump while jobs load | Reduce to a muted single-line loading text or skeleton row | tsc + vite |
| todo | P2 | `/tasks` | `real-estate-hub/tasks/index.tsx` DealTaskRow "Open deal" link | `font-mono-ui text-[0.68rem] font-semibold uppercase tracking-[0.12em]` link sits next to a regular shadcn Button; inconsistent visual weight | Convert "Open deal" to a `Button variant="ghost" size="sm"` matching "Run AI" | tsc + vite |
| todo | P1 | `/tasks` | `real-estate-hub/tasks/index.tsx` HandoffRow | Shows raw `fromAgentId → toAgentId` with no human labels; no approve/reroute action — only a "waiting" badge | Map agent IDs to display names; add a primary "Review" or "Open session" Button so the queue is actionable, not just a status list | tsc + vite |
| todo | P1 | `/sessions` | `SessionsPage.tsx` SessionRow + ToolCallBlock | Uses `border-l-2 border-l-warning` (ToolCallBlock) and `border-l-2 border-l-success` (active SessionRow). Border-left stripes are explicitly banned by the project design system | Replace active SessionRow with a small `Live` badge + ring (already exists); ToolCallBlock keep border-warning text colour but drop the stripe | tsc + vite |
| todo | P2 | `/sessions` | `SessionsPage.tsx` expanded session content | `py-8` loading, `py-4 text-center` empty/error inside the row | Use single-line muted loading + muted single-line empty/error | tsc + vite |
| todo | P2 | `/logs` | `LogsPage.tsx` error block | `border-l-2 border-l-destructive` stripe vs the no-stripes design rule | Drop the left stripe; keep destructive text + bg-card row | tsc + vite |
| todo | P2 | `/logs` | `LogsPage.tsx` empty log lines | `text-muted-foreground text-center py-8` heavy empty state | Switch to single-line muted text | tsc + vite |
| todo | P2 | `/analytics` | `AnalyticsPage.tsx` loading/error/empty | `py-24` loading, `py-6 text-center` error, `py-12` empty with centered icon are heavy AI-style empty states | Use single-line muted text matching the leads/skills convention; drop large centered icon | tsc + vite |
| todo | P3 | `/project` | `ProjectPage.tsx` loading | `py-24` initial loading block jumps layout | Single-line muted loading row | tsc + vite |
| todo | P3 | `/env` | `EnvPage.tsx` loading | `py-24` initial loading block jumps layout | Single-line muted loading row | tsc + vite |
