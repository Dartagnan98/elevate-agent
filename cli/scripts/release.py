...
VERSION_FILE = REPO_ROOT / "elevate_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "elevate@nousresearch.com": "NousResearch",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoelevate@cursor.com", "elevate@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Dartagnan98/elevate-agent",
    ...
    lines.append(f"# Elevate v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Elevate. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Elevate Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Elevate Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Elevate v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Elevate v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Elevate v{new_version} ({calver_date})' "
