 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.elevate/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.elevate', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.elevate', 'image_cache');
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Elevate's own reply messages in self-chat mode to avoid loops.
