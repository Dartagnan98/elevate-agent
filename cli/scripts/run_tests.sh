#!/usr/bin/env bash
# Canonical test runner for elevate. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.elevate/elevate/venv"; do
...
# Unset ELEVATE_* behavioral vars too.
unset ELEVATE_YOLO_MODE ELEVATE_INTERACTIVE ELEVATE_QUIET ELEVATE_TOOL_PROGRESS \
      ELEVATE_TOOL_PROGRESS_MODE ELEVATE_MAX_ITERATIONS ELEVATE_SESSION_PLATFORM \
      ELEVATE_SESSION_CHAT_ID ELEVATE_SESSION_CHAT_NAME ELEVATE_SESSION_THREAD_ID \
      ELEVATE_SESSION_SOURCE ELEVATE_SESSION_KEY ELEVATE_GATEWAY_SESSION \
      ELEVATE_PLATFORM ELEVATE_INFERENCE_PROVIDER ELEVATE_MANAGED ELEVATE_DEV \
      ELEVATE_CONTAINER ELEVATE_EPHEMERAL_SYSTEM_PROMPT ELEVATE_TIMEZONE \
      ELEVATE_REDACT_SECRETS ELEVATE_BACKGROUND_NOTIFICATIONS ELEVATE_EXEC_ASK \
      ELEVATE_HOME_MODE 2>/dev/null || true
...
WORKERS="${ELEVATE_TEST_WORKERS:-4}"
