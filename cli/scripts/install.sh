...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.elevate/elevate)"
            echo "  --elevate-home PATH  Data directory (default: ~/.elevate, or \$ELEVATE_HOME)"
...
get_elevate_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/elevate" ]; then
        echo "$link_dir/elevate"
    else
        echo "elevate"
    fi
}
...
check_node() {
    ...
    if [ -x "$ELEVATE_HOME/node/bin/node" ]; then
        export PATH="$ELEVATE_HOME/node/bin:$PATH"
        local found_ver=$("$ELEVATE_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Elevate-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.elevate/node/..."
    ...
    # Place into ~/.elevate/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$ELEVATE_HOME/node"
    mkdir -p "$ELEVATE_HOME"
    mv "$extracted_dir" "$ELEVATE_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$ELEVATE_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$ELEVATE_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$ELEVATE_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$ELEVATE_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ELEVATE_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.elevate/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Elevate itself does not require or retain root access."
    ...
    if [ -d "$checkout_dir" ]; then
        if [ -d "$checkout_dir/.git" ] && [ "$HAS_GIT" = true ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="elevate-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Elevate behaves unexpectedly."
                    else
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        if [ -z "$ELEVATE_BIN" ]; then
            log_warn "elevate not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ELEVATE_BIN" ]; then
        log_warn "elevate entry point not found at $ELEVATE_BIN"
    ...
    # Create a user-facing shim for the elevate command.
    mkdir -p "$command_link_dir"
    ln -sf "$ELEVATE_BIN" "$command_link_dir/elevate"
    log_success "Symlinked elevate → $command_link_display_dir/elevate"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "elevate command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Elevate — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so elevate works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.elevate directory structure (config at top level, code in subdir)
    mkdir -p "$ELEVATE_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.elevate/.env (top level, easy to find)
    if [ ! -f "$ELEVATE_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ELEVATE_HOME/.env"
            log_success "Created ~/.elevate/.env from template"
        else
            touch "$ELEVATE_HOME/.env"
            log_success "Created ~/.elevate/.env"
        fi
    else
        log_info "~/.elevate/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.elevate/config.yaml (top level, easy to find)
    if [ ! -f "$ELEVATE_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ELEVATE_HOME/config.yaml"
            log_success "Created ~/.elevate/config.yaml from template"
        fi
    else
        log_info "~/.elevate/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ELEVATE_HOME/SOUL.md" ]; then
        cat > "$ELEVATE_HOME/SOUL.md" << 'SOUL_EOF'
# Elevate Persona
...
-->
SOUL_EOF
        log_success "Created ~/.elevate/SOUL.md (edit to customize personality)"
    fi
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (elevate --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'elevate setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Elevate to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ELEVATE_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'elevate whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ELEVATE_CMD="$(get_elevate_command_path)"
                $ELEVATE_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'elevate whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'elevate gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ELEVATE_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ELEVATE_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: elevate gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: elevate gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ELEVATE_CMD gateway > "$ELEVATE_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.elevate/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: elevate gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: elevate gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.elevate/):${NC}"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}elevate${NC}              Start chatting"
    echo -e "   ${GREEN}elevate setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}elevate config${NC}       View/edit configuration"
    echo -e "   ${GREEN}elevate config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}elevate gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}elevate update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'elevate' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'elevate' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
