"""CLI entry point for the elevate ACP adapter.

Loads environment variables from ``~/.elevate/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ELEVATE_HOME (default ``~/.elevate``)."""
    from elevate_cli.env_loader import load_elevate_dotenv

    elevate_home = get_elevate_home()
    loaded = load_elevate_dotenv(elevate_home=elevate_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", elevate_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting elevate ACP adapter")
    ...
    import acp
    from .server import ElevateACPAgent

    agent = ElevateACPAgent()
    try:
