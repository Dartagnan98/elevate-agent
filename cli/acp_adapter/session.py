"""ACP session manager — maps ACP sessions to Elevate AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.elevate/state.db``) so they
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["elevate-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Elevate agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Elevate AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Elevate runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.elevate/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ELEVATE_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_elevate_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=elevate_home / "state.db")
            return self._db_instance
        except Exception:
