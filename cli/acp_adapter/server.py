"""ACP agent server — exposes Elevate via the Agent Client Protocol."""
...
class ElevateACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Elevate AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Elevate version",
        },
    )
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from elevate_cli.models import detect_provider_for_model, parse_model_input
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["elevate-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Elevate using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="elevate", version=ELEVATE_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Elevate' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Elevate on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # not the executor's. Also set ELEVATE_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue ELEVATE_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            previous_interactive = os.environ.get("ELEVATE_INTERACTIVE")
            ...
            try:
                ...
                if previous_interactive is None:
                    ...
                    os.environ["ELEVATE_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Elevate v{ELEVATE_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Elevate has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
