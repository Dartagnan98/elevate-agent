...
if tinker_atropos_dir.exists():
    ...
    os.environ['ELEVATE_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to elevate directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.elevate/config.yaml if not provided)
    ...
    """
    # Load config from ~/.elevate/config.yaml
    config = load_elevate_config()
