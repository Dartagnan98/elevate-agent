"""
Elevate MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Elevate-specific extra)

Usage:
    elevate mcp serve
    elevate mcp serve --verbose
...
logger = logging.getLogger("elevate.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ELEVATE_HOME."""
    try:
        from elevate_constants import get_elevate_home
        return get_elevate_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ELEVATE_HOME", Path.home() / ".elevate")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from elevate_constants import get_elevate_home
        directory_file = get_elevate_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ELEVATE_HOME", Path.home() / ".elevate")
...
class EventBridge:
    ...
    This is the Elevate equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from elevate_constants import get_elevate_home
            db_file = get_elevate_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ELEVATE_HOME", Path.home() / ".elevate")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Elevate MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "elevate",
        instructions=(
            "Elevate messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Elevate MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
