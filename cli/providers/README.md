...
Registry and ABC for every inference provider Elevate knows about.
...
`$ELEVATE_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
under `$ELEVATE_HOME/plugins/model-providers/` for a private plugin).
