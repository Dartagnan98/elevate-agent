...
t1 = kanban_create(
    ...
    tenant=os.environ.get("ELEVATE_TENANT"),
)["task_id"]
...
**Tenant inheritance.** If `ELEVATE_TENANT` is set in your env, pass `tenant=os.environ.get("ELEVATE_TENANT")` on every `kanban_create` call so child tasks stay in the same namespace.
