...
- Operational source of truth: the realtor's maintained local approved-forms library and form-version register, not WEBForms/TransactionDesk, because WEBForms two-step authentication often blocks workflows. WEBForms/TransactionDesk is optional for downloading/seeding templates or cross-checking when accessible.
...
7. Select the approved CPS base form source. Priority order:
   ...
   - the realtor's local approved-forms pack/register only when `data/forms/local-approved-forms-register.json` and the blank/template path confirm the required clean current form and tested fill mapping.
