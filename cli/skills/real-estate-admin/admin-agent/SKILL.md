...
When the user has a deal `waiting_human` for missing intake and they reply with the answers (Telegram or chat) — e.g. a few lines of values, or `field: value` pairs — treat that as filling the prompt, NOT a new request. Steps:
1. Identify the target `waiting_human` run (match by deal/address in context; if only one is pending, use it).
2. Map their reply to that run's `requiredFields` (line order, or explicit `field: value`).
...
4. Reply briefly, e.g. `Got it — running the CMA now.` Do not re-ask for anything they already answered.
Never dump internal detail back at them; keep replies to one clean line.
