...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ELEVATE_HOME", os.path.expanduser("~/.elevate")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ELEVATE_HOME", Path.home() / ".elevate")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Elevate config paths
# ═══════════════════════════════════════════════════════════════════

ELEVATE_HOME = Path(os.getenv("ELEVATE_HOME", Path.home() / ".elevate"))
CONFIG_PATH = ELEVATE_HOME / "config.yaml"
PREFILL_PATH = ELEVATE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Elevate models — already uncensored, just needs clean prompt
    "elevate": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "elevate" in model_lower or "nous" in model_lower:
        return "elevate"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Elevate config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.elevate/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Elevate for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Elevate for changes to take effect.")
