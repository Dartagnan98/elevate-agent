...
## 5. GODMODE FAST — Elevate 4 405B (Zero Refusal)

**Model:** `nousresearch/elevate-4-405b`
**Strategy:** Elevate models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
Pick a template above and set it in `~/.elevate/config.yaml`:
