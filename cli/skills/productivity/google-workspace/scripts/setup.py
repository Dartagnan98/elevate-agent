#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Elevate.
...
try:
    ...
    ELEVATE_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if ELEVATE_AGENT_ROOT.exists():
        sys.path.insert(0, str(ELEVATE_AGENT_ROOT))
    from elevate_constants import display_elevate_home, get_elevate_home

ELEVATE_HOME = get_elevate_home()
TOKEN_PATH = ELEVATE_HOME / "google_token.json"
CLIENT_SECRET_PATH = ELEVATE_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ELEVATE_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Elevate profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Elevate home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_elevate_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Elevate")
