...
The registry of record is `elevate_cli/commands.py` — every consumer
...
/reload-skills       Re-scan ~/.elevate/skills/ for added/removed skills
...
~/.elevate/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.elevate/state.db          Canonical session store (SQLite + FTS5)
~/.elevate/logs/             Gateway and error logs
~/.elevate/auth.json         OAuth tokens and credential pools
~/.elevate/hermes-agent/     Source code (if git-installed)
```
...
Full enumeration lives in `toolsets.py` as the `TOOLSETS` dict; `_ELEVATE_CORE_TOOLS` is the default bundle most platforms inherit from.
...
**Restart required.** `security.redact_secrets` is snapshotted at import time — toggling it mid-session (e.g. via `export ELEVATE_REDACT_SECRETS=true` from a tool call) will NOT take effect for the running process. Tell the user to run `hermes config set security.redact_secrets true` in a terminal, then start a new session. This is deliberate — it prevents an LLM from flipping the toggle on itself mid-task.
...
- `hermes --yolo …`
- `export ELEVATE_YOLO_MODE=1`

Note: YOLO / `approvals.mode: off` does NOT turn off secret redaction. They are independent.

### Shell hooks allowlist

Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.elevate/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
- **Telemetry:** sidecar at `~/.elevate/skills/.usage.json` holds
...
see a focused `kanban_*` toolset gated by `ELEVATE_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `ELEVATE_KANBAN_BOARD` pinned in env); tenant is a soft namespace
...
| Source code | `~/.elevate/hermes-agent/` |
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/elevate_cli/test_auth_toctou_file_modes.py`)
- `signal.SIGALRM` → Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
