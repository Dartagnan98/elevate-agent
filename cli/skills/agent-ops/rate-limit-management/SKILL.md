...
```bash
# Elevate surfaces usage in the dashboard; there is no agent usage CLI.
```
...
| `--chat-id ID` | the agent's Comms channel | Telegram chat ID for alerts |
| `--force` | false | Bypass 3-minute result cache |

**Cache:** Results are cached for 3 minutes at the Elevate usage cache under ~/.elevate to avoid hitting the API hard limit (~5 requests per token before 429).
...
```json
{
  "name": "usage-check",
  "interval": "15m",
  "prompt": "Check usage status in the dashboard and apply the rate limit protocol from .claude/skills/rate-limit-management/SKILL.md based on current utilization."
}
```
