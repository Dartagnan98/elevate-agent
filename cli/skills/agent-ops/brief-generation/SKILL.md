...
Briefs and the run summary are files in the agent's workdir. To get them in front of a person or another agent, use Elevate-native delivery — never a daemon, file inbox, or IPC push:

- **Hand off to a person or another agent:** use the **agent_handoff** tool, and post the brief or summary into the conversation via native **Comms**.
- **If delivery is approval-gated:** open the item in native **Approvals** and only deliver once approved. The "Delivery Status" line should read `pending approval` until then. (No Telegram approvals — dashboard only.)
- **If a brief implies follow-up work** (e.g. "investigate further", "create content"): create it in native **Tasks** (or via the **agent_bus** tool, action `create_task`), so it lands on the board rather than living only inside the summary.
- **Record the run** in memory via the **memory** tool and the agent's `MEMORY.md` / `memory/<day>.md`, so the next run knows what was already covered.

If a desired delivery destination has no Elevate mechanism (e.g. an external system this agent has no tool for), do not invent a command — note it in the summary and raise a `[HUMAN]` task describing the manual step needed.
