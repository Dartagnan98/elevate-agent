...
# voice-agent-factory — Elevate agent -> ElevenLabs voice agent
...
- An Elevate install with the target agent onboarded (`<agentDir>` = the agent's
  workspace, e.g. `orgs/<org>/agents/<name>/`), and the Agent Hub configured.
...
- A process manager (the desktop app or equivalent) for the gateway. The quick tunnel itself
...
`the agent's skills`), identity from AGENTS/IDENTITY/SOUL/
