...
# All tasks this week
agent_bus list-tasks
agent_bus list-tasks --status completed
...
cat GOALS.md
your goals via agent_bus (action get_goals)
...
```bash
send to the realtor via Comms: "<message chunk>"
```
...
```bash
# Log event
agent_bus log-event action briefing_sent info --meta '{"type":"weekly_review"}'

# Update heartbeat
agent_bus update-heartbeat "weekly review complete - next week planned"
