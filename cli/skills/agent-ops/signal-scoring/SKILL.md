...
This step runs inside the agent's own workdir (its identity is the agent name; its
working directory holds `research/`). Run it as part of a recurring research run via
the **cron** tool, or hand it to an isolated worker via **delegate_task** when you
want the scoring pass to run in parallel without touching the agent's main session.
It reads and writes only local files under `research/` -- no external service is
required to score.
...
not suppressed if delivery fails. After writing `signals-selected.json`, pass the
shortlist to the next step (brief-generation / delivery-routing) with the
**agent_handoff** tool, or post it for review with the native **Tasks** /
**Approvals** tools if a human gate is required.
