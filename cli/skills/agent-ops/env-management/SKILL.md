---
name: env-management
description: "Handle the full lifecycle of Elevate secrets and env vars. Use when the user says 'add an API key', 'update this credential', 'what keys are configured', 'this key is missing', or is onboarding a tool that needs credentials or reconfiguring agents after a key change. Not specifically for OAuth token refresh/repair — use oauth-rotation instead. Agents can't write the secret store; raise a [HUMAN] task to store it, then manage_agent to reload — never edit files."
