...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(elevate_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(elevate_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(elevate_home):
    """When the budget is exhausted, a '⏸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from elevate_cli.goals import GoalManager, save_goal
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(elevate_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(elevate_home):
