...
See: https://github.com/NousResearch/elevate/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for elevate_constants.get_subprocess_home()."""

    def test_returns_none_when_elevate_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        profile_home = elevate_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
        from elevate_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".elevate" / "profiles"
        ...
        monkeypatch.setenv("ELEVATE_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ELEVATE_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(elevate_home / "home")
    ...
    def test_no_injection_when_elevate_home_unset(self, monkeypatch):
        monkeypatch.delenv("ELEVATE_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from elevate_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from elevate_cli.profiles import create_profile
