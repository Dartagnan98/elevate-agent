...
def _receipt() -> tuple[dict, bytes, bytes, bytes]:
    recovery_feed = _feed("1.2.97", "Elevate-Beta-Recovery")
    stable_feed = _feed("1.2.63", "Elevate")
    candidate_feed = _feed("1.2.96", "Elevate-Beta")
...
def test_local_recovery_drill_rolls_beta_forward_and_leaves_stable_and_data_untouched(
    ...
    assert result["candidate_version"] == "1.2.96"
    assert result["recovery_version"] == "1.2.97"
