"""Tests for elevate_cli/goals.py — persistent cross-turn goals."""
...
def hermes_home(tmp_path, monkeypatch):
    """Isolated ELEVATE_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves ELEVATE_HOME.
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from elevate_cli.goals import judge_goal
...
class TestGoalManager:
    ...
    def test_continuation_prompt_shape(self, hermes_home):
        ...
        mgr.set("port goal command to elevate")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to elevate" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from elevate_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, hermes_home):
        ...
        from elevate_cli import goals
        from elevate_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        import json
        from elevate_cli.goals import GoalState
