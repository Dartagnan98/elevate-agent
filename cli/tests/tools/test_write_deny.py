...
class TestWriteDenyExactPaths:
    ...
    def test_elevate_env(self):
        # ``.env`` under the active ELEVATE_HOME (profile-aware, not just
        # ``~/.elevate``) must be write-denied. The hermetic test conftest
        # points ELEVATE_HOME at a tempdir — resolve via get_elevate_home()
        # to match the denylist.
        from elevate_constants import get_elevate_home
        path = str(get_elevate_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_elevate_config_not_env(self):
        path = os.path.join(str(Path.home()), ".elevate", "config.yaml")
