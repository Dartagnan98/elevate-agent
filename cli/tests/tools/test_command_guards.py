...
def _clean_state():
    ...
    saved = {}
    for k in ("ELEVATE_INTERACTIVE", "ELEVATE_GATEWAY_SESSION", "ELEVATE_EXEC_ASK", "ELEVATE_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ELEVATE_INTERACTIVE"] = "1"
        session_key = os.getenv("ELEVATE_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ELEVATE_INTERACTIVE or ELEVATE_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
