...
def test_elevate_provider_subclass_exists():
    """ElevateMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ELEVATE_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ELEVATE_PROVIDER_CLS is not None
    assert issubclass(_ELEVATE_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_elevate_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ElevateMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ELEVATE_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ELEVATE_HOME", str(tmp_path))
    ...
    assert _ELEVATE_PROVIDER_CLS is not None
    assert isinstance(provider, _ELEVATE_PROVIDER_CLS)
    assert provider._elevate_server_name == "srv"
