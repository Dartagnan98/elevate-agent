...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._elevate_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_elevate_bin(self):
        """After _INSTALL_FAILED, manual install in ELEVATE_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        elevate_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(elevate_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(elevate_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._elevate_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == elevate_bin
            assert _tirith_mod._resolved_path == elevate_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ELEVATE_HOME isolation
# ---------------------------------------------------------------------------

class TestElevateHomeIsolation:
    def test_elevate_bin_dir_respects_elevate_home(self):
        """_elevate_bin_dir must use ELEVATE_HOME, not hardcoded ~/.elevate."""
        from tools.tirith_security import _elevate_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ELEVATE_HOME": tmpdir}):
            result = _elevate_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_elevate_home(self):
        """_failure_marker_path must use ELEVATE_HOME, not hardcoded ~/.elevate."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ELEVATE_HOME": "/custom/elevate"}):
            result = _failure_marker_path()
        assert result == "/custom/elevate/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ELEVATE_HOME; verify it's active."""
        elevate_home = os.getenv("ELEVATE_HOME")
        assert elevate_home is not None, "ELEVATE_HOME should be set by conftest"
        assert "elevate_test" in elevate_home, "Should point to test temp dir"

    def test_get_elevate_home_fallback(self):
        """Without ELEVATE_HOME set, falls back to ~/.elevate."""
        from tools.tirith_security import _get_elevate_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ELEVATE_HOME entirely
            os.environ.pop("ELEVATE_HOME", None)
            result = _get_elevate_home()
        assert result == os.path.join(os.path.expanduser("~"), ".elevate")
