...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        elevate_home.mkdir()
        (elevate_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(elevate_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.elevate/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        (elevate_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        (elevate_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        (elevate_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_custom_container_base(self, tmp_path):
        elevate_home = tmp_path / ".elevate"
        (elevate_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ELEVATE_HOME": str(elevate_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.elevate")

        assert mounts[0]["container_path"] == "/home/user/.elevate/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.elevate/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.elevate/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ELEVATE_HOME."""
        monkeypatch.setenv("ELEVATE_HOME", str(tmp_path / ".elevate"))
        (tmp_path / ".elevate").mkdir()

        # Create a sensitive file one level above elevate_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside elevate_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ELEVATE_HOME must still be registered."""
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
        (elevate_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_elevate_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ELEVATE_HOME must be allowed."""
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        subdir = elevate_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ELEVATE_HOME pointing outside must be rejected."""
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))

        # Create a sensitive file outside elevate_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside elevate_home pointing outside
        symlink = elevate_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ELEVATE_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, elevate_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ELEVATE_HOME."""
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
        ...
        sensitive.write_text("{}")
        self._write_config(elevate_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(elevate_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ELEVATE_HOME via config must still mount."""
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))

        (elevate_home / "oauth.json").write_text("{}")
        self._write_config(elevate_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        (elevate_home / "cache" / "documents").mkdir(parents=True)
        (elevate_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.elevate/cache/documents" in paths
        assert "/root/.elevate/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.elevate/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        # Use legacy dir name — get_elevate_dir prefers old if it exists
        (elevate_home / "document_cache").mkdir()
        (elevate_home / "image_cache").mkdir()
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(elevate_home / "document_cache") in host_paths
        assert str(elevate_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.elevate/cache/documents" in container_paths
        assert "/root/.elevate/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        elevate_home = tmp_path / ".elevate"
        doc_dir = elevate_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        elevate_home = tmp_path / ".elevate"
        ss_dir = elevate_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.elevate/cache/screenshots/session_abc/screen1.png"
