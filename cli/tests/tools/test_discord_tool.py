...
# ---------------------------------------------------------------------------
# Toolset: discord_server only in elevate-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_server_in_elevate_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord_server" in TOOLSETS["elevate-discord"]["tools"]

    def test_discord_server_not_in_core_tools(self):
        from toolsets import _ELEVATE_CORE_TOOLS
        assert "discord_server" not in _ELEVATE_CORE_TOOLS

    def test_discord_server_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name == "elevate-discord":
                continue
            # The gateway toolset might include it if it unions all platform tools
            if name == "elevate-gateway":
                continue
