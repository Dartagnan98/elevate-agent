...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in elevate-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_hermes_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["elevate-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["elevate-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _ELEVATE_CORE_TOOLS
        assert "discord" not in _ELEVATE_CORE_TOOLS
        assert "discord_admin" not in _ELEVATE_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"elevate-discord", "hermes-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("elevate_cli.config.read_raw_config", bad_load)
        assert _load_allowed_actions_config() is None
...
@pytest.mark.skip(reason="Requires model_tools._clear_tool_defs_cache helper not yet ported from Hermes (B5b-phase3 pending)")
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["elevate-discord"], quiet_mode=True)
