...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("elevate/elevate-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned elevate/* and pr-* branches."""

    def test_prunes_orphaned_elevate_branch(self, git_repo):
        """elevate/elevate-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "elevate/elevate-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "elevate/elevate-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "elevate/elevate-deadbeef" in result.stdout
        ...
        assert "elevate/elevate-deadbeef" in orphaned
        ...
        assert "elevate/elevate-deadbeef" not in result.stdout
