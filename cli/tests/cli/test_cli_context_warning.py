...
def _isolate(tmp_path, monkeypatch):
    """Isolate ELEVATE_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal ElevateCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ElevateCLI.__new__(ElevateCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["elevate-core"]
        obj.compact = False
