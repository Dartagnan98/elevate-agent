...
"""
    ...
    assert "Tyler Williams > Active Projects: Elevate" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `elevate cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # elevate_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.elevate/config.yaml;
    #                      the script never writes to that file
    # hermes_config_mod   — docstring/help text references ``~/.Hermes/config.yaml``
    #                      as the SOURCE platform being migrated FROM; the script
    #                      reads from Hermes but never writes back to it.
    #
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Elevate prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Elevate to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Elevate config is great"
    assert mod.rebrand_text("openclaw should always respond concisely") == "Elevate should always respond concisely"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Elevate uses tools well"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Elevate remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "Elevate prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Elevate was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "Elevate uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Elevate said to ask Elevate about Elevate settings"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Elevate" in result
