"""Unit tests for elevate_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf elevate-ok"])
        try:
            output = _read_until(bridge, b"elevate-ok")
            assert b"elevate-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$ELEVATE_PTY_TEST\""],
            env={**os.environ, "ELEVATE_PTY_TEST": "pty-env-works"},
        )
        try:
