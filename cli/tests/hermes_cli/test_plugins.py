"""Tests for the Elevate plugin system (elevate_cli.plugins)."""
...
from elevate_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<elevate_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<elevate_home>/plugins/``; we derive
    ``<elevate_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ELEVATE_HOME>/config.yaml.
        # Config is always read from ELEVATE_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        elevate_home_str = os.environ.get("ELEVATE_HOME")
        if elevate_home_str:
            elevate_home = Path(elevate_home_str)
        else:
            elevate_home = base.parent
        elevate_home.mkdir(parents=True, exist_ok=True)
        cfg_path = elevate_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.elevate/plugins/ are discovered."""
        plugins_dir = tmp_path / "elevate_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("ELEVATE_HOME", str(tmp_path / "elevate_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ELEVATE_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".elevate" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        elevate_home = tmp_path / "elevate_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under elevate_plugins.<name>."""
        plugins_dir = tmp_path / "elevate_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ELEVATE_HOME", str(tmp_path / "elevate_test"))

        # Clean up any prior namespace module
        sys.modules.pop("elevate_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "elevate_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="elevate_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import elevate_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("elevate_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        elevate_home = tmp_path / "elevate_test"
        plugins_dir = elevate_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "elevate_test" / "plugins"
        # Set ELEVATE_HOME BEFORE _make_plugin_dir so auto-enable targets
