"""Tests for Bug #12905 fix — stale OAuth token detection in elevate model flow.

Bug 3: `elevate model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.elevate/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        from elevate_cli.main import _model_flow_anthropic
        cfg = {}
