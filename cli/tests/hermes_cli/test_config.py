"""Tests for elevate_cli configuration management."""
...
class TestGetElevateHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ELEVATE_HOME", None)
            home = get_elevate_home()
            assert home == Path.home() / ".elevate"

    def test_env_override(self):
        with patch.dict(os.environ, {"ELEVATE_HOME": "/custom/path"}):
            home = get_elevate_home()
            assert home == Path("/custom/path")


class TestEnsureElevateHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"ELEVATE_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ELEVATE_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ELEVATE_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"ELEVATE_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from elevate_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
