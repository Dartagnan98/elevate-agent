...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ELEVATE_HOME with .container-mode file."""
    elevate_home = tmp_path / ".elevate"
    elevate_home.mkdir()
    monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
    monkeypatch.delenv("ELEVATE_DEV", raising=False)

    container_mode = elevate_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=elevate\n"
        "exec_user=elevate\n"
        "elevate_bin=/data/current-package/bin/elevate\n"
    )
    return elevate_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("elevate_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "elevate"
    assert info["exec_user"] == "elevate"
    assert info["elevate_bin"] == "/data/current-package/bin/elevate"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("elevate_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_elevate_dev(container_env, monkeypatch):
    """Returns None when ELEVATE_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ELEVATE_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_elevate_dev_zero(container_env, monkeypatch):
    """ELEVATE_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ELEVATE_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        elevate_home = Path(tmpdir) / ".elevate"
        elevate_home.mkdir()
        (elevate_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("elevate_constants.is_container", return_value=False), \
             patch("elevate_cli.config.get_elevate_home", return_value=elevate_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ELEVATE_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=elevate-custom\n"
        "exec_user=myuser\n"
        "elevate_bin=/opt/elevate/bin/elevate\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "elevate-custom"
    assert info["exec_user"] == "myuser"
    assert info["elevate_bin"] == "/opt/elevate/bin/elevate"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from elevate_cli.main import _exec_in_container
