"""Tests for elevate_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("elevate_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from elevate_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("elevate_cli.config.load_config")
    ...
    @patch("elevate_cli.config.load_config")
    @patch.dict("os.environ", {"ELEVATE_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ELEVATE_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ELEVATE_SESSION_PLATFORM should be used when ELEVATE_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_elevate_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ELEVATE_PLATFORM should win over ELEVATE_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("ELEVATE_HOME", str(tmp_path))
        monkeypatch.setenv("ELEVATE_PLATFORM", "telegram")
        monkeypatch.setenv("ELEVATE_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ELEVATE_HOME", str(tmp_path))
        monkeypatch.setenv("ELEVATE_PLATFORM", "telegram")
        monkeypatch.setenv("ELEVATE_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from elevate_cli.skills_config import _get_categories
        skills = [
