"""Tests for the `elevate memory reset` CLI command.
...
- Profile-scoped reset (uses ELEVATE_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ELEVATE_HOME with memory files."""
    elevate_home = tmp_path / ".elevate"
    memories = elevate_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nElevate repo is at ~/.elevate/elevate\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `elevate memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        elevate_home = tmp_path / ".elevate"
        (elevate_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ELEVATE_HOME."""
        ...
        monkeypatch.setenv("ELEVATE_HOME", str(profile_home))
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))

        # The memories dir won't exist; get_elevate_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
