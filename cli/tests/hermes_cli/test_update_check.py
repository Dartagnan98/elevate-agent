"""Tests for the update check mechanism in elevate_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("elevate_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "elevate_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when ELEVATE_HOME has no git repo."""
    import elevate_cli.banner as banner
    ...
    # Point ELEVATE_HOME at a temp dir with no elevate/.git
    monkeypatch.setenv("ELEVATE_HOME", str(tmp_path))
    with patch("elevate_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from elevate_cli.main import _invalidate_update_cache

    # Build a fake ~/.elevate with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"ELEVATE_HOME": str(default_home)}):
