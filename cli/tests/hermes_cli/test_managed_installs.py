...
def test_get_managed_system_homebrew(monkeypatch):
    ...
    assert recommended_update_command() == "brew upgrade elevate"


def test_format_managed_message_homebrew(monkeypatch):
    monkeypatch.setenv("ELEVATE_MANAGED", "homebrew")

    message = format_managed_message("update Elevate")

    assert "managed by Homebrew" in message
    assert "brew upgrade elevate" in message
...
def test_cmd_update_blocks_managed_homebrew(monkeypatch, capsys):
    ...
    with patch("elevate_cli.main.subprocess.run") as mock_run:
        cmd_update(SimpleNamespace())

    assert not mock_run.called
    captured = capsys.readouterr()
    assert "managed by Homebrew" in captured.err
    assert "brew upgrade elevate" in captured.err


def test_optional_skill_source_honors_env_override(monkeypatch, tmp_path):
    optional_dir = tmp_path / "optional-skills"
    optional_dir.mkdir()
    monkeypatch.setenv("ELEVATE_OPTIONAL_SKILLS", str(optional_dir))
