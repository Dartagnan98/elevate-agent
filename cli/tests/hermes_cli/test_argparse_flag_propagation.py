...
subparser, placing the flag BEFORE the subcommand (e.g. 'elevate --yolo chat')
...
def _build_parser():
    """Build the elevate argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets ELEVATE_YOLO_MODE regardless of flag position.
    ...
    def test_yolo_before_chat_sets_env(self):
        ...
        assert os.environ.get("ELEVATE_YOLO_MODE") == "1"
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `elevate gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `elevate <argv>` must exit 0 (help) rather than
