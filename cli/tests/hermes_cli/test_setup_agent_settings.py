...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("elevate_cli.setup.get_env_value", lambda key: "60" if key == "ELEVATE_MAX_ITERATIONS" else "")
    monkeypatch.setattr("elevate_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("elevate_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
