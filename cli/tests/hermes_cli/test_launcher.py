"""Tests for the top-level `./elevate` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./elevate` should use `elevate_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "elevate"
    called = []

    fake_main_module = types.ModuleType("elevate_cli.main")

    def fake_main():
        called.append("elevate_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "elevate_cli.main", fake_main_module)
    ...
    assert called == ["elevate_cli.main"]
