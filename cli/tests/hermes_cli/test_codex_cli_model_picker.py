...
Covers:
 - Normal path (tokens already in Elevate auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Elevate
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `elevate auth openai-codex`. The old
...
@pytest.fixture()
def elevate_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Elevate auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(elevate_auth_only_env):
    """openai-codex appears when tokens are already in Elevate auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Elevate
    auth store."""
    elevate_home = tmp_path / ".elevate"
    elevate_home.mkdir()
