...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("ELEVATE_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("ELEVATE_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("ELEVATE_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_elevate_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: elevate gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup elevate gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.elevate.gateway"])
            ),
        )
...
class TestSystemUnitElevateHome:
    """ELEVATE_HOME in system units must reference the target user, not root."""

    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        # Simulate sudo: Path.home() returns /root, target user is alice
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("ELEVATE_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'ELEVATE_HOME=/home/alice/.elevate' in unit
        assert '/root/.elevate' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: ELEVATE_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ELEVATE_HOME", "/root/.elevate/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'ELEVATE_HOME=/home/alice/.elevate/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_elevate_home(self, monkeypatch):
        # Custom ELEVATE_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ELEVATE_HOME", "/opt/elevate-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'ELEVATE_HOME=/opt/elevate-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's ELEVATE_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        elevate_home = str(gateway_cli.get_elevate_home().resolve())
        assert f'ELEVATE_HOME={elevate_home}' in unit


class TestElevateHomeForTargetUser:
    """Unit tests for _elevate_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("ELEVATE_HOME", raising=False)

        result = gateway_cli._elevate_home_for_target_user("/home/alice")
        assert result == "/home/alice/.elevate"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._elevate_home_for_target_user("/home/alice")
        assert result == "/home/alice/.elevate/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ELEVATE_HOME", "/opt/elevate")

        result = gateway_cli._elevate_home_for_target_user("/home/alice")
        assert result == "/opt/elevate"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "elevate gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_elevate_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.elevate should not produce a --profile flag."""
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
        result = gateway_cli._profile_arg(str(elevate_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.elevate/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".elevate" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile ELEVATE_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "elevate"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.elevate/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".elevate" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".elevate" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ELEVATE_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_elevate_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".elevate" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.elevate.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".elevate" / "elevate"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".elevate" / "elevate")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/elevate", str(tmp_path / "alice"))
        assert result == "/opt/elevate"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".elevate" / "elevate")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".elevate" / "elevate"
        ...
        monkeypatch.setenv("ELEVATE_HOME", str(root_home / ".elevate"))
        monkeypatch.setattr(gateway_cli, "get_elevate_home", lambda: root_home / ".elevate")
        ...
        assert "WorkingDirectory=/home/alice/.elevate/elevate" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "elevate-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "elevate-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'elevate gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'elevate gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'elevate gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "elevate gateway run" in out


class TestLegacyElevateUnitDetection:
    """Tests for _find_legacy_elevate_units / has_legacy_elevate_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``elevate.service`` unit behind when the service was
    renamed to ``elevate-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``elevate.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``elevate-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Elevate Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m elevate_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_elevate_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "elevate.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_elevate_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "elevate.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_elevate_units() is True

    def test_detects_legacy_elevate_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "elevate.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_elevate_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``elevate-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "elevate-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "elevate-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "elevate-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_elevate_units() is False

    def test_ignores_unrelated_elevate_service(self, tmp_path, monkeypatch):
        """Third-party ``elevate.service`` that isn't ours stays untouched.

        If a user has some other package named ``elevate`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "elevate.service").write_text(
            "[Unit]\nDescription=Some Other Elevate\n[Service]\n"
            "ExecStart=/opt/other-elevate/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_elevate_units() == []
        assert gateway_cli.has_legacy_elevate_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "elevate.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "elevate.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m elevate_cli.main gateway run
          - python path/to/elevate_cli/main.py gateway run
          - elevate gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m elevate_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/elevate/elevate_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/elevate gateway run --replace",
            "ExecStart=/venv/bin/python /opt/elevate/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"elevate.service" if i == 0 else f"elevate.service"  # same name
            # Test each variant fresh
            (user_dir / "elevate.service").write_text(
                f"[Unit]\nDescription=Old Elevate\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_elevate_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "elevate.service" in out
        assert "elevate gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "elevate.service"
...
class TestRemoveLegacyElevateUnits:
    """Tests for remove_legacy_elevate_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_elevate_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_elevate_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop elevate.service" in c for c in cmds_joined)
        assert any("--user disable elevate.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo elevate gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop elevate.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable elevate.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "elevate.service"
        system_legacy = system_dir / "elevate.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (elevate-gateway-coder.service)
        ...
        profile_unit = user_dir / "elevate-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "elevate-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_elevate_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `elevate gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import elevate_cli.main as cli_main
        ...
        # public, just confirm that `elevate gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import elevate_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "elevate_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_elevate_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "elevate_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_elevate_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_elevate_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_elevate_units", fake_remove)
