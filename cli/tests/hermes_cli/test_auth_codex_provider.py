"""Tests for Codex auth — tokens stored in Elevate auth store (~/.elevate/auth.json)."""
...
def _setup_elevate_auth(elevate_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Elevate auth store."""
    ...
    auth_file = elevate_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_elevate_auth(elevate_home)
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    elevate_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (elevate_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    elevate_home = tmp_path / "elevate"
    _setup_elevate_auth(elevate_home, access_token="")
    monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_elevate_auth(elevate_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
    ...
    monkeypatch.setattr("elevate_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    elevate_home = tmp_path / "elevate"
    _setup_elevate_auth(elevate_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Elevate auth store, not ~/.codex/."""
    elevate_home = tmp_path / "elevate"
    codex_home = tmp_path / "codex-cli"
    elevate_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "elevate-at", "refresh_token": "elevate-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Elevate store
    assert not (codex_home / "auth.json").exists()

    # Elevate auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "elevate-at"


def test_resolve_returns_elevate_auth_store_source(tmp_path, monkeypatch):
    elevate_home = tmp_path / "elevate"
    _setup_elevate_auth(elevate_home)
    monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "elevate-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("elevate_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "elevate_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "elevate_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("elevate_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("elevate_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
