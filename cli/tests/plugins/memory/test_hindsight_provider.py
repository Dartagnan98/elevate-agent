...
def provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_elevate_home", lambda: tmp_path
    )

    p = HindsightMemoryProvider()
    p.initialize(session_id="test-session", elevate_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:elevate, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:elevate",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:elevate"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:elevate"]
...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Elevate and the User"

    def test_custom_config_values(self, provider_with_config):
        p = provider_with_config(
            retain_tags=["tag1", "tag2"],
            retain_source="elevate",
        ...
        assert p._retain_source == "elevate"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_elevate_home",
        ...
        assert cfg["banks"]["elevate"]["bankId"] == "env-bank"
        assert cfg["banks"]["elevate"]["budget"] == "high"
...
class TestPostSetup:
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("elevate_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("elevate_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(elevate_home), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        assert (elevate_home / ".env").read_text() == "HINDSIGHT_LLM_API_KEY=sk-local-test\nHINDSIGHT_TIMEOUT=120\n"

        profile_env = user_home / ".hindsight" / "profiles" / "elevate.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(elevate_home))
        provider.post_setup(str(elevate_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        elevate_env = user_home / ".hindsight" / "profiles" / "elevate.env"
        assert coder_env.exists()
        assert not elevate_env.exists()
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["context"] == "conversation between Elevate and the User"
        ...
        assert item["metadata"]["source"] == "elevate"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Elevate and the User"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_elevate_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", elevate_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", elevate_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("elevate") == "elevate"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "elevate-{user}" becomes "elevate-" -> trimmed to "elevate"
        result = _resolve_bank_id_template(
            "elevate-{user}", fallback="default",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "elevate-{unknown}", fallback="elevate",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "elevate"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "elevate-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="elevate",
        )
        assert p._bank_id == "elevate-coder"
        assert p._bank_id_template == "elevate-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "elevate-" which collapses to "elevate"
        p.initialize(session_id="s1", elevate_home=str(tmp_path), platform="cli")
        assert p._bank_id == "elevate"
