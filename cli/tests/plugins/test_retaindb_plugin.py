...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure ELEVATE_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", elevate_home=str(tmp_path / ".elevate"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", elevate_home=profile_home)
        assert p._client.project == "elevate-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".elevate" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        elevate_home = tmp_path / ".elevate"
        elevate_home.mkdir(exist_ok=True)
        monkeypatch.setenv("ELEVATE_HOME", str(elevate_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", elevate_home=str(elevate_home))
        return p
