...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "elevate"
        assert config.workspace_id == "elevate"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.elevate/honcho.json
        monkeypatch.setenv("ELEVATE_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"elevate": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"elevate": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"elevate": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="elevate"
        ):
            result = config.resolve_session_name("/home/user/elevate/subdir")
        assert result == "elevate"
...
class TestResolveConfigPath:
    def test_prefers_elevate_home_when_exists(self, tmp_path):
        elevate_home = tmp_path / "elevate"
        elevate_home.mkdir()
        local_cfg = elevate_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"ELEVATE_HOME": str(elevate_home)}):
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        elevate_home = tmp_path / "elevate"
        elevate_home.mkdir()
        # No honcho.json in ELEVATE_HOME — also isolate ~/.elevate so
    ...
    def test_falls_back_to_global_without_elevate_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
...
class TestResolveActiveHost:
    def test_default_returns_elevate(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ELEVATE_HONCHO_HOST", None)
            os.environ.pop("ELEVATE_HOME", None)
            assert resolve_active_host() == "elevate"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ELEVATE_HONCHO_HOST": "elevate.coder"}):
            assert resolve_active_host() == "elevate.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ELEVATE_HONCHO_HOST", None)
            with patch("elevate_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "elevate.coder"

    def test_default_profile_returns_elevate(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ELEVATE_HONCHO_HOST", None)
            with patch("elevate_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "elevate"

    def test_custom_profile_returns_elevate(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ELEVATE_HONCHO_HOST", None)
            with patch("elevate_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "elevate"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ELEVATE_HONCHO_HOST", None)
            # Temporarily remove elevate_cli.profiles to simulate import failure
            saved = sys.modules.get("elevate_cli.profiles")
            sys.modules["elevate_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "elevate"
            finally:
                if saved is not None:
                    sys.modules["elevate_cli.profiles"] = saved
                else:
                    sys.modules.pop("elevate_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="elevate.coder")
        assert config.host == "elevate.coder"
        assert config.workspace_id == "elevate"  # shared workspace
        assert config.ai_peer == "elevate.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="elevate")
        assert config.host == "elevate"
        assert config.workspace_id == "elevate"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "elevate": {"aiPeer": "elevate", "peerName": "alice"},
                "elevate.coder": {
                    "aiPeer": "elevate.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="elevate.coder", config_path=config_file,
        )
        assert config.host == "elevate.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "elevate.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="elevate.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "elevate.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"elevate": {"enabled": True, "aiPeer": "elevate"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"elevate": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_elevate_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="elevate",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("elevate_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_elevate_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="elevate",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("elevate_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"elevate": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
