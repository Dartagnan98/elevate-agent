"""
Tests for mcp_serve — Elevate MCP server.
...
def _isolate_elevate_home(tmp_path, monkeypatch):
    """Redirect ELEVATE_HOME to a temp directory."""
    monkeypatch.setenv("ELEVATE_HOME", str(tmp_path))
    try:
        import elevate_constants
        monkeypatch.setattr(elevate_constants, "get_elevate_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking elevate_state schema."""
