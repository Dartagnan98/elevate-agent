"""Shared fixtures for the elevate test suite.
...
2. **Isolated ELEVATE_HOME.** ELEVATE_HOME points to a per-test tempdir so
   code reading ``~/.elevate/*`` via ``get_elevate_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".elevate"`` instead of the canonical
   ``get_elevate_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ELEVATE_SESSION_* inheritance** — the agent's current gateway
...
# ELEVATE_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ELEVATE_BEHAVIORAL_VARS = frozenset({
    "ELEVATE_YOLO_MODE",
    "ELEVATE_INTERACTIVE",
    "ELEVATE_QUIET",
    ...
    "ELEVATE_SESSION_CHAT_ID",
    "ELEVATE_SESSION_CHAT_NAME",
    "ELEVATE_SESSION_THREAD_ID",
    "ELEVATE_SESSION_SOURCE",
    "ELEVATE_SESSION_KEY",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ELEVATE_HOME to per-test tempdirs so code that
    reads ``~/.elevate/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ELEVATE_* vars that could change test semantics.
    ...
    #    fixture. Any code in the codebase reading ``~/.elevate/*`` via
    #    ``Path.home() / ".elevate"`` instead of ``get_elevate_home()``
    #    is a bug to fix at the callsite.
    fake_elevate_home = tmp_path / "elevate_test"
    fake_elevate_home.mkdir()
    (fake_elevate_home / "sessions").mkdir()
    (fake_elevate_home / "cron").mkdir()
    ...
    # opts into PG reads. Production keeps the module default.
    ...
    #    ~/.elevate/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import elevate_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_elevate_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal elevate config dict suitable for unit tests."""
    return {
