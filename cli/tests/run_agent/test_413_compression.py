...
class TestPreflightCompression:
    ...
    def test_preflight_defers_when_recent_real_usage_fit(self, agent):
        ...
        This is the post-compaction re-compaction loop fix (ported from Hermes
        e38b0b55d): once a compressed request actually fit at the provider, the
        schema-inflated rough estimate must not re-trigger the ~89s compaction.
        """
        agent.compression_enabled = True
        ...
        with (
            patch("run_agent.estimate_request_tokens_rough", return_value=114_000),
            patch.object(agent, "_compress_context") as mock_compress,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
    ...
    def test_preflight_compresses_when_rough_growth_after_fit_is_large(self, agent):
        ...
        # First rough estimate (preflight) is well above the fit baseline so the
        # defer guard does NOT engage; every estimate after the compaction is
        # low so the multi-pass loop exits. Function form is robust to however
        # many times the loop estimates (elevate has an extra main-loop call).
        _est_calls = {"n": 0}
        def _est(*_a, **_k):
            _est_calls["n"] += 1
            return 125_000 if _est_calls["n"] == 1 else 40_000

        with (
            patch("run_agent.estimate_request_tokens_rough", side_effect=_est),
            patch.object(agent, "_compress_context") as mock_compress,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
