"""
Tests for timezone support (elevate_time module + integration points).
...
def _reset_elevate_time_cache():
    """Reset the elevate_time module cache (replacement for removed reset_cache)."""
    elevate_time._cached_tz = None
    elevate_time._cached_tz_name = None
    elevate_time._cache_resolved = False


# =========================================================================
# elevate_time.now() — core helper
# =========================================================================

class TestElevateTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_elevate_time_cache()

    def teardown_method(self):
        _reset_elevate_time_cache()
        os.environ.pop("ELEVATE_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["ELEVATE_TIMEZONE"] = "Asia/Kolkata"
        result = elevate_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["ELEVATE_TIMEZONE"] = "UTC"
        result = elevate_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["ELEVATE_TIMEZONE"] = "America/New_York"
        result = elevate_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ELEVATE_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="elevate_time"):
            result = elevate_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ELEVATE_TIMEZONE"] = "Europe/London"
        tz = elevate_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ELEVATE_TIMEZONE"] = "Not/A/Timezone"
        tz = elevate_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ELEVATE_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ELEVATE_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ELEVATE_TIMEZONE=" + os.environ.get("ELEVATE_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ELEVATE_TIMEZONE=NOT_SET" in result["output"], (
            "ELEVATE_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ELEVATE_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ELEVATE_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=elevate_tz) which shifted
        absolute time when system-local tz != Elevate tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_elevate_tz(self):
        """Already-aware datetimes should be normalized to Elevate tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Elevate tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Elevate tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Elevate is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Elevate tz
        ...
        # Use a Elevate timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _elevate_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ELEVATE_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_elevate_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Elevate timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ELEVATE_TIMEZONE"] = "US/Eastern"
        _reset_elevate_time_cache()
