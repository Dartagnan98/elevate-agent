...
class TestToolsetConsistency:
    ...
    def test_elevate_platforms_share_core_tools(self):
        """All elevate-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord_server`` on
        elevate-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["elevate-cli", "elevate-telegram", "elevate-discord", "elevate-whatsapp", "elevate-slack", "elevate-signal", "elevate-homeassistant"]
