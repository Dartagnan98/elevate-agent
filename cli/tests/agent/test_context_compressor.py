...
class TestPreflightDeferral:
    """The post-compaction re-compaction loop fix: once a compressed request
    has actually fit at the provider, don't re-fire compaction off the
    schema-inflated rough estimate (ported from Hermes e38b0b55d)."""
...
class TestNonStringContent:
    ...
    def test_none_content_coerced_to_empty(self):
        ...
        assert "empty response" in c._last_summary_error
...
class TestSummaryFailureCooldown:
    def test_length_summary_never_replaces_or_updates_context(self):
        with patch(
        ...
        response.choices[0].message.content = "CUT_SUMMARY_MUST_NOT_PERSIST"
        messages = [
            {"role": "user", "content": "do something"},
        ...
        assert "incomplete response (length)" in c._last_summary_error
    ...
    def test_rejected_response_is_not_mislabeled_as_missing_provider(self):
        ...
        rejection = AuxiliaryResponseRejectedError(
            "Gemini response rejected (SAFETY)"
        )
        ...
        assert "SAFETY" in c._last_summary_error
        assert c._summary_failure_cooldown_until < 1600.0

    def test_malformed_response_is_not_mislabeled_as_missing_provider(self):
        from agent.auxiliary_client import AuxiliaryResponseValidationError
        ...
        validation_error = AuxiliaryResponseValidationError(
        ...
        assert summary is None
        assert c._last_summary_error != "no auxiliary LLM provider configured"
        assert "invalid response" in c._last_summary_error
        assert c._summary_failure_cooldown_until < 1600.0
...
class TestSummaryFallbackToMainModel:
    ...
    def test_model_not_found_404_falls_back_to_main_and_succeeds(self):
        ...
        mock_ok.choices[0].finish_reason = "stop"
...
class TestSummaryTargetRatio:
    ...
    def test_default_threshold_is_85_percent(self):
        ...
        # 85% of 70K = 59.5K, but the floor is 64K
    ...
    def test_threshold_floor_does_not_apply_above_floor(self):
        ...
        with patch("agent.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        # 85% of 200K = 170K, which is above the 64K floor
        assert c.threshold_tokens == 170_000

    def test_should_prune_only_band_and_rate_limit(self):
        """prune_only fires between 72% of context and the full threshold,
        and is rate-limited until the prompt regrows 5% of the context."""
        with patch("agent.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        # threshold = 170K, soft bar = 144K
        assert not c.should_prune_only(0)
        assert not c.should_prune_only(140_000)   # below soft bar
        assert c.should_prune_only(150_000)       # in band
        assert not c.should_prune_only(175_000)   # full-compress territory
        c._last_prune_attempt_tokens = 150_000
        assert not c.should_prune_only(151_000)   # rate limited
        assert c.should_prune_only(161_000)       # regrew 5% (10K)
...
class TestTruncateToolCallArgsJson:
    ...
    def test_shrunken_args_remain_valid_json(self):
        ...
        original = _json.dumps({
            "path": "~/.elevate/skills/shopping/browser-setup-notes.md",
