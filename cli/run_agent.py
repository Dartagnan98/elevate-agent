...
_loaded_env_paths = load_elevate_dotenv(elevate_home=_elevate_home, project_env=_project_env)
...
EMPTY_RESPONSE_FAILURE_MESSAGE = (
...
class _SafeWriter:
    ...
    When elevate runs as a systemd service, Docker container, or headless
...
class IterationBudget:
    ...
    The parent's budget is capped at ``max_iterations`` (default 150 from the Elevate CLI).
...
class AIAgent:
    ...
    def __init__(
        ...
        self._cache_ttl = "5m"
        try:
            from elevate_cli.config import load_config as _load_pc_cfg
        ...
        setup_logging(elevate_home=_elevate_home)
        ...
        try:
            from elevate_cli.config import load_config as _load_agent_config
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from elevate_cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Elevate.  Choose a model with at least "
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``elevate chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ELEVATE_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ELEVATE_API_TIMEOUT`` fallback would always be
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ELEVATE_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("ELEVATE_API_CALL_STALE_TIMEOUT")
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("ELEVATE_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the elevate install
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from elevate_cli.auth import resolve_nous_runtime_credentials
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from elevate_cli.copilot_auth import resolve_copilot_token
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            _base_timeout = (
                ...
                else float(os.getenv("ELEVATE_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # ELEVATE_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                _stream_read_timeout = _provider_timeout_cfg
            else:
                _stream_read_timeout = float(os.getenv("ELEVATE_STREAM_READ_TIMEOUT", 120.0))
                # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
                # prefill on large contexts before producing the first token.
                # Auto-increase the httpx read timeout unless the user explicitly
                # overrode ELEVATE_STREAM_READ_TIMEOUT.
            ...
            return SimpleNamespace(
                ...
                choices=[mock_choice],
                usage=usage_obj,
                _elevate_stream_incomplete_error=(
                    None
        ...
        _stream_stale_timeout_base = float(os.getenv("ELEVATE_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set ELEVATE_STREAM_STALE_TIMEOUT.
            ...
            if deltas_were_sent["yes"]:
                ...
                return SimpleNamespace(
                    ...
                    choices=[SimpleNamespace(
                        index=0, message=_stub_msg, finish_reason="incomplete",
                    )],
                    usage=None,
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from elevate_cli.models import github_model_reasoning_efforts
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        # Keep the caller's list identity intact so the terminal assistant
        # summary is persisted by the outer conversation loop.
        messages[:] = self._externalize_inline_media_messages(messages)
    ...
    def run_conversation(
        ...
        # are reused.  The system prompt is Elevate's territory; plugins
        ...
        def _terminal_result(
            ...
            *,
            ...
            if result_messages is not None:
                returned_messages = list(result_messages)
                if not (
                    returned_messages
                    and returned_messages[-1].get("role") == "assistant"
                    and returned_messages[-1].get("content") == text
                ):
                    returned_messages.append(dict(terminal_message))
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for Elevate internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if env_var_enabled("ELEVATE_DUMP_REQUESTS"):
                    ...
                    if response_invalid:
                        ...
                        if retry_count >= max_retries:
                            ...
                            _invalid_error = (
                                f"Invalid API response after {max_retries} "
                                f"retries: {_failure_hint}"
                            )
                            return _terminal_result(
                                _invalid_error, error=_invalid_error
                            )
                    ...
                    if self.api_mode == "codex_responses":
                        ...
                        _stream_incomplete_error = getattr(
                    ...
                    if finish_reason == "length":
                        ...
                        if self.api_mode in ("chat_completions", "bedrock_converse", "anthropic_messages"):
                            ...
                            if assistant_message is not None and not _trunc_has_tool_calls:
                                ...
                                return _terminal_result(
                                    partial_response or _truncation_error,
                        ...
                        if len(messages) > 1:
                            ...
                            return _terminal_result(
                                ...
                                error=_truncation_error,
                                partial=True,
                                result_messages=rolled_back_messages,
                            )
                        else:
                            ...
                            return _terminal_result(
                                _truncation_error, error=_truncation_error
                            )
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: elevate login --provider nous")
                    ...
                    if (
                        ...
                        from elevate_constants import display_elevate_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Elevate-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: elevate config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: elevate config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if self._interrupt_requested:
                        ...
                        _interrupt_text = (
                            ...
                            f"({error_type}: "
                            f"{self._clean_error_message(str(api_error))})."
                        )
                        return _terminal_result(
                            _interrupt_text,
                            error=_interrupt_text,
                    ...
                    if is_payload_too_large:
                        ...
                        if compression_attempts > max_compression_attempts:
                            ...
                            _compression_error = (
                                "Request payload too large: max compression "
                    ...
                    if is_context_length_error:
                        ...
                        if available_out is not None:
                            ...
                            if compression_attempts > max_compression_attempts:
                                ...
                                _compression_error = (
                                    "Context length exceeded: max compression "
                                    f"attempts ({max_compression_attempts}) reached."
                                )
                                return _terminal_result(
                                    _compression_error,
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `elevate auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: elevate setup", force=True)
                        ...
                        _client_error = str(api_error)
                        return _terminal_result(
                            _client_error, error=_client_error
                        )
                    ...
                    if retry_count >= max_retries:
                        ...
                        return _terminal_result(
                            _final_response, error=_final_summary
                        )
                    ...
                    while time.time() < sleep_end:
                        if self._interrupt_requested:
                            self._vprint(f"{self.log_prefix}⚡ Interrupt detected during retry wait, aborting.", force=True)
                            _interrupt_text = (
                                "Operation interrupted: retrying API call after "
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if self._has_content_after_think_block(_partial_streamed):
                            ...
                            _drop_trailing_empty_response_scaffolding(messages)
                            recovered_msg = self._build_assistant_message(
                                assistant_message, "error"
                            )
                            recovered_msg["content"] = _recovered
                            recovered_msg["finish_reason"] = "error"
                        ...
                        _drop_trailing_empty_response_scaffolding(messages)
                        assistant_msg = self._build_assistant_message(assistant_message, finish_reason)
                        assistant_msg["content"] = EMPTY_RESPONSE_FAILURE_MESSAGE
                        ...
                        final_response = EMPTY_RESPONSE_FAILURE_MESSAGE
                        break
