...
Backend is selected during ``/tools`` setup (web.backend in config.yaml).
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``/tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `/tools` repaint.
        try:
