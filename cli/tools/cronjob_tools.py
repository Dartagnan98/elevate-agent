"""
Cron job management tools for Elevate.
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via elevate model.
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under {display_elevate_home()}/scripts/. On update, pass empty string to clear."
            },
            "enabled_toolsets": {
                "type": "array",
                "items": {"type": "string"},
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("ELEVATE_INTERACTIVE")
        or os.getenv("ELEVATE_GATEWAY_SESSION")
        or os.getenv("ELEVATE_EXEC_ASK")
    )
