...
Architecture:
    ~/.elevate/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        ELEVATE_WORKDIR                               — original dir path
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal Elevate infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "elevate@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Elevate Checkpoint"], shadow_repo, working_dir)
