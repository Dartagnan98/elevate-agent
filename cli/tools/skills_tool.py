...
# All skills live in ~/.elevate/skills/ (seeded from bundled skills/ on install).
...
def _is_gateway_surface() -> bool:
    if os.getenv("ELEVATE_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("ELEVATE_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_elevate_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.elevate/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ELEVATE_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ELEVATE_PLATFORM`` environment variable
    3. ``ELEVATE_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from elevate_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ELEVATE_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``elevate skills`` config UI). Default False
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: elevate plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.elevate/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `elevate skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.elevate.* first (agentskills.io convention), fall back to top-level
        elevate_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            elevate_meta = metadata.get("elevate", {}) or {}

        tags = _parse_tags(elevate_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            elevate_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
