...
# display_elevate_home imported lazily at call site (stale-module safety during hermes update)
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("ELEVATE_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ELEVATE_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ELEVATE_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (ELEVATE_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and env_var_enabled("ELEVATE_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.elevate/.env or environment variables."
        )


def _safe_getcwd() -> str:
    """Return the process cwd, resilient to it having been deleted under us.

    The Elevate gateway is launched with its cwd inside the app bundle
    (``/Applications/Elevate.app/Contents/Resources/cli``). A desktop
    auto-update (Squirrel/ShipIt) relocates that bundle, leaving the
    long-lived gateway process holding a dangling cwd. ``os.getcwd()`` then
    raises ``FileNotFoundError`` (errno 2), which crashed the terminal
    tool's requirements check and silently disabled ALL shell access for
    the session (no PDFs, scripts, or file work) until the gateway was
    restarted. Fall back to ``TERMINAL_CWD``, then the user's home, then
    ``/`` so the terminal tool stays usable.
    """
    try:
        return os.getcwd()
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("ELEVATE_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("ELEVATE_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("ELEVATE_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("ELEVATE_SESSION_USER_ID", "")
                        _gw_user_name = _gse("ELEVATE_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("ELEVATE_SESSION_MESSAGE_ID", "")
...
if __name__ == "__main__":
    ...
    print(f"  TERMINAL_CWD: {os.getenv('TERMINAL_CWD') or _safe_getcwd()}")
    from elevate_constants import display_elevate_home as _dhh
