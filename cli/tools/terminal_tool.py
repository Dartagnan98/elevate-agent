...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Elevate exit
...
# display_elevate_home imported lazily at call site (stale-module safety during elevate update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of elevate directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "elevate-*")):
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    if not has_configured_password and not sudo_password and os.getenv("ELEVATE_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Elevate can track lifecycle and output.
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Modal credentials/config were found. Log in with `elevate model` or "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(command):
        return (
            ...
            "Use terminal(background=true) so Elevate can track the process, then run "
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paid Nous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `elevate model` "
