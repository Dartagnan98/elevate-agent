...
_ELEVATE_PROVIDER_ENV_FORCE_PREFIX = "_ELEVATE_FORCE_"
...
_ELEVATE_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Hermes home override into subprocess env."""
    try:
        from elevate_constants import get_elevate_home_override

        value = get_elevate_home_override()
        if value:
            env["ELEVATE_HOME"] = value
    except Exception:
        pass
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ELEVATE_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _ELEVATE_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_ELEVATE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ELEVATE_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _ELEVATE_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ELEVATE_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        ...
        "Or set ELEVATE_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ELEVATE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ELEVATE_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _ELEVATE_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # npm …) into {ELEVATE_HOME}/home/ when that directory exists.  Only the
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ELEVATE_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ELEVATE_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_elevate_home() / "cache" / "terminal"
            except Exception:
