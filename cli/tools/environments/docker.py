...
from tools.environments.local import _ELEVATE_PROVIDER_ENV_BLOCKLIST
...
def _load_elevate_env_vars() -> dict[str, str]:
    """Load ~/.elevate/.env values without failing Docker command execution."""
    try:
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ELEVATE_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ELEVATE_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ELEVATE_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'elevate'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.elevate/sandboxes/). Non-persistent
        ...
        container_name = f"elevate-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Elevate secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ELEVATE_PROVIDER_ENV_BLOCKLIST)
        elevate_env = _load_elevate_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = elevate_env.get(key)
