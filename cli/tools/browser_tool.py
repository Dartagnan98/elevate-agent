...
# plugins drop in under ``~/.elevate/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.elevate/plugins/browser/<vendor>/``) participate
...
def _find_agent_browser() -> str:
    ...
    try:
        from elevate_cli.dep_ensure import ensure_dependency
        if ensure_dependency("browser"):
            ...
            if not recheck:
                hermes_nm = str(get_elevate_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=hermes_nm)
            if not recheck:
                hermes_node_bin = str(get_elevate_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=hermes_node_bin)
            if not recheck:
                hermes_node_root = str(get_elevate_home() / "node")
