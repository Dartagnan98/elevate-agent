...
# Path to tinker-atropos submodule (relative to elevate root)
ELEVATE_ROOT = Path(__file__).parent.parent
TINKER_ATROPOS_ROOT = ELEVATE_ROOT / "tinker-atropos"
ENVIRONMENTS_DIR = TINKER_ATROPOS_ROOT / "tinker_atropos" / "environments"
CONFIGS_DIR = TINKER_ATROPOS_ROOT / "configs"
LOGS_DIR = get_elevate_home() / "logs" / "rl_training"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.elevate/.env",
        }, indent=2)
...
# Schemas + Registry
# ---------------------------------------------------------------------------
from tools.registry import registry  # noqa: E402 - deferred until after tool definitions
...
registry.register(name="rl_get_current_config", emoji="🧪", toolset="rl", schema=RL_GET_CURRENT_CONFIG_SCHEMA,
    handler=lambda args, **kw: rl_get_current_config(), check_fn=check_rl_python_version, requires_env=[],
    is_async=True, effects={"read:rl"})
