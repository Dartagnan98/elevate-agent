...
def _resolve_elevate_home() -> Path:
...
def register_credential_file(
    ...
    *relative_path* is relative to ``ELEVATE_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ELEVATE_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    elevate_home = _resolve_elevate_home()

    # Reject absolute paths — they bypass the ELEVATE_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ELEVATE_HOME)",
            relative_path,
        )
        return False

    host_path = elevate_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape ELEVATE_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, elevate_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.elevate/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="elevate-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching elevate_constants.get_elevate_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_elevate_dir()`` for backward compatibility with old directory layouts.
    """
    from elevate_constants import get_elevate_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_elevate_dir(new_subpath, old_name)
        if host_dir.is_dir():
