    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "ELEVATE_HOME", os.path.join(os.path.expanduser("~"), ".hermes")
                )
            )
            candidates = [
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    try:
        ...
        try:
            from elevate_cli.env_loader import load_hermes_dotenv
            load_hermes_dotenv()
        except Exception:
            pass
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``/tools`` interactive configuration — connects to each
