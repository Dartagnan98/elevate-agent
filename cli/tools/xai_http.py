...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``elevate tools`` repaint, tool-registration scans,
    ...
    2. ``~/.elevate/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_elevate_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.elevate/.env`` first, then ``os.environ``.

    Wraps :func:`elevate_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from elevate_cli.config import get_env_value as _elevate_get_env_value

        value = _elevate_get_env_value(name)
...
def elevate_xai_user_agent() -> str:
    ...
    try:
        ...
        __version__ = "unknown"
    return f"Elevate-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Elevate-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``elevate_cli.config.get_env_value`` so keys
    stored in ``~/.elevate/.env`` (the standard Elevate location) are honored —
