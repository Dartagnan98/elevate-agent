...
This module provides the glue:
    - ``ElevateTokenStorage``: persists tokens/client-info to disk so they
...
def _get_token_dir() -> Path:
    ...
    Uses ELEVATE_HOME so each profile gets its own OAuth tokens.
    Layout: ``ELEVATE_HOME/mcp-tokens/``
    """
    try:
        from elevate_constants import get_elevate_home
        base = Path(get_elevate_home())
    except ImportError:
        base = Path(os.environ.get("ELEVATE_HOME", str(Path.home() / ".elevate")))
...
# ---------------------------------------------------------------------------
# ElevateTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ElevateTokenStorage:
