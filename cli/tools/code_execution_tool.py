...
Lets the LLM write a Python script that calls /tools via RPC,
...
def _connect():
    ...
    ELEVATE_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ELEVATE_RPC_SOCKET"]
...
"""Auto-generated /tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ELEVATE_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def execute_code(
    ...
    # selector from ELEVATE_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  ELEVATE_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ELEVATE_RPC_SOCKET"] = rpc_endpoint
        ...
        # ELEVATE_TIMEZONE is an internal Hermes setting and must not leak
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``/tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call /tools programmatically. "
