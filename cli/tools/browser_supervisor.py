...
One ``CDPSupervisor`` runs per Elevate ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "elevate-dialog-bridge.invalid"
...
(() => {
  if (window.__elevateDialogBridgeInstalled) return;
  window.__elevateDialogBridgeInstalled = true;
  const ENDPOINT = "http://elevate-dialog-bridge.invalid/";
