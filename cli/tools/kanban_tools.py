...
running under the dispatcher (env var ``ELEVATE_KANBAN_TASK`` set) or when
...
def _check_kanban_mode() -> bool:
    ...
    1. ``ELEVATE_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    ...
    """
    if os.environ.get("ELEVATE_KANBAN_TASK"):
        return True
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("ELEVATE_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("ELEVATE_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("ELEVATE_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``ELEVATE_KANBAN_TASK`` set
    ...
    ``ELEVATE_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            claim_lock = os.environ.get("ELEVATE_KANBAN_CLAIM_LOCK")
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("ELEVATE_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets ELEVATE_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("ELEVATE_SESSION_ID")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("ELEVATE_PROFILE") or "worker",
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to ELEVATE_KANBAN_TASK from the env "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to ELEVATE_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
