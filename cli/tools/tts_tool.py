...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.elevate/config.yaml``. Hermes
...
Configuration is loaded from ~/.elevate/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``elevate_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from elevate_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import elevate_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_elevate_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.elevate/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.elevate/config.yaml.
    ...
    try:
        ...
        logger.debug("elevate_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ELEVATE_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": elevate_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.elevate/cache/piper-voices/`` under the active
    ELEVATE_HOME so voice downloads follow profile boundaries.
    """
    from elevate_constants import get_elevate_dir
    root = Path(get_elevate_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.elevate/config.yaml (tts: section).
    ...
    platform = get_session_env("ELEVATE_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_elevate_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
