...
it is automatically downloaded from GitHub releases to $ELEVATE_HOME/bin/tirith.
...
def _get_elevate_home() -> str:
    """Return the Elevate home directory, respecting ELEVATE_HOME env var."""
    return str(get_elevate_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_elevate_home(), ".tirith-install-failed")
...
def _elevate_bin_dir() -> str:
    """Return $ELEVATE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_elevate_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ELEVATE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_elevate_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ELEVATE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ELEVATE_HOME/bin/tirith
    ...
    elevate_bin = os.path.join(_elevate_bin_dir(), "tirith")
    if os.path.isfile(elevate_bin) and os.access(elevate_bin, os.X_OK):
        _resolved_path = elevate_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return elevate_bin
