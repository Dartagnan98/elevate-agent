"""
Session-scoped context variables for the Elevate gateway.

Replaces the previous ``os.environ``-based session state
(``ELEVATE_SESSION_PLATFORM``, ``ELEVATE_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["ELEVATE_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("ELEVATE_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("ELEVATE_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("ELEVATE_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("ELEVATE_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("ELEVATE_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("ELEVATE_SESSION_THREAD_ID", default=_UNSET)
...
_SESSION_ID: ContextVar = ContextVar("ELEVATE_SESSION_ID", default=_UNSET)
...
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("ELEVATE_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("ELEVATE_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("ELEVATE_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "ELEVATE_SESSION_PLATFORM": _SESSION_PLATFORM,
    "ELEVATE_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "ELEVATE_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "ELEVATE_SESSION_THREAD_ID": _SESSION_THREAD_ID,
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``ELEVATE_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("ELEVATE_SESSION_*", default)``.
