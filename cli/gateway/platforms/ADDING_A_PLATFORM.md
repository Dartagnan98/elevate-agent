...
Checklist for integrating a new messaging platform into the Elevate gateway.
...
And add it to the `elevate-gateway` composite:

```python
"elevate-gateway": {
    "includes": [..., "elevate-your-platform"]
}
```
...
## 12. Status Display (`elevate_cli/status.py`)
...
## 13. Gateway Setup Wizard (`elevate_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ elevate_cli/ toolsets.py \
  --include="*.py" -l | sort -u
