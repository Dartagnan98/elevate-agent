...
class WhatsAppAdapter(BasePlatformAdapter):
    ...
    DEFAULT_REPLY_PREFIX = "⚕ *Elevate Agent*\n────────────\n"
    
    # Default bridge location relative to the elevate-agent install
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_elevate_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `elevate gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run elevate whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            ...
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `elevate whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: elevate whatsapp")
