...
AnythingLLM, NextChat, ChatBox, etc.) can connect to elevate
...
def _derive_chat_session_id(
    ...
    the same Elevate session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through elevate's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``elevate sessions list``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "elevate"})
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        provided_session_id = request.headers.get("X-Elevate-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Elevate-Session-Id rejected: "
            ...
            # conversation map to the same Elevate session.  The first user
        ...
        if stream:
            ...
            def _on_tool_progress(event_type, name, preview, args, **kwargs):
                ...
                ``event: elevate.tool.progress`` line that compliant frontends
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: elevate.tool.progress`` SSE event so
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from elevate_cli.auth import has_usable_secret
