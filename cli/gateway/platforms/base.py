...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through the DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Gateway-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.elevate/.env manually."
)
...
# Default location: {ELEVATE_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_elevate_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; ElevateAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_elevate_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_elevate_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_elevate_dir("cache/documents", "document_cache")
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?elevate\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+elevate[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def delete_message(
        ...
        a fresh message so the platform's visible timestamp reflects
        completion time.
    ...
    def _get_ephemeral_system_ttl_default(self) -> int:
        ...
        """
        try:
            from elevate_cli.config import load_config as _load_config
        except Exception:
            return 0
        try:
            cfg = _load_config()
    ...
    async def send_slash_confirm(
        ...
        Used by the gateway's generic slash-confirm primitive for commands
        that have a non-destructive but expensive side effect the user should
        explicitly acknowledge — e.g. ``/reload-mcp``, which invalidates the
    ...
    async def send_clarify(
        ...
        Two render modes:
          ...
          * **Open-ended** (``choices`` is None or empty) — render the
            ...
            resolves the clarify automatically.
        ...
        catches the user's reply instead of timing out.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from elevate_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ELEVATE_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ELEVATE_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ELEVATE_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ELEVATE_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("ELEVATE_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("ELEVATE_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
