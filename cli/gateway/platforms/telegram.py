...
def _coerce_chat_id(chat_id: Any):
    """Return a Telegram-valid chat target, or None if the value is unusable.

    Telegram accepts a numeric chat_id (incl. negative -100… supergroup ids) OR
    an '@username' string. Blindly int()-ing the target crashed the send path
    with ValueError when a non-numeric value leaked in (e.g. a pairing code like
    '8RWK85SD' stored as TELEGRAM_HOME_CHANNEL), looping cron delivery failures.
    Digit strings → int; '@handle' → passthrough str; anything else → None so the
    caller skips with one clean error instead of raising.
    """
    if chat_id is None:
        return None
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("ELEVATE_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = float(os.getenv("ELEVATE_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("ELEVATE_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "(possibly OpenClaw or another Elevate instance). "
            "Elevate stopped Telegram polling after %d retries. "
            "Only one poller can run per token — stop the other process "
            "and restart with 'elevate start'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("ELEVATE_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("ELEVATE_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("ELEVATE_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("ELEVATE_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("ELEVATE_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            proxy_url = resolve_proxy_url("TELEGRAM_PROXY")
            disable_fallback = (os.getenv("ELEVATE_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in ("1", "true", "yes", "on"))
    ...
    async def send(
        ...
        # Resolve the chat target ONCE. A non-numeric/non-@handle value (e.g. a
        # pairing code leaked into TELEGRAM_HOME_CHANNEL) must not crash the send
        # loop on int() — skip with one clean error instead of looping ValueError.
        resolved_chat_id = _coerce_chat_id(chat_id)
        if resolved_chat_id is None:
            logger.error(
                "[%s] invalid Telegram chat target %r — not a numeric id or "
                "@username; skipping send", self.name, chat_id,
            )
            return SendResult(success=False, error=f"invalid chat target: {chat_id!r}")
    ...
    def _message_mentions_bot(self, message: Message, bot: Any = None) -> bool:
        ...
        # raw substring matches like "foo@elevate_bot.example" are not mentions
