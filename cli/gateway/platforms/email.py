"""
Email platform adapter for the Elevate gateway.

Allows users to interact with Elevate by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "elevate-agent" "version" "{_elevate_version}" '
            '"vendor" "Ctrl Strategies" '
            '"support-email" "support@ctrlstrategies.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Elevate")
        ...
        msg_id = f"<elevate-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
