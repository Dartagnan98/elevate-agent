# OpenAI-Compatible API Server for Elevate
...
SSE streaming. By exposing this endpoint, elevate becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.elevate/.env`
2. Run `elevate gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with elevate through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns elevate as a model) |
...
Authorization: Bearer elevate-api-key-here
...
But elevate sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "elevate"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured elevate model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of elevate's core prompt
...
Once implemented, elevate works as a drop-in backend for:
