"""
Elevate Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with elevate;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.elevate/plugins/<name>/``
3. **Project plugins** – ``./.elevate/plugins/<name>/`` (opt-in via
   ``ELEVATE_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``elevate_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``ELEVATE_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
...
# ---------------------------------------------------------------------------
#
# Set ``ELEVATE_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.elevate/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ELEVATE_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ELEVATE_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Elevate loggers.
    """
    ...
    logger.debug(
        "ELEVATE_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "elevate_agent.plugins"

_NS_PARENT = "elevate_plugins"
...
class PluginManifest:
    ...
    #              in ~/.elevate/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``elevate plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``elevate honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``elevate <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.elevate/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to elevate_cli/. Two layouts are
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with elevate and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Elevate ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `elevate plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``elevate_plugins.<slug>``.
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="elevate-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``elevate tools`` TUI so plugin-provided toolsets appear
