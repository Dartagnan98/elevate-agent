"""elevate webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.elevate/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _elevate_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: elevate gateway run
"""
...
def webhook_command(args):
    """Entry point for 'elevate webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: elevate webhook {subscribe|list|remove|test}")
        print("Run 'elevate webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (elevate gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: elevate webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from elevate_core webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (elevate gateway run)")
