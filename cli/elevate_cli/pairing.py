...
Usage:
    elevate pairing list              # Show all pending + approved users
    elevate pairing approve <platform> <code>  # Approve a pairing code
    elevate pairing revoke <platform> <user_id> # Revoke user access
    elevate pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle elevate pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: elevate pairing {list|approve|revoke|clear-pending}")
        print("Run 'elevate pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.elevate/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'elevate pairing list' to see pending codes.\n")
