"""`elevate checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.elevate/checkpoints/``.  Actions:

    elevate checkpoints               # same as `status`
    elevate checkpoints status        # total size, project count, breakdown
    elevate checkpoints list          # per-project checkpoint counts + workdir
    elevate checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    elevate checkpoints clear [-f]    # nuke the entire base (asks first)
    elevate checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    elevate checkpoints
    elevate checkpoints prune --retention-days 3 --max-size-mb 200
    elevate checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: elevate checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``elevate checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `elevate checkpoints` → status
