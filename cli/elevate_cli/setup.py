"""
Interactive setup wizard for Elevate.
...
from elevate_cli.nous_subscription import get_nous_subscription_features
...
# display_elevate_home imported lazily at call sites (stale-module safety during elevate update)
...
from elevate_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("▲ Elevate Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Elevate using environment variables or config commands:")
    print_info("  elevate config set model.provider custom")
    print_info("  elevate config set model.base_url http://localhost:8080/v1")
    print_info("  elevate config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'elevate setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'elevate setup')")


def _print_setup_summary(config: dict, elevate_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'elevate setup' to configure"))
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (Elevate subscription)", True, None))
    ...
    # Image generation — FAL (direct or via Elevate), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (Elevate subscription)", True, None))
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via Elevate subscription)", True, None))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'elevate setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (Elevate subscription)", True, None))
    elif config.get("terminal", {}).get("backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'elevate setup terminal'"))
    elif managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via Elevate subscription)", True, None))
    ...
    # Spotify (OAuth via elevate auth spotify — check auth.json, not env vars)
    try:
        from elevate_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'elevate setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {elevate_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('elevate config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('elevate config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('elevate config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('elevate', Colors.GREEN)}              Start chatting")
    print(f"   {color('elevate gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('elevate doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `elevate tools` and `elevate setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``elevate model``)
    ...
    provider added to ``elevate model`` is automatically available here.
    ...
    # Delegate to the shared elevate model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from elevate_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: elevate model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from elevate_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Elevate can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Elevate can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'elevate setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("Elevate Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "nous-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Elevate gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.elevate/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"elevate setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'elevate setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Elevate runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Elevate via Telegram/Discord, this is where")
        print_info(
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Elevate gateway and bill to your subscription.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("ELEVATE_MAX_ITERATIONS", str(max_iter))
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Elevate delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Elevate delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'elevate whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'elevate whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from elevate_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from elevate_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from elevate_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from elevate_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from elevate_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from elevate_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from elevate_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from elevate_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_bluebubbles():
    ...
    print_info("Connects Elevate to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from elevate_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Elevate from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'elevate setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     elevate config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_elevate_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  elevate gateway")
        else:
            from elevate_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   elevate gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   elevate gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `elevate setup tools` and `elevate tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``elevate_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from elevate_cli.config
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_elevate.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_elevate migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_elevate", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Elevate.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Elevate to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Elevate at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Elevate at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Elevate at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Elevate at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Elevate equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Elevate config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Elevate.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Elevate's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(elevate_home: Path) -> bool:
    ...
    print_info("Elevate can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: elevate claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=elevate_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: elevate claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Elevate configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Elevate config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Elevate (use elevate claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      elevate setup           — full or quick (auto-detected)
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ▲ Elevate Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ▲ Elevate Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Elevate installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, elevate_home)
            return
        elif choice == 1:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, elevate_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(elevate_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, elevate_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {elevate_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'elevate config edit'")
...
def _resolve_elevate_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``elevate chat`` in a fresh process."""
    elevate_bin = shutil.which("elevate")
    if elevate_bin:
        return [elevate_bin, "chat"]

    try:
        if importlib.util.find_spec("elevate_cli") is not None:
            return [sys.executable, "-m", "elevate_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch elevate chat now?", True):
        return

    chat_argv = _resolve_elevate_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Elevate automatically. Run 'elevate chat' manually.")
        return
...
def _run_first_time_quick_setup(config: dict, elevate_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'elevate setup gateway'",
        ],
        0,
    )
...
def _run_quick_setup(config: dict, elevate_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'elevate setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Elevate to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'elevate setup gateway'.")
