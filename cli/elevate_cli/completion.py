"""Shell completion script generation for elevate CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_elevate_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Elevate bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef elevate
# Elevate zsh completion
# Add to ~/.zshrc:
...
{top_cmds_str}
            ...
            _describe 'elevate command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Elevate fish completion",
        "# Add to your config:",
        "#   elevate completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __elevate_profiles",
        "    echo default",
        "    if test -d $HOME/.elevate/profiles",
        "        ls $HOME/.elevate/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c elevate -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c elevate -f -s p -l profile"
        " -d 'Profile name' -xa '(__elevate_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__elevate_profiles)' -d 'Profile name'"
                )
