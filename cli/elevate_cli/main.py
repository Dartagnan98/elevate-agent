#!/usr/bin/env python3
"""
Elevate CLI - Main entry point.

Usage:
    elevate                     # Interactive chat (default)
    elevate chat                # Interactive chat
    elevate gateway             # Run gateway in foreground
    elevate gateway start       # Start gateway as service
    elevate gateway stop        # Stop gateway service
    elevate gateway status      # Show gateway status
    elevate gateway install     # Install gateway service
    elevate gateway uninstall   # Uninstall gateway service
    ...
    elevate honcho map <name>               # Map current directory to a session name
    elevate honcho peer                     # Show peer names and dialectic settings
    elevate honcho peer --user NAME         # Set user peer name
    elevate honcho peer --ai NAME           # Set AI peer name
    elevate honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    elevate honcho mode                     # Show current memory mode
    elevate honcho mode [hybrid|honcho|local]  # Set memory mode
    elevate honcho tokens                   # Show token budget settings
    elevate honcho tokens --context N       # Set session.context() token cap
    elevate honcho tokens --dialectic N     # Set dialectic result char cap
    elevate honcho identity                 # Show AI peer identity representation
    elevate honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    elevate honcho migrate                  # Step-by-step migration guide: OpenClaw native → Elevate + Honcho
    elevate version             Show version
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ELEVATE_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (elevate tools, elevate setup, elevate model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'elevate {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any elevate module import.
#
# Many modules cache ELEVATE_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ELEVATE_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.elevate/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # 2. If no flag, check active_profile in the elevate root
    if profile_name is None:
        try:
            from elevate_constants import get_default_elevate_root

            active_path = get_default_elevate_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ELEVATE_HOME
    if profile_name is not None:
        try:
            from elevate_cli.profiles import resolve_profile_env

            elevate_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent elevate from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ELEVATE_HOME"] = elevate_home
...
load_elevate_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ELEVATE_REDACT_SECRETS env
# var BEFORE elevate_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "ELEVATE_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early
        _cfg_path = get_elevate_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["ELEVATE_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `elevate` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from elevate_logging import setup_logging as _setup_logging
...
try:
    from elevate_cli.config import load_config as _load_config_early
    from elevate_constants import apply_ipv4_preference as _apply_ipv4
...
from elevate_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Elevate itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_elevate_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_elevate_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Elevate has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Elevate to use their tokens.
    if _has_elevate_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, elevate_bin
        cli_args: the original CLI arguments (everything after 'elevate')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    elevate_bin = container_info["elevate_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo elevate {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo elevate {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str]) -> None:
    ...
    print(f"  elevate --tui --resume {target}")
    if title:
        print(f'  elevate --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @elevate/ink is missing or node_modules is behind package-lock.json (post-pull)."""
    ink = root / "node_modules" / "@elevate" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: ELEVATE_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("ELEVATE_TUI_DIR")
    if env:
        p = Path(env)
...
def _elevate_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "elevate-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``ELEVATE_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ELEVATE_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    elevate_home = os.environ.get("ELEVATE_HOME") or str(Path.home() / ".elevate")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ELEVATE_HOME": elevate_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(elevate_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ELEVATE_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ELEVATE_NODE")
    ...
    # pre-built dist + node_modules (nix / full ELEVATE_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("ELEVATE_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("ELEVATE_QUIET"):
    ...
    if tui_dev:
        if _elevate_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/elevate-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
                ...
                print("@elevate/ink build failed.")
                if preview:
                    print(preview)
                sys.exit(1)
...
def _launch_tui(resume_session_id: Optional[str] = None, tui_dev: bool = False):
    ...
    if resume_session_id:
        env["ELEVATE_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'elevate sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Elevate isn't configured yet -- no API keys or providers found."
        )
        print()
    ...
    try:
        from elevate_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ELEVATE_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.elevate/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ELEVATE_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ELEVATE_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ELEVATE_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from elevate_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Elevate?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: elevate gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  elevate gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '▲ Elevate'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  elevate gateway")
        ...
        print()
        print("  Or install as a service: elevate gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'elevate whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from elevate_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``elevate model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("ELEVATE_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'elevate model' for "
                "available providers, or run 'elevate doctor' to diagnose config "
                "issues."
            )
    ...
    from elevate_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.elevate/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.elevate/.env if the active provider is not 'custom'.
    ...
    """
    from elevate_cli.config import get_env_value, save_env_value, load_config
...
#
# Elevate uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `elevate model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `elevate model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Elevate only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``elevate model`` flow, then route aux tasks to them here.
    """
    from elevate_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from elevate_cli.auth import _prompt_model_selection
    from elevate_cli.models import get_pricing_for_provider
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from elevate_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from elevate_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from elevate_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from elevate_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from elevate_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `elevate auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from elevate_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from elevate_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    from elevate_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from elevate_cli.auth import _save_model_choice, deactivate_provider
    from elevate_cli.config import get_env_value, load_config, save_config
    ...
    from elevate_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `elevate model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from elevate_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from elevate_cli.config import save_env_value, load_config, save_config
    from elevate_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from elevate_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from elevate_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Elevate turns to `copilot --acp`.")
    print("  Elevate currently starts its own ACP subprocess for each request.")
    print("  Elevate uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ELEVATE_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from elevate_cli.config import get_env_value, save_env_value, load_config, save_config
    from elevate_cli.models import fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so elevate knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Elevate typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Elevate, enable billing on your "
    ...
    if provider_id == "ollama-cloud":
        from elevate_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from elevate_constants import display_elevate_home as _dhh_fn

            print(
                f"    Elevate will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'elevate model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Elevate CLI with a provider."""
    from elevate_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from elevate_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from elevate_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from elevate_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from elevate_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from elevate_cli.webhook import webhook_command
...
def cmd_hooks(args):
    ...
    from elevate_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from elevate_cli.doctor import run_doctor
...
def cmd_debug(args):
    ...
    from elevate_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from elevate_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Elevate home directory to a zip file."""
    if getattr(args, "quick", False):
        from elevate_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Elevate backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"Elevate v{__version__} ({__release_date__})")
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``elevate update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``elevate web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (elevate web will not be available)")
        )
        if fatal:
...
def _update_via_zip(args):
    ...
    try:
        tmp_dir = tempfile.mkdtemp(prefix="elevate-update-")
        zip_path = os.path.join(tmp_dir, f"elevate-{branch}.zip")
        ...
        # GitHub ZIPs extract to elevate-<branch>/
        extracted = os.path.join(tmp_dir, f"elevate-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "elevate-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Elevate behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes elevate completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Elevate couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Elevate couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Elevate behaves unexpectedly.")
    return True
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_elevate_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_elevate_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``elevate update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from elevate_constants import get_default_elevate_root

    default_home = get_default_elevate_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "elevate[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``elevate update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.elevate/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``elevate update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``elevate update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.elevate/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``elevate update --gateway``) the update is already
    ...
    try:
        ...
        # elevate_cli.config.get_elevate_home to simulate setup failure.
        from elevate_cli.config import get_elevate_home as _get_elevate_home

        logs_dir = _get_elevate_home() / "logs"
...
def cmd_update(args):
    ...
    """
    from elevate_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Elevate")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # attributes like display_elevate_home() added since the last release.
        try:
            import importlib
            import elevate_constants as _hc
        ...
        try:
            from elevate_cli.profiles import (
                list_profiles,
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'elevate config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: elevate config migrate")
            else:
                print()
        ...
        # When running as ``elevate update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_elevate_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all elevate-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. elevate-gateway.service or elevate-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: elevate gateway run")
        ...
        # Warn if legacy Elevate gateway unit files are still installed.
        # When both elevate.service (from a pre-rename install) and the
        # current elevate-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `elevate update` surfaces the issue until the user migrates.
        try:
            from elevate_cli.gateway import (
                has_legacy_elevate_units,
                _find_legacy_elevate_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_elevate_units():
                print()
                print("⚠ Legacy Elevate gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_elevate_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (elevate.service) fight the current")
                print("  elevate-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    elevate gateway migrate-legacy")
                print()
        ...
        print("  elevate model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``elevate -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `elevate profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_elevate_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → elevate -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.elevate)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  elevate profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     elevate -p {name} chat")
                else:
        ...
        from elevate_cli.profiles import profile_exists
        ...
        from elevate_cli.profiles import rename_profile
        ...
        from elevate_cli.profiles import export_profile
        ...
        from elevate_cli.profiles import import_profile
...
def cmd_completion(args, parser=None):
    ...
    from elevate_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Elevate log files."""
    from elevate_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for elevate CLI."""
    parser = argparse.ArgumentParser(
        prog="elevate",
        description="Elevate - AI assistant with tool-calling capabilities",
...
""",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to ELEVATE_ACCEPT_HOOKS=1 or "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.elevate/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Elevate",
    )
    ...
    chat_parser.add_argument(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also ELEVATE_ACCEPT_HOOKS env var and "
    ...
    chat_parser.add_argument(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.elevate/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    chat_parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: elevate-cli)",
    )
    model_parser.add_argument(
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy elevate.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Elevate gateway unit files "
            "(e.g. elevate.service) left over from older installs. Profile "
            "units (elevate-gateway-<profile>.service) and unrelated "
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Elevate CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: elevate-cli)"
    )
    ...
    license_parser = subparsers.add_parser(
        "license",
        help="Show or manage your Elevate subscription license",
    )
    license_parser.add_argument(
        "license_action",
        nargs="?",
        choices=["status", "refresh", "logout"],
        default="status",
    )
    license_parser.set_defaults(func=elevate_license.cmd_license)

    from elevate_cli import cloud_skills as elevate_cloud_skills

    cloud_parser = subparsers.add_parser(
        "cloud-skills",
        help="Manage cloud skills fetched from your Elevate subscription",
    )
    cloud_sub = cloud_parser.add_subparsers(dest="cloud_action", required=True)
    cloud_list = cloud_sub.add_parser("list", help="List cloud skills available at your tier")
    cloud_list.set_defaults(func=elevate_cloud_skills.cmd_cloud_list)
    cloud_fetch = cloud_sub.add_parser("fetch", help="Fetch a single skill body from the backend")
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate Elevate with Spotify via PKCE")
    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set ELEVATE_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Elevate components",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.elevate/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.elevate/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Elevate setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Elevate setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Elevate. Use 'elevate debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    elevate debug share              Upload debug report and print URL
...
""",
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Elevate home directory to a zip file",
        description="Create a zip archive of your entire Elevate configuration, "
        "skills, sessions, and data (excludes the elevate codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/elevate-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Elevate backup from a zip file",
        description="Extract a previously created Elevate backup into your "
        "Elevate home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Elevate configuration",
    )
    ...
    def cmd_pairing(args):
        from elevate_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.elevate/skills/.bundled_manifest) "
            "so future 'elevate update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from elevate_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from elevate_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/elevate-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `elevate plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from elevate_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "benchmark":
            ...
            mem_dir = get_elevate_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_elevate_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_elevate_home()}/memories/\n")
        else:
            from elevate_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'elevate tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # elevate tools list [--platform cli]
    ...
    # elevate tools disable <name...> [--platform cli]
    ...
    # elevate tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from elevate_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from elevate_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Elevate as an MCP server",
        description=(
            "Manage MCP server connections and run Elevate as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'elevate mcp add' to connect to a new server, or\n"
            "'elevate mcp serve' to expose Elevate conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Elevate as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from elevate_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch elevate --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            elevate_bin = shutil.which("elevate")
            if elevate_bin:
                os.execvp(elevate_bin, ["elevate", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "elevate_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Elevate",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Elevate",
    ...
    def cmd_claw(args):
        from elevate_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Elevate to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Elevate",
        description="Remove Elevate from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Elevate as an ACP (Agent Client Protocol) server",
        description="Start Elevate in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Elevate as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Elevate instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Elevate web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `elevate --tui` via PTY/WebSocket). "
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Elevate log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    elevate logs                    Show last 50 lines of agent.log
    elevate logs -f                 Follow agent.log in real time
    elevate logs errors             Show last 50 lines of errors.log
    elevate logs gateway -n 100     Show last 100 lines of gateway.log
    elevate logs --level WARNING    Only show WARNING and above
    elevate logs --session abc123   Filter by session ID
    elevate logs --component tools  Only show tool-related lines
    elevate logs --since 1h         Lines from the last hour
    elevate logs --since 30m -f     Follow, starting from 30 min ago
    elevate logs list               List available log files with sizes
""",
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `elevate model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    def _cmd_fallback(args):
        from elevate_cli.fallback_cmd import cmd_fallback as _impl
        return _impl(args)

    fallback_parser.set_defaults(func=_cmd_fallback)
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.elevate/.env.  Currently supports "
    ...
    from elevate_cli import secrets_cli as _secrets_cli
    ...
    # =========================================================================
    # migrate command — config migration for retired models / deprecated settings
    # =========================================================================
    ...
    def _cmd_migrate(args):
        from elevate_cli.migrate import cmd_migrate as _impl
        return _impl(args)

    def _cmd_migrate_xai(args):
        from elevate_cli.migrate import cmd_migrate_xai as _impl
        return _impl(args)

    migrate_xai.set_defaults(func=_cmd_migrate_xai)
    migrate_parser.set_defaults(func=_cmd_migrate)

    # =========================================================================
    # slack command — Slack integration helpers (manifest generation, etc.)
    # =========================================================================
    slack_parser = subparsers.add_parser(
        "slack",
        help="Slack integration helpers (manifest generation, etc.)",
        description="Slack integration helpers for Elevate.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ELEVATE_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Elevate")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    def _cmd_slack(args):
        ...
        if sub == "manifest":
            from elevate_cli.slack_cli import slack_manifest_command
            return slack_manifest_command(args)
        slack_parser.print_help()
        return 0

    slack_parser.set_defaults(func=_cmd_slack)

    # =========================================================================
    # checkpoints command — inspect / prune / clear the checkpoint store
    # =========================================================================
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.elevate/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo elevate uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from elevate_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    from elevate_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    try:
        from elevate_cli.curator import register_cli as _register_curator_cli
    ...
    # e.g. ``elevate -c Pokemon Agent Dev`` → ``elevate -c 'Pokemon Agent Dev'``
    ...
    from elevate_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'elevate -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (elevate hooks list, cron
