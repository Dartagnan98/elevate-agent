...
9. **Ask for workflows:**
   ...
   - Add it as a persistent cron (survives restarts automatically):
     ```bash
     agent_bus add-cron your agent name <workflow-name> <interval> <prompt>
     ```
     For example:
     ```bash
     agent_bus add-cron your agent name heartbeat 6h Read HEARTBEAT.md and follow its instructions.
     agent_bus add-cron your agent name daily-report "0 9 * * 1-5" Generate and send the daily analytics report.
     ```
...
11b. **Ask about migration:**
    ...
    **END YOUR TURN.** If the user says no migration, skip to step 12. If yes, continue below.
    ...
    If migrating from a different workspace (not another Elevate agent):
...
12. **Confirm heartbeat cadence:**
    ...
    ```bash
    agent_bus add-cron your agent name heartbeat <new_interval> Read HEARTBEAT.md and follow its instructions.
    ```
...
13. **Knowledge base setup — ALWAYS DO THIS STEP:**
    ...
    **END YOUR TURN.** Wait for the user's answers.
    ...
    agent_bus kb-ingest \
      ...
      --org your org --scope private \
      --agent your agent name \
      --collection "memory-your agent name" --force
    ```
