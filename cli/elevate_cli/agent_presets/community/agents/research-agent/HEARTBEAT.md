...
```bash
agent_bus update-heartbeat "<1-sentence summary of current work>"
```
...
```bash
agent_bus ack-inbox "<message_id>"
```
...
When done:
```bash
agent_bus complete-task "<task_id>" --result "<summary of what was produced>"
```
...
If yes, log it:
```bash
agent_bus log-event action guardrail_triggered info --meta '{"guardrail":"<which one>","context":"<what happened>"}'
```
