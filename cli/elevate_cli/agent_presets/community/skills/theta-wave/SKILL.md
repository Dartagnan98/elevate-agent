...
4. Log the outcome:
   ```bash
   agent_bus log-event action theta_wave_complete info --meta '{"proposed":X,"approved":Y,"deferred":Z}'
   ```
...
agent_bus log-event action theta_wave_start info --meta '{"agent":"'your agent name'","initiated_by":"<analyst_name>"}'

# When complete
agent_bus log-event action theta_wave_complete info --meta '{"proposed":X,"approved":Y}'
```
