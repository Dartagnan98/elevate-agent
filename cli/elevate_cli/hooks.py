"""elevate hooks — inspect and manage shell-script hooks.

Usage::

    elevate hooks list
    elevate hooks test <event> [--for-tool X] [--payload-file F]
    elevate hooks revoke <command>
    elevate hooks doctor

Consent records live under ``~/.elevate/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.elevate/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``elevate hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: elevate hooks {list|test|revoke|doctor}")
        print("Run 'elevate hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.elevate/config.yaml.")
        print("See `elevate hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `elevate hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `elevate hooks test` and `elevate hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Elevate wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `elevate hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `elevate hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `elevate hooks doctor`.")
