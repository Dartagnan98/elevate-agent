"""
Unified tool configuration for Elevate.

`elevate tools` and `elevate setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.elevate/config.yaml under
...
from elevate_cli.nous_subscription import (
...
from elevate_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ELEVATE_CORE_TOOLS (available at runtime if enabled),
...
from elevate_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        # to ~/.elevate/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from elevate_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: elevate auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `elevate auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: elevate auth spotify")
        except Exception as exc:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "elevate-cli" (which include all _ELEVATE_CORE_TOOLS) cause disabled
    ...
    # shipped as a bundled plugin but user must opt in via `elevate tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by elevate tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (elevate-cli, elevate-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `elevate tools` and `elevate setup tools`.
    ...
    print(color("  Guide: https://elevate.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from elevate_constants import display_elevate_home
    print(color(f"  Tool configuration saved to {display_elevate_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'elevate' or gateway restart.", Colors.DIM))
    print()
