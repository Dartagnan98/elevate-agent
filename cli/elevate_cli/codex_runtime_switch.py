...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Elevate's
...
The actual runtime resolution happens in elevate_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "elevate", "hermes"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Elevate's MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Elevate for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from elevate_cli.codex_runtime_plugin_migration import migrate
            ...
            # Permissions + Elevate tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                msg_lines.append(
                    "Elevate tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Elevate runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Elevate tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Elevate runtime.")
