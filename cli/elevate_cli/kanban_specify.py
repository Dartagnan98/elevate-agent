...
Used by ``elevate kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``elevate_cli/goals.py`` — same aux
...
ELEVATE_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    int(os.getenv("ELEVATE_KANBAN_SPECIFY_MAX_TOKENS", "6000")),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Elevate Agent board.
...
def _profile_author() -> str:
    """Mirror of ``elevate_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("ELEVATE_PROFILE")
