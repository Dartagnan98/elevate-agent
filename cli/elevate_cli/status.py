"""
Status command for elevate CLI.

Shows the status of all Elevate Agent components.
"""
...
from elevate_cli.runtime_provider import resolve_requested_provider
from elevate_cli.vercel_auth import describe_vercel_auth
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``elevate config``'s
...
def show_status(args):
    ...
    print(color("│                 ⚕ Elevate Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: elevate model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: elevate model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from elevate_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'elevate-agent[vercel]')"
    ...
    try:
        from elevate_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   elevate gateway")
    ...
    jobs_file = get_elevate_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_elevate_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'elevate doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'elevate setup' to configure", Colors.DIM))
    print()
