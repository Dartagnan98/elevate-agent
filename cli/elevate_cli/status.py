...
Shows the status of all Elevate components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" sentinel with DIM color when *key* is empty.
    """
...
def show_status(args):
    """Show status of all Elevate components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ▲ Elevate Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from elevate_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    if not terminal_env:
        ...
        # (elevate status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
