...
on Japanese locales, etc.).  Elevate's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``elevate_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Elevate is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ELEVATE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ELEVATE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Elevate-managed Git install directories so
    ...
    # already-running shells don't see the change — which means elevate
    ...
    # install without Elevate-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Elevate's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Elevate's
    ``elevate config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Elevate (User env var in Windows Settings, or
    export in a PowerShell profile) and Elevate picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Elevate-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\elevate\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``elevate`` from right after install) retain their old PATH.

    Any subprocess Elevate spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Elevate-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "elevate", "git", "cmd"),
        os.path.join(local_appdata, "elevate", "git", "bin"),
        os.path.join(local_appdata, "elevate", "git", "usr", "bin"),
        # Elevate venv Scripts directory — host of the elevate.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes elevate, but harmless to include.
        os.path.join(local_appdata, "elevate", "elevate-agent", "venv", "Scripts"),
