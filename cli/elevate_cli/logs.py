"""``elevate logs`` — view and filter Elevate log files.
...
Usage examples::
    ...
    elevate logs                    # last 50 lines of agent.log
    elevate logs -f                 # follow agent.log in real time
    elevate logs errors             # last 50 lines of errors.log
    elevate logs gateway -n 100    # last 100 lines of gateway.log
    elevate logs --level WARNING    # only WARNING+ lines
    elevate logs --session abc123   # filter by session ID substring
    elevate logs --component tools  # only tool-related lines
    elevate logs --since 1h         # lines from the last hour
    elevate logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_elevate_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Elevate runs — try 'elevate chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_elevate_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_elevate_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_elevate_home()}/logs/")
        return

    print(f"Log files in {display_elevate_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'elevate chat' to generate logs)")
