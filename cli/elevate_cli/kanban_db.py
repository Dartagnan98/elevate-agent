...
``<root>`` is the **shared Elevate root** (the parent of any active
...
``elevate -p <profile>`` joins the same board as the dispatcher that
...
the profile's Elevate home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ELEVATE_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ELEVATE_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``elevate kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.elevate``. In Docker / custom
deployments where ``ELEVATE_HOME`` points outside ``~/.elevate`` (e.g.
``/opt/elevate``), ``<root>`` is ``ELEVATE_HOME``. Legacy env-var
overrides still work:

* ``ELEVATE_KANBAN_DB`` — pin the database file path directly.
* ``ELEVATE_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ELEVATE_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ELEVATE_KANBAN_DB``,
``ELEVATE_KANBAN_WORKSPACES_ROOT``, and ``ELEVATE_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/elevate-kanban-v1-spec.pdf`` for the full
...
# ``ELEVATE_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ELEVATE_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ELEVATE_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``elevate-agent``
...
def kanban_home() -> Path:
    """Return the shared Elevate root that anchors the kanban board.

    Resolution order:

    1. ``ELEVATE_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_elevate_root()``, which already returns ``<root>``
       when ``ELEVATE_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ELEVATE_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ELEVATE_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("ELEVATE_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``elevate kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``ELEVATE_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``elevate kanban boards
    ...
    """
    env = os.environ.get("ELEVATE_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``elevate kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ELEVATE_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ELEVATE_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ELEVATE_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("ELEVATE_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``elevate kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``ELEVATE_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``ELEVATE_SESSION_ID``. NULL
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from elevate_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``elevate --skills ...`` alongside the built-in
    ...
    # `elevate --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``elevate kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `elevate kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``elevate kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``elevate kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``elevate kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``elevate kanban complete <id>``) works without requiring
...
#      via ``elevate kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Elevate
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``elevate kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Elevate profile.
    ...
    if not rows:
        return False
    try:
        from elevate_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Elevate profile.
        # `_default_spawn` invokes ``elevate -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Elevate
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `elevate kanban tail` — without
...
def _resolve_elevate_argv() -> list[str]:
    ...
    1. ``$ELEVATE_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("elevate")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\elevate.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m elevate_cli.main`` — fallback for setups where
       Elevate is launched from a venv and the ``elevate`` shim is not on
...
def _kanban_worker_skill_available(elevate_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``elevate -p <name>``), its ``SKILLS_DIR``
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``elevate -p <profile> chat -q ...`` subprocess.
    ...
    ``ELEVATE_KANBAN_DB`` / ``ELEVATE_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject ELEVATE_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `elevate -p <name>` the
    # _apply_profile_override() runs *before* elevate_constants is imported.
    # If ELEVATE_HOME is absent from the child's env, get_elevate_home() falls
    # back to Path.home() / ".elevate" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from elevate_cli.profiles import resolve_profile_env
    try:
        env["ELEVATE_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ELEVATE_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ELEVATE_HOME never had profiles created.
        pass
    if task.tenant:
        env["ELEVATE_TENANT"] = task.tenant
    env["ELEVATE_KANBAN_TASK"] = task.id
    env["ELEVATE_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["ELEVATE_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ELEVATE_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ELEVATE_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`elevate -p <name>`
    # rewrites ELEVATE_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_elevate_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ELEVATE_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ELEVATE_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ELEVATE_KANBAN_BOARD"] = resolved_board
    # ELEVATE_PROFILE is the author the kanban_comment tool defaults to.
    # `elevate -p <assignee>` activates the profile, but the env var is
    ...
    if _kanban_worker_skill_available(env.get("ELEVATE_HOME")):
    ...
    # `elevate kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`elevate` executable not found on PATH. "
            "Install Elevate Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``elevate kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ELEVATE_PROFILE values like "elevate-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Elevate root exists

    Reads profile paths directly so this module has no import dependency on
    ``elevate_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from elevate_constants import get_default_elevate_root
        default_root = get_default_elevate_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``elevate kanban assignees`` for the terminal.
