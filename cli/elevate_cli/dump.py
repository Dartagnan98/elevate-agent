"""
Dump command for elevate CLI.

Outputs a compact, plain-text summary of the user's Elevate setup
...
def _gateway_status() -> str:
    ...
    try:
        from elevate_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(elevate_home: Path) -> int:
...
def _cron_summary(elevate_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = elevate_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"elevate_home:      {display_elevate_home()}")
    ...
    toolsets = config.get("toolsets", ["elevate-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(elevate_home)}")
    lines.append(f"  skills:             {_count_skills(elevate_home)}")
