"""
Gateway subcommand for elevate CLI.

Handles: elevate gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_elevate_home is imported lazily at call sites to avoid ImportError
# when elevate_constants is cached from a pre-update version during `elevate update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "elevate-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "elevate_cli.main gateway",
        "elevate_cli.main --profile",
        "elevate_cli.main -p",
        "elevate_cli/main.py gateway",
        "elevate_cli/main.py --profile",
        "elevate_cli/main.py -p",
        "elevate gateway",
        "gateway/run.py",
    ]
    current_home = str(get_elevate_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"ELEVATE_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "ELEVATE_HOME=" in command and f"ELEVATE_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``elevate update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Elevate profile are returned.
    """
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}elevate gateway status\n"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from elevate_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `elevate gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ELEVATE_HOME-scoped).
...
from elevate_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "elevate-gateway"
SERVICE_DESCRIPTION = "Elevate Gateway - Messaging Platform Integration"
...
def _profile_suffix() -> str:
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ELEVATE_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(elevate_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ELEVATE_HOME is a named profile.

    For ``~/.elevate/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        elevate_home: Optional explicit ELEVATE_HOME path. Defaults to the current
            ``get_elevate_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from elevate_constants import get_default_elevate_root
    home = Path(elevate_home or str(get_elevate_home())).resolve()
    default = get_default_elevate_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ELEVATE_HOME.

    Default ``~/.elevate`` returns ``elevate-gateway`` (backward compatible).
    Profile ``~/.elevate/profiles/coder`` returns ``elevate-gateway-coder``.
    Any other ELEVATE_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Elevate installs that predate the
# elevate-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (elevate-gateway-*.service) and unrelated third-party
# "elevate" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("elevate.service",)
...
def _find_legacy_elevate_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Elevate gateway units.

    Detects unit files installed by older Elevate versions that used a
    different service name (e.g. ``elevate.service`` before the rename to
    ``elevate-gateway.service``). When both a legacy unit and the current
    ``elevate-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``elevate-gateway-coder.service`` and unrelated third-party
      ``elevate-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``elevate.service`` running an unrelated
...
def has_legacy_elevate_units() -> bool:
    """Return True when any legacy Elevate gateway unit files exist."""
    return bool(_find_legacy_elevate_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Elevate gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_elevate_units()
    if not legacy:
        return
    print_warning("Legacy Elevate gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current elevate-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    elevate gateway migrate-legacy")


def remove_legacy_elevate_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Elevate gateway unit files.

    Iterates over whatever ``_find_legacy_elevate_units()`` returns — which is
    ...
    legacy = _find_legacy_elevate_units()
    if not legacy:
        print("No legacy Elevate gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Elevate gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: elevate gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo elevate gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    elevate gateway uninstall")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Elevate can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo elevate gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo elevate gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo elevate gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Elevate often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.elevate`` → ``ai.elevate.gateway.plist`` (backward compatible).
    Profile ``~/.elevate/profiles/coder`` → ``ai.elevate.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.elevate.gateway-{suffix}" if suffix else "ai.elevate.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.elevate/elevate  -> /home/alice/.elevate/elevate
      /opt/elevate                 -> /opt/elevate  (kept as-is)
...
def _elevate_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ELEVATE_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_elevate_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.elevate                    → /home/alice/.elevate
      /root/.elevate/profiles/coder     → /home/alice/.elevate/profiles/coder
      /opt/custom-elevate               → /opt/custom-elevate  (kept as-is)
    """
    current_elevate = get_elevate_home().resolve()
    current_default = (Path.home() / ".elevate").resolve()
    target_default = Path(target_home_dir) / ".elevate"

    # Default ~/.elevate → remap to target user's default
    if current_elevate == current_default:
        return str(target_default)

    # Profile or subdir of ~/.elevate → preserve the relative structure
    try:
        relative = current_elevate.relative_to(current_default)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    # Prefer .venv (Python 3.12, has psycopg) over venv (legacy 3.11, broken).
    venv_bin = project_root / ".venv" / "bin"
    if not _is_dir(venv_bin):
        venv_bin = project_root / "venv" / "bin"
    if _is_dir(venv_bin):
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        elevate_home = _elevate_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m elevate_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="ELEVATE_HOME={elevate_home}"
Restart=on-failure
RestartSec=30
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Elevate install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (elevate.service from pre-rename installs)
    # before installing the new elevate-gateway.service. If both remain, they
    ...
    if has_legacy_elevate_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_elevate_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}elevate gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}elevate gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `elevate gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}elevate gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}elevate gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}elevate gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.elevate.gateway-{suffix}" if suffix else "ai.elevate.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Elevate install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  elevate gateway status             # Check status")
    from elevate_constants import display_elevate_home as _dhh
...
def launchd_stop():
    ...
    # `elevate gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Elevate install")
    else:
        print("⚠ Service definition is stale relative to the current Elevate install")
        print("  Run: elevate gateway start")
    ...
    if deep:
        log_file = get_elevate_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ▲ Elevate Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Elevate will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @elevate:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. elevate) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Elevate delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Elevate agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Elevate will use (e.g., elevate@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Elevate connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: elevate pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_elevate_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'elevate pairing approve')",
...
def _setup_whatsapp():
    ...
    from elevate_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from elevate_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Elevate will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Elevate will store the returned account_id/token in ~/.elevate/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `elevate gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_elevate_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `elevate pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `elevate pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"ElevateAgent\"")
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: elevate gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: elevate gateway install")
                else:
                    print_info("  You can install later: elevate gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo elevate gateway install --system")
                    print_info("  Or run in foreground:  elevate gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: elevate gateway run")
                print_info("  For persistence:   tmux new -s elevate 'elevate gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: elevate gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup elevate gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: elevate gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'elevate gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: elevate gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: elevate gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s elevate 'elevate gateway run'")
                print()
            ...
            print()
            print("  elevate gateway run                              # direct foreground")
            print("  tmux new -s elevate 'elevate gateway run'         # persistent via tmux")
            print("  nohup elevate gateway run > ~/.elevate/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: elevate gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: elevate gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: elevate gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: elevate gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: elevate gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  elevate gateway install")
                    print("  sudo elevate gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  elevate gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup elevate gateway run > ~/.elevate/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s elevate 'elevate gateway run'         # persistent via tmux")
                    print("  nohup elevate gateway run > ~/.elevate/logs/gateway.log 2>&1 &  # background")
                else:
        ...
        # pre-rename installs (e.g. elevate.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_elevate_units(interactive=not yes, dry_run=dry_run)
