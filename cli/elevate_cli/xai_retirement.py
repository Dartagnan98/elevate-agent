...
Pure logic: walks an Elevate config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `elevate doctor` and a future `elevate migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in an Elevate config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in an Elevate config and return retirement issues.
