...
# argparse runs (it sets ``ELEVATE_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``elevate_cli.relaunch`` should carry over when
...
def build_top_level_parser():
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via ELEVATE_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via ELEVATE_INFERENCE_PROVIDER env var."
        ),
    )
    parser.add_argument(
        "-t",
