...
- ``elevate_cli.providers``        -- canonical provider identity + overlays
- ``elevate_cli.model_normalize``  -- per-provider name formatting
...
from elevate_cli.providers import (
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``elevate config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from elevate_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'elevate model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'elevate doctor' — config issues detected:"
    ...
    # Mirrors the same logic in elevate_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from elevate_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from elevate_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by elevate provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest so newly added
    # Portal models surface in the /model picker without requiring a release.
    # Falls back to the in-repo _PROVIDER_MODELS["nous"] snapshot when the
    ...
    if "lmstudio" not in curated and (
        ...
        from elevate_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Elevate-mapped providers ---
    for elevate_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(elevate_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ELEVATE_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from elevate_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and elevate_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Elevate release.
        model_ids = curated.get(elevate_id, [])
        if elevate_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(elevate_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Elevate-only providers (nous, openai-codex, copilot, opencode-go) ---
    from elevate_cli.providers import ELEVATE_OVERLAYS
    from elevate_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Elevate provider ID.
    # ELEVATE_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Elevate IDs ("copilot").
    _mdev_to_elevate = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ELEVATE_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Elevate slug — e.g. "github-copilot" → "copilot"
        elevate_slug = _mdev_to_elevate.get(pid, pid)
        if elevate_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(elevate_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, elevate_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or elevate_slug in providers_store
                    or pid in pool_store or elevate_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(elevate_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", elevate_slug, exc)
        ...
        # configured.
        if not has_creds and elevate_slug == "anthropic":
            try:
                ...
                if (elevate_creds and elevate_creds.get("accessToken")) or \
        ...
        if elevate_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(elevate_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(elevate_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(elevate_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Elevate slug, fall back to overlay key
            model_ids = curated.get(elevate_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if elevate_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(elevate_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": elevate_slug,
            "name": get_label(elevate_slug),
            "is_current": elevate_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "elevate",
        ...
        seen_slugs.add(elevate_slug.lower())
        _record_builtin_endpoint(elevate_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ELEVATE_OVERLAYS (keeps /model in sync
    # with `elevate model`).
    try:
        from elevate_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Elevate's canonical write key (matches
            ...
            # Elevate writes ``models:`` as a dict keyed by model id
            # (see elevate_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Elevate's own writer uses; a hyphen variant is accepted
            ...
            # active model. Elevate's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, elevate_cli/config.py) already consume that dict.
        ...
        for grp_key, grp in groups.items():
            ...
            if _grp_url_norm and _grp_url_norm in _builtin_endpoints:
                continue
            # NOTE: Section 4 live probe from Hermes intentionally NOT ported.
            # It would replace explicit ``model:`` entries with the live
            # /v1/models output whenever an api_key is present, which breaks
            # Elevate's test contract that explicit models survive grouping.
            # Section 3 (user_providers:) still probes — that path has no
            # equivalent test expectations.
            results.append({
                "slug": slug,
                "name": grp["name"],
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`elevate_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from elevate_cli.models import fetch_openrouter_models
