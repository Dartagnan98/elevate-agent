"""Slash command definitions and autocomplete for the Elevate CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Elevate works on across turns until achieved", "Session",
    ...
    CommandDef("yolo", "Toggle YOLO mode (skip all dangerous command approvals)",
               "Configuration"),
    CommandDef("plan", "Enter plan mode — read-only research, the runtime blocks edits/commands until you approve",
               "Configuration", aliases=("planmode",)),
    CommandDef("run", "Approve the plan and leave plan mode (back to normal execution)",
               "Configuration", aliases=("execute", "exitplan", "approve-plan")),
    ...
    CommandDef("busy", "Control what Enter does while Elevate is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.elevate/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Elevate Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`elevate_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/elevate`` subcommand mapping, and
    ...
    """
    try:
        from elevate_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
Telegram only displays a small BotCommand menu in practice.  The full Elevate
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``elevate skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``elevate skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``elevate skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/elevate <verb>`` subcommand.
    ...
    can still reach them via ``/elevate <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/elevate`` is always reserved as the first entry so the
    legacy ``/elevate <subcommand>`` form keeps working for anything that
    ...
    # Reserve /elevate as the catch-all top-level command.
    entries.append(("elevate", "Talk to Elevate or run a subcommand", "[subcommand] [args]"))
    seen.add("elevate")
...
def slack_app_manifest(request_url: str = "https://elevate.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /elevate handler.

    Maps both canonical names and aliases so /elevate bg do stuff works
    the same as /elevate background do stuff.

    Plugin-registered slash commands are included so ``/elevate <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from elevate_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
