...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ELEVATE_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ELEVATE_HOME pointing
    ...
    ``~/.elevate`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    env: dict[str, str] = {}
    # ELEVATE_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_elevate_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ELEVATE_HOME its launcher sets
    ...
    # — that would override the launcher's ELEVATE_HOME and pin the subprocess
    ...
    env["ELEVATE_QUIET"] = "1"
    env["ELEVATE_REDACT_SECRETS"] = env.get("ELEVATE_REDACT_SECRETS", "true")
