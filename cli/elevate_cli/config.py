"""
Configuration management for Elevate.

Config files are stored in ~/.elevate/ for easy access:
- ~/.elevate/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.elevate/.env         - API keys and secrets

This module provides:
- elevate config          - Show current configuration
- elevate config edit     - Open config in editor
- elevate config set      - Set a specific value
- elevate config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.elevate/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``elevate logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  elevate config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "TELEGRAM_UNAUTHORIZED_DM_BEHAVIOR", "DISCORD_UNAUTHORIZED_DM_BEHAVIOR",
    "WHATSAPP_UNAUTHORIZED_DM_BEHAVIOR", "SLACK_UNAUTHORIZED_DM_BEHAVIOR",
    "SIGNAL_UNAUTHORIZED_DM_BEHAVIOR", "MATTERMOST_UNAUTHORIZED_DM_BEHAVIOR",
    "MATRIX_UNAUTHORIZED_DM_BEHAVIOR", "EMAIL_UNAUTHORIZED_DM_BEHAVIOR",
    "SMS_UNAUTHORIZED_DM_BEHAVIOR", "DINGTALK_UNAUTHORIZED_DM_BEHAVIOR",
    "FEISHU_UNAUTHORIZED_DM_BEHAVIOR", "WECOM_UNAUTHORIZED_DM_BEHAVIOR",
    "WECOM_CALLBACK_UNAUTHORIZED_DM_BEHAVIOR", "WEIXIN_UNAUTHORIZED_DM_BEHAVIOR",
    "BLUEBUBBLES_UNAUTHORIZED_DM_BEHAVIOR", "QQ_UNAUTHORIZED_DM_BEHAVIOR",
...
from elevate_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ELEVATE_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_elevate_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Elevate is running in package-manager-managed mode.

    Two signals: the ELEVATE_MANAGED env var (set by the systemd service),
    or a .managed marker file in ELEVATE_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Elevate was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.elevate/.install_method`` file (written by installers)
    2. ELEVATE_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_elevate_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.elevate/.install_method."""
    stamp = get_elevate_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade elevate"
        return "pip install --upgrade elevate"
...
def format_managed_message(action: str = "modify this Elevate installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ELEVATE_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Elevate installation is managed by NixOS "
            f"(ELEVATE_MANAGED={env_hint}).\n"
            "Edit services.elevate.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Elevate installation is managed by Homebrew "
            f"(ELEVATE_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade elevate"
        )

    return (
        f"Cannot {action}: this Elevate installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Elevate."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ELEVATE_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, elevate_bin
    or None if container mode is not active, we're already inside the
    container, or ELEVATE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ELEVATE_DEV") == "1":
        return None
    ...
    container_mode_file = get_elevate_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "elevate")
    exec_user = info.get("exec_user", "elevate")
    elevate_bin = info.get("elevate_bin", "/data/current-package/bin/elevate")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "elevate_bin": elevate_bin,
    }
...
# Re-export from elevate_constants — canonical definition lives there.
from elevate_constants import get_elevate_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_elevate_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_elevate_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the elevate group can
    share state with the gateway service.

    The mode can be overridden via the ELEVATE_HOME_MODE environment variable
    (e.g. ELEVATE_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ELEVATE_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ELEVATE_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Elevate runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ELEVATE_CONTAINER") or os.environ.get("ELEVATE_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ELEVATE_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def ensure_elevate_home():
    """Ensure ~/.elevate directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_elevate_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_elevate_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ELEVATE_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Elevate-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Elevate auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Elevate sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Elevate
    ...
    "browser": {
        ...
        "camofox": {
            ...
            # app owns the visible browser and Elevate should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # single read_file page exceeds these limits, Elevate truncates the
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.elevate/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "plugins": {
        ...
        "elevate-memory-store": {
            ...
            "relations_prune_max_delete": 5000,
            "relations_prune_batch_size": 500,
            "relations_incremental_vacuum_pages": 2000,
    ...
    # always goes to ~/.elevate/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ELEVATE_SKILL_DIR} and ${ELEVATE_SESSION_ID} in SKILL.md
    ...
    # This section is only needed for elevate-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "▲ *Elevate*" header.
    ...
    # stored approval from ~/.elevate/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ELEVATE_ACCEPT_HOOKS=1.
    ...
    "code_execution": {
        ...
        #     elevate's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.elevate/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.elevate/state.db.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Elevate auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @elevate:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ELEVATE_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'elevate' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Elevate API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Elevate API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Elevate API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ELEVATE_TOOL_PROGRESS and ELEVATE_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "ELEVATE_TOOL_PROGRESS": {
    ...
    "ELEVATE_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.elevate.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Elevate versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by the agent's startup resolution and mid-session ``/model`` switch.
    """
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'elevate setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Elevate won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'elevate doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ELEVATE_HOME", "~/.elevate")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ELEVATE_TOOL_PROGRESS")
            old_mode = get_env_value("ELEVATE_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ELEVATE_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ELEVATE_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ELEVATE_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ELEVATE_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ELEVATE_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_elevate_home() / "plugins"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: elevate config set <key> <value>")
    ...
    # metadata.elevate.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _elevate_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.elevate/config.yaml as-is, without merging defaults or migrating.
...
#   openai-codex (OAuth — elevate auth) — OpenAI Codex
#   nous         (OAuth — elevate auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.elevate/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.elevate/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Elevate env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.elevate/.env in place.
...
def reload_env() -> int:
    """Re-read ~/.elevate/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Elevate — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Elevate vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.elevate/.env or environment."""
...
def show_config():
    ...
    print(color("│              ▲ Elevate Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  elevate config edit     # Edit config file", Colors.DIM))
    print(color("  elevate config set <key> <value>", Colors.DIM))
    print(color("  elevate setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: elevate config set <key> <value>")
            print()
            print("Examples:")
            print("  elevate config set model anthropic/claude-sonnet-4")
            print("  elevate config set terminal.backend docker")
            print("  elevate config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'elevate config migrate' to add them")
        ...
        print("  elevate config           Show current configuration")
        print("  elevate config edit      Open config in editor")
        print("  elevate config set <key> <value>   Set a config value")
        print("  elevate config check     Check for missing/outdated config")
        print("  elevate config migrate   Update config with new options")
        print("  elevate config path      Show config file path")
        print("  elevate config env-path  Show .env file path")
        sys.exit(1)


# ── Provider profile env var injection ───────────────────────────────────────
# Surfaces api_key env vars declared by ProviderProfile entries
# (providers.list_providers()) in OPTIONAL_ENV_VARS so the setup wizard /
# config UI can ask for them.
...
# ``_inject_profile_env_vars`` surfaces them in ``elevate config`` UI so users
