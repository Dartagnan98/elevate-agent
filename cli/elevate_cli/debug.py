"""``elevate debug`` — debug tools for Elevate.

Currently supports:
    elevate debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    """Path to ``~/.elevate/pastes/pending.json``.
    ...
    ran ``elevate debug share`` repeatedly.  We now persist the schedule
    to disk and sweep expired entries on the next debug invocation.
    """
    return get_elevate_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``elevate debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``elevate debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Elevate version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `elevate debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "elevate/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``elevate debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.elevate/pastes/pending.json``
    and rely on opportunistic sweeps (``_sweep_expired_pastes``) called from
    every ``elevate debug`` invocation.  If the user never runs ``elevate debug``
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"elevate debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "elevate/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ElevateDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from elevate_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_elevate_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``elevate dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``elevate dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "debug-share: redaction enabled (force=True) for log uploads"
        )
    ...
    print(f"To delete now:  elevate debug delete <url>")

    print(f"\nShare these links with the Elevate team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: elevate debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'elevate debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``elevate debug`` call.
    ...
    # best-effort — any failure is swallowed so ``elevate debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: elevate debug <command>")
        print()
        print("Commands:")
