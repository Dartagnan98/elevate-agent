"""
Profile management for multiple isolated Elevate instances.

Each profile is a fully independent ELEVATE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.elevate/profiles/<name>/`` by default.

The "default" profile is ``~/.elevate`` itself — backward compatible,
zero migration needed.

Usage::

    elevate profile create coder          # fresh profile + bundled skills
    elevate profile create coder --clone  # also copy config, .env, SOUL.md, skills
    elevate profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    elevate -p coder chat                 # or via flag
    elevate profile use coder             # set as sticky default
    elevate profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See elevate_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `elevate profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `elevate
...
# `elevate skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Elevate infrastructure directories that only the default profile
         (``~/.elevate``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_elevate_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.elevate) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "elevate",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "elevate", "default", "test", "tmp", "root", "sudo",
})

# Elevate subcommands that cannot be used as profile names/aliases
_ELEVATE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the elevate root, NOT to the current ELEVATE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ELEVATE_HOME points outside
    ``~/.elevate``, profiles live under ``ELEVATE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_elevate_home() / "profiles"


def _get_default_elevate_home() -> Path:
    """Return the default (pre-profile) ELEVATE_HOME path.

    In standard deployments this is ``~/.elevate``.
    In Docker/custom deployments where ELEVATE_HOME is outside ``~/.elevate``
    (e.g. ``/opt/data``), returns ELEVATE_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_elevate_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Elevate installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ELEVATE_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_elevate_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, elevate subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ELEVATE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a elevate subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``elevate profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Elevate config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_elevate_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``elevate update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.elevate)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.elevate/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `elevate update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`elevate profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `elevate update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `elevate profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ELEVATE_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``elevate profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.elevate).\n"
            "To remove everything, use: elevate uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ELEVATE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ELEVATE_HOME")
    try:
        os.environ["ELEVATE_HOME"] = str(profile_dir)
        from elevate_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ELEVATE_HOME"] = old_home
        elif "ELEVATE_HOME" in os.environ:
            del os.environ["ELEVATE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.elevate/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: elevate profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ELEVATE_HOME.

    Returns ``"default"`` if ELEVATE_HOME is not set or points to ``~/.elevate``.
    Returns the profile name if ELEVATE_HOME points into ``~/.elevate/profiles/<name>``.
    Returns ``"custom"`` if ELEVATE_HOME is set to an unrecognized path.
    """
    from elevate_constants import get_elevate_home
    elevate_home = get_elevate_home()
    resolved = elevate_home.resolve()

    default_resolved = _get_default_elevate_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.elevate itself — its parent is ~/ and its
        # directory name is ".elevate", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: elevate profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.elevate itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.elevate). "
            "Specify a different name: elevate profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="elevate_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"elevate.{old_name}"
    new_host = f"elevate.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_elevate_home() / "honcho.json",
...
# ---------------------------------------------------------------------------
# Shell completion (Elevate-specific — preserved across Hermes re-ports)
# ---------------------------------------------------------------------------

def generate_bash_completion() -> str:
    """Generate a bash completion script for elevate profile names."""
    return '''# Elevate profile completion
# Add to ~/.bashrc: eval "$(elevate completion bash)"

_elevate_profiles() {
    local profiles_dir="$HOME/.elevate/profiles"
...
_elevate_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_elevate_profiles)" -- "$cur"))
        return
    fi
...
complete -F _elevate_completion elevate
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for elevate profile names."""
    return '''#compdef elevate
# Elevate profile completion
# Add to ~/.zshrc: eval "$(elevate completion zsh)"

_elevate() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.elevate/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.elevate/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ELEVATE_HOME path string.

    Called early in the CLI entry point, before any elevate modules
    are imported, to set the ELEVATE_HOME environment variable.
    """
