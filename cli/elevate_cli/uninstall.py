...
def remove_path_from_shell_configs():
    """Remove Elevate PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing elevate or elevate PATH entries
            new_lines = []
            skip_next = False
            ...
            for line in content.split('\n'):
                ...
                # Remove any PATH line containing elevate
                if 'elevate' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the elevate wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "elevate",
        Path("/usr/local/bin/elevate"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains elevate_cli reference)
                content = wrapper.read_text()
                if 'elevate_cli' in content or 'elevate' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from elevate_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from elevate_cli.gateway import get_launchd_plist_path
...
def _is_default_elevate_home(elevate_home: Path) -> bool:
    """Return True when ``elevate_home`` points at the default (non-profile) root."""
    try:
        from elevate_constants import get_default_elevate_root
        return elevate_home.resolve() == get_default_elevate_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ELEVATE_HOME directory.

    We shell out to ``elevate -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ELEVATE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m elevate_cli.main` so we don't depend on a `elevate`
    #    wrapper that may be half-removed mid-uninstall.
    elevate_invocation = [_sys.executable, "-m", "elevate_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                elevate_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ELEVATE_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.elevate/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.elevate/ for future reinstall
    """
    ...
    is_default_profile = _is_default_elevate_home(elevate_home)
    ...
    print(f"  Config:  {elevate_home / 'config.yaml'}")
    print(f"  Secrets: {elevate_home / '.env'}")
    print(f"  Data:    {elevate_home / 'cron/'}, {elevate_home / 'sessions/'}, {elevate_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ELEVATE_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Elevate data!", Colors.RED, Colors.BOLD))
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile ELEVATE_HOME dirs live under
        ...
        try:
            if elevate_home.exists():
                shutil.rmtree(elevate_home)
                log_success(f"Removed {elevate_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {elevate_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {elevate_home}")
